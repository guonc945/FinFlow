...
const Houses = () => {
    ...
    const columns = [
        { key: 'house_id' as keyof House, title: '房屋ID', width: 120 },
        {
            ...
            title: '所属园区',
            width: 150,
            render: (val: unknown) => <span className="text-secondary font-medium">{String(val || '-')}</span>
        },
        {
            key: 'house_name' as keyof House,
            title: '房屋名称',
            render: (val: unknown, row: House): ReactNode => (
        ...
        },
        {
            ...
            title: '面积 (m²)',
            width: 120,
            render: (val: unknown) => val ? `${Number(val).toFixed(2)}` : '-'
        },
        {
            key: 'user_num' as keyof House,
            title: '住户',
            width: 220,
            render: (_value: unknown, row: House): ReactNode => {
        ...
        },
        {
            ...
            render: (val: unknown) => val && typeof val === 'object' ? (
                <div className="flex flex-col">
                    ...
                    <span className="text-xs text-slate-400 font-mono">wtw8: {String((val as { wtw8_number?: unknown }).wtw8_number || '')}</span>
                </div>
        ...
        },
        {
            ...
            title: '采集时间',
            render: (val: unknown): ReactNode => new Date(String(val)).toLocaleString()
        },
        {
            key: 'actions' as keyof House,
            title: '操作',
            render: (_value: unknown, item: House): ReactNode => (
