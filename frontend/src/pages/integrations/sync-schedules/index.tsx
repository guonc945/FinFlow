...
const normalizeExecutionText = (value?: string | null) => {
    const text = String(value || '').trim();
    return text || '';
};

const buildExecutionSummary = (execution: SyncScheduleExecution) => {
    const summary = normalizeExecutionText(execution.summary);
    if (summary) return summary;

    const errorMessage = normalizeExecutionText(execution.error_message);
    if (errorMessage) return errorMessage;

    switch (execution.status) {
        case 'success':
            return execution.success_targets > 0
                ? `本次执行完成，${execution.success_targets} 个目标成功。`
                : '本次执行完成。';
        case 'partial':
            return `本次执行部分完成，成功 ${execution.success_targets} 个，失败 ${execution.failed_targets} 个。`;
        case 'failed':
            return execution.failed_targets > 0
                ? `本次执行失败，${execution.failed_targets} 个目标失败。`
                : '本次执行失败。';
        case 'running':
            return '本次执行仍在进行中。';
        default:
...
const buildTargetResultMessage = (target: SyncScheduleExecutionTargetResult, label?: string) => {
    const message = normalizeExecutionText(target.message);
    if (message) return message;

    const targetLabel = label || target.code;
    switch (target.status) {
        case 'success':
            return `${targetLabel}执行完成。`;
        case 'failed':
            return `${targetLabel}执行失败。`;
        case 'partial':
            return `${targetLabel}部分成功。`;
        case 'running':
            return `${targetLabel}执行中。`;
        default:
            return '暂无执行摘要';
    }
};
...
export const SyncSchedulesPage = ({ moduleType = 'data-sync' }: { moduleType?: ScheduleModuleType }) => {
    ...
    const loadSelectedExecutions = useCallback(async (scheduleId: number) => {
        const data = await getSyncScheduleExecutions(scheduleId, 20);
        setSelectedExecutions(data);
    }, []);

    const refreshAll = useCallback(async (scheduleId?: number | null) => {
        await loadBaseData();
        if (scheduleId) {
            await loadSelectedExecutions(scheduleId);
        }
    }, [loadBaseData, loadSelectedExecutions]);
    ...
    }, [copy.loadErrorText, loadBaseData, showToast]);
    ...
    }, [loadSelectedExecutions, selectedScheduleId]);
    ...
    }, [loadBaseData, loadSelectedExecutions, selectedScheduleId]);
    ...
    const handleAccountBookChange = (accountBookNumber: string) => {
        const book = accountBooks.find((item) => item.number === accountBookNumber);
        setFormState((prev) => ({
            ...prev,
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            showToast('error', '保存失败', apiError.response?.data?.detail || apiError.message || `${copy.title}保存失败`);
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleToggle = async (schedule: SyncSchedule) => {
        try {
            ...
            const apiError = error as ApiErrorLike;
            console.error(error);
            showToast('error', '操作失败', apiError.response?.data?.detail || apiError.message || '状态切换失败');
        }
    };
            ...
            {schedules.map((schedule) => (
                ...
                >
                    ...
                    <div className="schedule-footer">
                        ...
                        <p>{normalizeExecutionText(schedule.last_message) || '暂无执行结果'}</p>
                    </div>
                </article>
            ))}
        </div>
                ...
                <div className="execution-list">
                    {selectedExecutions.map((execution) => (
                        <div key={execution.id} className={`execution-card ${execution.status}`}>
                            ...
                            <p>{buildExecutionSummary(execution)}</p>
                            ...
                            {execution.result_payload.length > 0 && (
                                <>
                                    ...
                                    <div className="execution-target-detail-list">
                                        {execution.result_payload.map((target) => (
                                            (() => {
                                                const label = targetMap.get(target.code)?.label || target.code;
                                                return (
                                            <div
                                            ...
                                            >
                                                ...
                                                <p>{buildTargetResultMessage(target, label)}</p>
                ...
                <div className="voucher-form-panel">
                    ...
                    <div className="form-grid two-columns">
                        <label className="field">
                            ...
                            <Select
                                value={formState.account_book_number}
                                onChange={handleAccountBookChange}
                                options={[
                                    { value: '', label: requiresAccountBookSelection ? '请选择账簿' : '不限定账簿' },
                                    ...accountBooks.map((book) => ({
                                        value: book.number || '',
                                        label: (book.number || '未编码') + ' · ' + book.name,
                                    })),
                                ]}
                            />
                            <small>
