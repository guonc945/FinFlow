...
const APIManager = () => {
    ...
    const handleSaveApi = async () => {
        try {
            const targetServiceId = currentApi.service_id;
            if (!targetServiceId) {
                alert('请先为此接口选择所属服务');
                return;
            }
            ...
            const message = (error as ApiErrorLike).response?.data?.detail || (error as ApiErrorLike).message;
            alert('保存失败: ' + String(message || '未知错误'));
        }
    };
    ...
    const handleTestApi = async () => {
        ...
        try {
            ...
            setTestResult({
                success: false,
                error: String((err as ApiErrorLike).response?.data?.error || (err as ApiErrorLike).message || '请求执行失败')
            });
        } finally {
            setIsTesting(false);
        }
    ...
    const openEditApi = (api: ExternalApi) => {
        setCurrentApi(api);
    ...
    const handleImportApis = async () => {
        ...
        try {
            ...
            let count = 0;
            for (const [path, methods] of Object.entries(data.paths as Record<string, Record<string, OpenApiOperation>>)) {
            ...
            alert('导入失败: ' + String((e as ApiErrorLike).message || '未知错误'));
        }
    };
                                ...
                                {!isCollapsed && (
                                    <div className="api-controls animate-in fade-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
                                        ...
                                        <Select
                                            ...
                                            style={{ width: '130px', minWidth: '90px', color: '#334155', fontSize: '0.85rem', height: '34px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}
                                            value={selectedServiceId}
                                            onChange={(v) => setSelectedServiceId(v)}
                                            options={[
                                                { value: 'ALL', label: '全部服务' },
                                                ...services.map(s => ({ value: s.id.toString(), label: s.display_name || s.service_name })),
                                            ]}
                                        />
                                        <Select
                                            className="form-input"
                                            style={{ width: '110px', minWidth: '80px', color: '#334155', fontSize: '0.85rem', height: '34px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}
                                            ...
                                            <div key={api.id} className="api-card">
                                                <div className="card-method-header">
                                                    ...
                                                    <div className="api-card-actions">
                                                        <button
                                                            type="button"
                                                            className="api-card-action-btn edit"
                                                            title="编辑接口"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                e.stopPropagation();
                                                        ...
                                                        <button
                                                            type="button"
                                                            className="api-card-action-btn delete"
                                                            title="删除接口"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                e.stopPropagation();
                            ...
                            <div className="modal-content" style={{ maxWidth: '750px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{
                                    ...
                                    {[
                                        ...
                                        <button
                                            key={tab.key}
                                            onClick={() => setEditTabActive(tab.key as 'basic' | 'request' | 'response' | 'notes' | 'test')}
                                            style={{
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1rem', marginBottom: '1.5rem' }}>
                                                <div className="form-group" style={{ marginBottom: 0 }}>
                                                    ...
                                                    <Select
                                                        ...
                                                        onChange={(v) => setCurrentApi({ ...currentApi, method: v as ExternalApi['method'] })}
                                                        options={[
                                                            { value: 'GET', label: 'GET' },
                                            ...
                                            <div className="form-group">
                                                ...
                                                <div style={{ display: 'flex', gap: '1rem' }}>
                                                    <div style={{ flex: 1 }}>
                                                        <Select
                                                            className="form-select"
                                                            value={String(currentApi.service_id ?? (services[0]?.id ?? ''))}
                                                            onChange={(v) => setCurrentApi({ ...currentApi, service_id: Number(v) })}
                                                            options={services.map(s => ({ value: String(s.id), label: s.display_name || s.service_name }))}
                                                        />
                                                    ...
                                                    <div style={{ flex: 1, borderLeft: '1px solid #e2e8f0', paddingLeft: '1rem' }}>
                                                        ...
                                                        <Select
                                                            ...
                                                            onChange={(v) => setCurrentApi({ ...currentApi, category: v })}
                                                            options={MODAL_CATEGORY_OPTIONS.map(opt => ({ value: opt.value, label: opt.label }))}
                                                        />
                                                    ...
                                                    <div style={{
                                                        ...
                                                        <div style={{
                                                            ...
                                                            <div style={{
                                                                ...
                                                                {String(testResult.status_code || 'Error')}
                                                            </div>
                                                        </div>
                                                        <div style={{
                                                            padding: '1rem', borderRadius: '8px',
                                                            ...
                                                            <div style={{
                                                                ...
                                                                <Clock size={18} />
                                                                {String(testResult.duration_ms || 0)} ms
                                                            </div>
                                                        </div>
                                                    </div>
                            ...
                            <div className="modal-content" style={{ maxWidth: '500px' }}>
                                ...
                                <div className="modal-body">
                                    ...
                                    <div className="form-group">
                                        ...
                                        <Select
                                            className="form-select"
                                            value={String(importTargetServiceId ?? (services[0]?.id ?? ''))}
                                            onChange={(v) => setImportTargetServiceId(Number(v))}
                                            options={services.map(s => ({ value: String(s.id), label: s.display_name || s.service_name }))}
                                        />
                                    </div>
