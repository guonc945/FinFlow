...
const CredentialsManager: React.FC<CredentialsManagerProps> = ({ hideHeader = false }) => {
    ...
    const handleDeleteService = async (id: number) => {
        if (!confirm('确定要删除此接口认证配置吗？删除后相关集成将和服务断开。')) return;
        try {
            await axios.delete(`${API_BASE_URL}/external/services/${id}`);
            showToast('success', '删除成功', '接口认证配置已移除');
            fetchServices();
        } catch (_error) {
    ...
    const handleSaveService = async (serviceData: Partial<ExternalService>) => {
        try {
            ...
            if (payload.id && !payload.app_secret?.trim()) {
                delete payload.app_secret;
            }
            if (serviceData.id) {
                await axios.put(`${API_BASE_URL}/external/services/${serviceData.id}`, payload);
            } else {
                await axios.post(`${API_BASE_URL}/external/services`, payload);
            }
            setIsEditingService(false);
            showToast('success', '保存成功', '接口认证配置已更新');
            fetchServices();
        } catch (error) {
    ...
    const handleRefreshToken = async (id: number) => {
        try {
            ...
            if (res.data.success) {
                showToast('success', '认证刷新成功', `有效期至: ${new Date(res.data.expires_at).toLocaleString()}`);
                fetchServices();
            }
        } catch (error: unknown) {
            const message = (error as ApiErrorLike).response?.data?.detail || (error as ApiErrorLike).message;
            showToast('error', '刷新失败', typeof message === 'string' ? message : '无法连接到远程服务');
        }
    };
            ...
            <div className="credentials-grid">
                    ...
                    >
                        ...
                        <div className="card-body">
                            ...
                            <div className="info-row">
                                ...
                                <span className="info-value">
                                    {service.has_app_secret ? '已配置' : '未配置'}
                                </span>
                            </div>
                ...
                {services.length === 0 && (
                    <div className="empty-state-card">
                        ...
                        <h3 className="text-slate-600 font-bold mb-1">暂无接口认证</h3>
                        <p className="text-slate-400 text-sm mb-6">配置您的第一个外部系统连接认证</p>
                        <button
