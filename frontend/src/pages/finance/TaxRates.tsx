...
const TaxRates = () => {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            showToast('success', '同步任务已提交', '系统正在后台同步税率档案，请稍后刷新列表查看结果');
            setTimeout(() => {
                void fetchTaxRates();
            }, 3000);
        } catch (error: any) {
            showToast(
                'error',
                '同步异常',
                error.response?.data?.error || error.response?.data?.detail || error.message || '税率同步请求未能成功发起',
            );
        } finally {
            setLoading(false);
        }
                    ...
                    <div className="filter-content-wrapper fade-in">
                        <div className="action-row">
                            ...
                            <div className="page-size-selector flex items-center gap-2 text-slate-600">
                                <span>启用状态</span>
                                <select value={selectedEnable} onChange={(e) => setSelectedEnable(e.target.value)} className="page-size-select">
                                    {ENABLE_FILTERS.map((item) => (
                            ...
                            <div className="page-size-selector flex items-center gap-2 text-slate-600">
                                <span>数据状态</span>
                                <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="page-size-select">
