...
const BankAccounts = () => {
    ...
    useEffect(() => {
        const handleSyncFinished = (event: Event) => {
            ...
            if (customEvent.detail?.moduleCode !== 'kd-bank-accounts') {
                return;
            }
            void fetchAccounts();
        };
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            notifyFinanceSyncStarted('kd-bank-accounts');
            showToast('success', '同步任务已提交', '系统正在后台处理银行账户同步，完成后将自动刷新水位和列表');
        } catch (err: unknown) {
    ...
    return (
        <div className="page-container fade-in bank-accounts-page">
            <div className={`bills-filter-section ${isFilterCollapsed ? 'collapsed' : ''}`}>
                ...
                {!isFilterCollapsed && (
                    <div className="filter-content-wrapper fade-in">
                        ...
                        <FinanceSyncStatus moduleCode="kd-bank-accounts" />
                    </div>
                )}
            </div>
