...
const AuxiliaryDataPage = () => {
        ...
        const fetchAllCategories = async () => {
            try {
                ...
                if (res.data && Array.isArray(res.data.items)) {
                    setCategories(res.data.items.map((c: CategoryOption) => ({ number: c.number, name: c.name })));
                }
            } catch (err) {
    ...
    }, [currentPage, debouncedSearchTerm, pageSize, selectedCategories, showToast]);
    ...
    useEffect(() => {
        const handleSyncFinished = (event: Event) => {
            ...
            if (customEvent.detail?.moduleCode !== 'auxiliary-data') {
                return;
            }
            void fetchAuxData();
        };
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            notifyFinanceSyncStarted('auxiliary-data');
            showToast('success', '同步任务已提交', '系统正在后台处理辅助资料同步，完成后将自动刷新水位和列表');
        } catch (err: unknown) {
    ...
    return (
        <div className="page-container fade-in aux-data-page">
            <div className={`bills-filter-section ${isFilterCollapsed ? 'collapsed' : ''}`}>
                <div className="filter-header-row">
                    <div className="flex items-center gap-2">
                        <Filter size={16} className="text-primary" />
                        <h4 className="text-sm font-semibold">辅助资料同步与筛选</h4>
                    </div>
                ...
                {!isFilterCollapsed && (
                    <div className="filter-content-wrapper fade-in">
                        ...
                        <FinanceSyncStatus moduleCode="auxiliary-data" />
                    </div>
                )}
            </div>
