...
type AccountingSubjectCheckItem = {
...
const AccountingSubjects = () => {
    ...
    }, [currentPage, debouncedSearchTerm, pageSize, selectedType, showToast]);
    ...
    useEffect(() => {
        const handleSyncFinished = (event: Event) => {
            ...
            if (customEvent.detail?.moduleCode !== 'accounting-subjects') {
                return;
            }
            void fetchSubjects();
        };
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            notifyFinanceSyncStarted('accounting-subjects');
            showToast('success', '同步任务已提交', '系统正在后台处理会计科目同步，完成后将自动刷新水位和列表');
        } catch (err: unknown) {
            const apiError = err as ApiErrorLike;
            showToast('error', '同步异常', apiError.response?.data?.detail || apiError.message || '由于网络或授权原因，同步请求未能成功发起');
        } finally {
            setLoading(false);
        }
    };

    const handleToggleExpand = (id: string, e: MouseEvent) => {
        e.stopPropagation();
            ...
            <div className={`bills-filter-section ${isFilterCollapsed ? 'collapsed' : ''}`}>
                ...
                {!isFilterCollapsed && (
                    <div className="filter-content-wrapper fade-in">
                        ...
                        <FinanceSyncStatus moduleCode="accounting-subjects" />
                    </div>
                )}
            </div>

            <div className="accounting-subjects-workspace flex gap-4 mt-2 flex-1 overflow-hidden min-h-0">
                            ...
                            <tbody className="table-body">
                                {filteredSubjects.length > 0 ? (
                                    filteredSubjects.map(subject => {
                                        let parsedCheckItems: AccountingSubjectCheckItem[] = [];
                                        try {
                                            if (subject.check_items) {
                                                const parsed = JSON.parse(subject.check_items) as unknown;
                                            ...
                                            <tr key={subject.id} className="table-row">
                                                ...
                                                <td className="table-cell">
                                                    <div className="check-items-tags">
                                                        {parsedCheckItems.length > 0 ? parsedCheckItems.map((item, idx: number) => (
