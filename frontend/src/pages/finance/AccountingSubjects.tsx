...
const AccountingSubjects = () => {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            showToast('error', '同步异常', err.response?.data?.detail || err.message || '由于网络或授权原因，同步请求未能成功发送');
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredSubjects = useMemo(() => {
        ...
        // Tree mode: filter by visibility
        const isParentExpanded = (subject: AccountingSubject): boolean => {
            ...
            // If parent not found in current page, we might be orphans, but let's show anyway for now
            // Or look for a parent earlier in the list
            ...
            <div className="flex gap-4 mt-2 flex-1 overflow-hidden min-h-0">
                ...
                <div className="type-nav-sidebar glass-effect card-shadow min-w-[200px] flex flex-col h-full overflow-hidden">
                    ...
                    <div className="nav-list p-2 overflow-y-auto flex-1 custom-scrollbar">
                        <div
                            className={`nav-item-custom ${selectedType === 'all' ? 'active' : ''}`}
                        ...
                        {Object.entries(ACCOUNT_TYPE_MAP).map(([key, name]) => (
                            <div
                                key={key}
                                className={`nav-item-custom ${selectedType === key ? 'active' : ''}`}
                                        ...
                                        return (
                                            <tr key={subject.id} className="table-row">
                                                <td className="table-cell text-center font-medium text-slate-400">
                                                    {(currentPage - 1) * pageSize + filteredSubjects.findIndex(s => s.id === subject.id) + 1}
                                                </td>
                                                <td className="table-cell">
                                                    <div
                                                        className="subject-cell-content"
                                                        style={{ paddingLeft: searchTerm ? 0 : (subject.level - 1) * 24 }}
                                                    >
                                                    ...
                                                    <div className="flex flex-col gap-1">
                                                        {subject.account_type_number && ACCOUNT_TYPE_MAP[subject.account_type_number] && (
                                                            <span className="text-xs font-medium text-slate-500 bg-slate-100 px-2 py-0.5 rounded w-fit">
                                                                {ACCOUNT_TYPE_MAP[subject.account_type_number]}
                                                            </span>
                                                        )}
                                                ...
                                                <td className="table-cell">
                                                    <span className={`direction-tag ${subject.direction === '1' ? 'dr' : 'cr'}`}>
                                                        {subject.direction === '1' ? '借' : '贷'}
                                                    </span>
                                                </td>
                                                <td className="table-cell">
                                                    <div className="check-items-tags">
                                                        {parsedCheckItems.length > 0 ? (
                                                            parsedCheckItems.map((item: any, idx: number) => (
                                                                <span key={idx} className="check-item-tag">
                                                                    {item.asstactitem_name}
                                                                </span>
                                                            ))
                                                        ) : (
                                                ...
                                                <td className="table-cell">
                                                    {subject.is_leaf ?
                                                        <Check size={16} className="leaf-icon" /> :
                                                        <span className="text-slate-300">-</span>
                                                    }
                                                </td>
                                            </tr>
                ...
                <div className="modal-overlay">
                    <div className="modal-content confirm-modal animate-fade-in">
                        ...
                        <div className="modal-body">
                            <h2 className="modal-title">同步会计科目</h2>
                            <p className="modal-desc">确定要从外部系统同步会计科目吗？这将会更新现有的科目数据，同步过程将在后台异步进行。</p>
                        </div>
