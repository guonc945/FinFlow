...
const AccountBookPage = () => {
    ...
    useEffect(() => {
        const handleSyncFinished = (event: Event) => {
            ...
            if (customEvent.detail?.moduleCode !== 'kd-account-books') {
                return;
            }
            void fetchAccountBooks();
        };
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            notifyFinanceSyncStarted('kd-account-books');
            showToast('success', '同步任务已提交', '系统正在后台同步账簿数据，完成后将自动刷新水位和列表');
        } catch (err: unknown) {
            const apiError = err as ApiErrorLike;
            showToast('error', '同步异常', apiError.response?.data?.error || apiError.message || '同步请求未能成功发起');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="page-container fade-in account-book-page">
            <div className={`bills-filter-section ${isFilterCollapsed ? 'collapsed' : ''}`}>
                ...
                {!isFilterCollapsed && (
                    <div className="filter-content-wrapper fade-in">
                        ...
                        <FinanceSyncStatus moduleCode="kd-account-books" />
                    </div>
                )}
            </div>
