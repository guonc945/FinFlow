...
const Customers = () => {
    ...
    useEffect(() => {
        const handleSyncFinished = (event: Event) => {
            ...
            if (customEvent.detail?.moduleCode !== 'customers') {
                return;
            }
            void fetchCustomers();
        };
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            notifyFinanceSyncStarted('customers');
            showToast('success', '同步任务已提交', '系统正在后台处理客户同步，完成后将自动刷新水位和列表');
        } catch (err: unknown) {
    ...
    return (
        <div className="page-container fade-in customers-page">
            <div className={`bills-filter-section ${isFilterCollapsed ? 'collapsed' : ''}`}>
                ...
                {!isFilterCollapsed && (
                    <div className="filter-content-wrapper fade-in">
                        ...
                        <FinanceSyncStatus moduleCode="customers" />
                    </div>
                )}
            </div>
