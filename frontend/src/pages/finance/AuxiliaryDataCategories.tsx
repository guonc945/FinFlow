...
const AuxiliaryDataCategoriesPage = () => {
    ...
    useEffect(() => {
        const handleSyncFinished = (event: Event) => {
            ...
            if (customEvent.detail?.moduleCode !== 'auxiliary-data-categories') {
                return;
            }
            void fetchCategories();
        };
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            notifyFinanceSyncStarted('auxiliary-data-categories');
            showToast('success', '同步任务已提交', '系统正在后台同步辅助资料分类数据，完成后将自动刷新水位和列表');
        } catch (err: unknown) {
            const apiError = err as ApiErrorLike;
            showToast('error', '同步失败', apiError.response?.data?.detail || apiError.message || '无法启动同步任务');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="page-container fade-in aux-data-categories-page">
            <div className={`bills-filter-section ${isFilterCollapsed ? 'collapsed' : ''}`}>
                <div className="filter-header-row">
                    <div className="flex items-center gap-2">
                        <Filter size={16} className="text-primary" />
                        <h4 className="text-sm font-semibold">辅助资料分类同步与筛选</h4>
                    </div>
                ...
                {!isFilterCollapsed && (
                    <div className="filter-content-wrapper fade-in">
                        ...
                        <FinanceSyncStatus moduleCode="auxiliary-data-categories" />
                    </div>
                )}
            </div>
