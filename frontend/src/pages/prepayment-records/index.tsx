...
type PrepaymentRecordColumn = {
    key: keyof PrepaymentRecord | string;
    title: string;
    render?: (value: unknown, row: PrepaymentRecord) => ReactNode;
};
...
const PrepaymentRecords = () => {
    ...
    useEffect(() => {
        const handleOutsideClick = (event: MouseEvent) => {
            ...
            if ((event.target as HTMLElement)?.closest('.prepayment-export-action')) return;
    ...
    const columns = useMemo<PrepaymentRecordColumn[]>(() => [
        { key: 'id', title: '记录ID' },
    ...
    const handleExportPrepaymentRecords = useCallback(async (mode: 'current' | 'all') => {
        try {
            let exportData = items;
            
            if (mode === 'all') {
                showToast('info', '正在导出', '正在加载全部数据...');
                const allItems: PrepaymentRecord[] = [];
                const batchSize = 100;
                let skip = 0;
                let hasMore = true;
                ...
                while (hasMore) {
                    ...
                    const resp = await getPrepaymentRecords(params);
            ...
            exportTableToCsv(exportColumns, exportData as unknown as Array<Record<string, unknown>>, '预存款管理');
    ...
    return (
        <div className="page-container prepayment-records-page">
            <div className="bills-filter-section">
                <div className="action-row flex-wrap">
                            ...
                            <div className="prepayment-export-action" style={{ position: 'relative' }}>
                                <button
                                ...
                                {isExportMenuOpen && (
                                    ...
                                    >
                                        <button
                                            ...
                                            onClick={() => {
                                                setIsExportMenuOpen(false);
                                                void handleExportPrepaymentRecords('current');
                                            }}
                                        >
                                            导出当前页
                                        </button>
                                        <button
                                            ...
                                            onClick={() => {
                                                setIsExportMenuOpen(false);
                                                void handleExportPrepaymentRecords('all');
                                            }}
                                        >
                                            导出全部
                                        </button>
