...
type MeUser = {
    ...
    org_name?: string;
    role?: string;
    account_books?: AccountBookOption[];
};
...
const Account = () => {
    ...
    const [uiDensity, setUiDensity] = useState<'comfortable' | 'compact'>(
        localStorage.getItem('ff_ui_density') === 'compact' ? 'compact' : 'comfortable'
    );
    const [savingPrefs, setSavingPrefs] = useState(false);

    const accountBooks = useMemo(() => (me?.account_books || []) as AccountBookOption[], [me]);
    ...
    const fetchAll = async () => {
        ...
        try {
            ...
            const books = (meData.account_books || []) as AccountBookOption[];
            ...
            console.error(e);
            showToast('error', '加载失败', getErrorMessage(e, '无法获取用户信息'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            showToast('error', '保存失败', getErrorMessage(e, '无法保存个人信息'));
        } finally {
    ...
    const handleChangePassword = async () => {
        ...
        try {
            ...
            await login(me.username, pwd.current);

            await updateUser(me.id, { password: pwd.next });
            setPwd({ current: '', next: '', confirm: '' });
            showToast('success', '修改成功', '密码已更新，请牢记新密码');
        } catch (e: unknown) {
            showToast('error', '修改失败', getErrorMessage(e, '无法修改密码'));
        } finally {
    ...
    const navItems: Array<{ key: SectionKey; label: string; sub: string; icon: LucideIcon }> = [
                        ...
                        <section className="account-card">
                            ...
                            <div className="account-form one-col">
                                <div className="account-field">
                                    ...
                                    <Select
                                        value={defaultAccountBookId}
                                        onChange={(v) => setDefaultAccountBookId(v)}
                                        disabled={accountBooks.length === 0}
                                        options={
                                            accountBooks.length === 0
                                                ? [{ value: '', label: '暂无账套权限' }]
                                                : accountBooks.map(b => ({ value: String(b.id), label: b.name }))
                                        }
                                    />
                                </div>
                            </div>
