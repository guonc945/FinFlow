...
type ReceiptVoucherSkippedBill = {
    bill_id?: number | string | null;
    community_id?: number | string | null;
    reason?: string | null;
};

type ReceiptVoucherPreviewData = Record<string, unknown> & {
    source_bills?: BillVoucherPushStatus[];
    selected_bills?: BillVoucherPushStatus[];
    skipped_bills?: ReceiptVoucherSkippedBill[];
    source_bill_push_summary?: PushStatusSummary;
    selected_bill_push_summary?: PushStatusSummary;
    push_blocked?: boolean;
    push_block_reason?: string;
};
...
const getDrilldownColumns = (section: ReceiptBillDrilldownSection) => {
    if (section.source_type === 'bills') {
        return [
            ...
            { key: 'in_month', title: '所属月份' },
            {
                key: 'amount',
                title: '金额',
                render: (v: unknown) => `¥${Number(v || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}`,
            },
            { key: 'pay_status_str', title: '缴费状态' },
            {
                key: 'pay_time',
                title: '缴费时间',
                render: (v: unknown) => formatUnixTimestamp(v as number | null | undefined),
            },
        ];
    }
    ...
    if (section.source_type === 'deposit_records') {
        return [
            ...
            { key: 'pay_channel_str', title: '渠道' },
            {
                key: 'operate_time',
                title: '操作时间',
                render: (_value: unknown, record: DepositRecord) => formatUnixTimestamp(record.operate_time || record.pay_time),
            },
            {
                key: 'remark',
                title: '备注',
    ...
    return [
        ...
        { key: 'pay_channel_str', title: '渠道' },
        {
            key: 'operate_time',
            title: '操作时间',
            render: (_value: unknown, record: PrepaymentRecord) => formatUnixTimestamp(record.operate_time || record.pay_time),
        },
        {
            key: 'remark',
            title: '备注',
...
const SyncTriggerModal = ({
    ...
    useEffect(() => {
        const onClick = (e: MouseEvent) => {
            if (!communityDropdownRef.current?.contains(e.target as Node)) {
    ...
    const isCompleted = ['completed', 'failed', 'partially_completed'].includes(syncState.status);
    const isFailed = syncState.status === 'failed';
    const percentage = syncState.total > 0 ? Math.round((syncState.current / syncState.total) * 100) : 0;

    return (
        <div className="sync-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
            <div className="sync-modal" style={{ width: '720px', maxWidth: '96vw' }}>
                <div className="sync-header">
                    ...
                    {!isSyncing || isCompleted ? (
                        <div style={{ padding: '1.5rem 0' }}>
                            <div style={{
                                ...
                                <div style={{ marginBottom: '0.75rem', fontWeight: 600, color: '#475569' }}>
                                    园区范围
                                </div>
                                <div className="selection-group" ref={communityDropdownRef} style={{ maxWidth: '100%' }}>
                                    <div className={`custom-select-trigger ${isCommunityDropdownOpen ? 'active' : ''}`} onClick={() => setIsCommunityDropdownOpen(v => !v)}>
                                        <div className="trigger-content">
                                            <span className={communityFilter.length === 0 ? 'placeholder' : 'text-xs truncate'} style={{ maxWidth: '100%' }}>
                                    ...
                                    {isCommunityDropdownOpen && (
                                        <div className="custom-dropdown card-shadow slide-up" style={{ zIndex: 100, width: '100%' }}>
                            ...
                            <div style={{
                                ...
                                <div style={{ textAlign: 'center' }}>
                                    <RefreshCw size={48} style={{ color: '#64748b', marginBottom: '1rem' }} />
                                    <p style={{ color: '#475569', fontWeight: 600, marginBottom: '0.5rem' }}>
                                        {isCompleted ? (isFailed ? '同步失败' : '同步完成') : '准备同步收款单据'}
                                    </p>
                                    <p className="text-secondary text-sm">
                                        {isCompleted
                                            ? (isFailed ? '收款单据联动同步过程中发生错误' : `已完成 ${syncState.total} 个园区的收款单据及关联模块同步`)
                                            : '点击开始同步按钮，将依次同步运营账单、押金、预存款、收款单据'}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <>
                            <div className="progress-container">
                                <div className="progress-info">
                                    <span className="font-bold text-primary">{percentage}%</span>
                                    <span className="text-secondary text-sm">{syncState.current} / {syncState.total} 园区</span>
                                </div>
                                <div className="progress-bar-bg">
                                    <div className={`progress-bar-fill ${isFailed ? 'bg-error' : ''}`} style={{ width: `${percentage}%` }}></div>
                                </div>
                ...
                <div className="sync-footer">
                    {isCompleted ? (
                        ...
                        <button className="btn btn-primary w-full" onClick={onSync}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxHeight: '68vh', overflow: 'auto' }}>
                            {sections.map((section) => (
                                <div key={section.relation_key} style={{ border: '1px solid #e2e8f0', borderRadius: '0.75rem', overflow: 'hidden' }}>
                                    ...
                                    <DataTable<Bill | DepositRecord | PrepaymentRecord>
                                        columns={getDrilldownColumns(section) as unknown as Array<{
                                            key: string;
                                            title: ReactNode;
                                            render?: (value: unknown, record: Bill | DepositRecord | PrepaymentRecord, index: number) => ReactNode;
                                            width?: number | string;
                                            fixed?: 'left' | 'right';
                                            className?: string;
                                        }>}
                                        data={(section.items || []) as unknown as Array<Bill | DepositRecord | PrepaymentRecord>}
                                        loading={false}
...
const ReceiptBills = () => {
    ...
    const [isSyncTriggerOpen, setIsSyncTriggerOpen] = useState(false);
    ...
    const [voucherPreview, setVoucherPreview] = useState<{ isOpen: boolean; data: ReceiptVoucherPreviewData | null; isLoading: boolean; error: string | null }>({
    ...
    const summarizePushStatuses = useCallback((items: Array<Pick<BillVoucherPushStatus, 'push_status'>>): PushStatusSummary => items.reduce((acc, item) => {
        acc.total += 1;
    ...
    const handleConfirmModalConfirm = useCallback(async () => {
        ...
        try {
            ...
            showToast('error', '操作失败', getErrorMessage(err, '操作失败'));
        } finally {
    ...
    const fetchReceiptBills = useCallback(async () => {
        ...
        try {
            ...
            showToast('error', '收款账单加载失败', getErrorMessage(err, '加载失败'));
        } finally {
            setIsLoading(false);
        }
    ...
    useEffect(() => {
        const onClick = (e: MouseEvent) => {
            ...
            if (target?.closest('.receipt-export-action')) return;
    ...
    const handleSync = useCallback(async () => {
        try {
            ...
            showToast('error', '同步失败', getErrorMessage(err, '同步失败'));
            setIsSyncing(false);
        }
    ...
    const openReceiptDrilldown = useCallback(async (receiptBill: ReceiptBill) => {
        ...
        try {
            ...
            showToast('error', '关联数据加载失败', getErrorMessage(err, '加载失败'));
    ...
    const openReceiptPeriodStats = useCallback(async (receiptBill: ReceiptBill) => {
        ...
        try {
            ...
            showToast('error', '期间统计加载失败', getErrorMessage(err, '加载失败'));
    ...
    const notifySkippedBills = useCallback((skippedBills: ReceiptVoucherSkippedBill[]) => {
        if (!Array.isArray(skippedBills) || skippedBills.length === 0) return;

        const preview = skippedBills
            .slice(0, 5)
            .map((b) => `${b.community_id}:${b.bill_id}`)
            .join(', ');
        const reasons = Array.from(
            new Set(
                skippedBills
                    .map((b) => String(b?.reason || '').trim())
                    .filter(Boolean)
            )
        );
    ...
    const handlePreviewVoucherForReceipt = useCallback(async (receipt: ReceiptBill) => {
        ...
        try {
            const result = await previewVoucherForReceipt(Number(receipt.id), Number(receipt.community_id)) as ReceiptVoucherPreviewData;
            ...
            setVoucherPreview({
                ...
                error: getErrorMessage(err, '预览失败')
            });
        }
    }, [notifySkippedBills]);
    ...
    const handlePreviewBatchVoucher = useCallback(async () => {
        ...
        setVoucherPreview({ isOpen: true, data: null, isLoading: true, error: null });
        try {
            const result = await previewBatchVoucherForReceipts(receipts) as ReceiptVoucherPreviewData;
            notifySkippedBills(result?.skipped_bills || []);
            setVoucherPreview({ isOpen: true, data: result, isLoading: false, error: null });
        } catch (err: unknown) {
            setVoucherPreview({
    ...
    const handlePushVoucher = useCallback(async (kingdeeJson: unknown) => {
        try {
            ...
            const result = await pushVoucherToKingdee(kingdeeJson as Record<string, unknown>, sourceBills);
            if (result?.success) {
            ...
            showToast('error', '推送失败', getErrorMessage(err, '推送失败'));
            throw err;
        }
    ...
    const columns = [
        {
            ...
            render: (_value: unknown, row: ReceiptBill) => (
                <input
                    type="checkbox"
        ...
        { key: 'payer_name', title: '付款人' },
        {
            ...
            title: '收入类型',
            width: 120,
            render: (_value: unknown, record: ReceiptBill) => getReceiptBillDealTypeLabel(record.deal_type, record.deal_type_label),
        },
        {
            key: 'income_amount',
            title: '入账金额',
        ...
        },
        {
            ...
            title: '关联数据',
            width: 220,
            className: 'receipt-bills-col-related',
            render: (_value: unknown, record: ReceiptBill) => {
                const badges = getReceiptDrilldownBadges(record);
                return (
                    <div className="receipt-related-cell">
                        {badges.length > 0 && (
                            <div className="receipt-related-badges">
                                {badges.map((badge) => {
                                    const style = DRILLDOWN_BADGE_STYLES[badge.tone];
                                    return (
                                        <span
                                            key={badge.key}
                                            className="receipt-related-badge"
                                            style={{
                                                background: style.bg,
                                                color: style.color,
                        ...
                        <span
                            className={`receipt-related-summary ${record.drilldown_enabled ? 'is-active' : 'is-muted'}`}
                        >
        ...
        { key: 'pay_channel_str', title: '收款渠道' },
        {
            key: 'deal_time',
            title: '收款时间',
            render: (v: unknown, record: ReceiptBill) => {
                const ts = Number(v || 0);
    ...
    const handleExportReceiptBills = async (mode: 'current' | 'all') => {
        try {
            let exportData = items;
            
            if (mode === 'all') {
                showToast('info', '正在导出', '正在加载全部数据...');
                const allItems: ReceiptBill[] = [];
                const batchSize = 100;
                let skip = 0;
                let hasMore = true;
            ...
            exportTableToCsv(exportColumns, exportData as unknown as Record<string, unknown>[], '收款单据');
    ...
    return (
        <div className="page-container receipt-bills-page">
            ...
            <div className="table-area-wrapper receipt-bills-table">
                <DataTable
                    columns={columns}
                    data={items}
                    loading={isLoading}
                    ...
                    toolbar={
                        <>
                            <button className="btn-primary btn-refresh-list" onClick={() => setIsSyncTriggerOpen(true)}>
                                <RefreshCw size={14} className={isSyncing && !['completed', 'failed', 'partially_completed'].includes(syncState.status) ? 'animate-spin' : ''} />
                                {isSyncing && !['completed', 'failed', 'partially_completed'].includes(syncState.status) ? '同步中...' : '同步收款单据'}
                            </button>
                            <button className="btn-outline" onClick={() => void refreshReceiptViews()}>
                                <RefreshCw size={14} /> 刷新列表
                            </button>

                        <div className="receipt-export-action" style={{ position: 'relative' }}>
                            <button
                            ...
                            {isExportMenuOpen && (
                                ...
                                >
                                    <button
                                        ...
                                        onClick={() => {
                                            setIsExportMenuOpen(false);
                                            handleExportReceiptBills('current');
                                        }}
                                    >
                                        导出当前页
                                    </button>
                ...
                <div className="pagination-footer">
                    <div className="pagination-info" style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
                        <span style={{ color: '#64748b' }}>
                            共 <strong style={{ color: '#334155' }}>{totalRecords}</strong> 条
                        </span>
                        <span style={{ color: '#cbd5e1' }}>|</span>
                        <span style={{ color: '#64748b' }}>
                            入账总额 <strong style={{ color: '#2563eb', fontWeight: 600 }}>¥{totalIncomeAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</strong>
                        </span>
                        {selectedReceiptKeys.size > 0 && (
                            <>
                                <span style={{ color: '#cbd5e1' }}>|</span>
                                <span style={{ color: '#64748b' }}>
                                    已选 <strong style={{ color: '#334155' }}>{selectedReceiptKeys.size}</strong> 条
                                    <span style={{ color: '#cbd5e1', margin: '0 0.25rem' }}>/</span>
                                    入账合计 <strong style={{ color: '#16a34a', fontWeight: 600 }}>¥{selectedTotalAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</strong>
                                </span>
                                <span style={{ color: '#cbd5e1' }}>|</span>
                                <span style={{ color: '#64748b' }}>
                                    已推送 <strong style={{ color: '#334155' }}>{selectedSuccessCount}</strong> 条
                                </span>
                            </>
                        )}
                    </div>
                    ...
                    <div className="pagination-controls" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                            <button className="page-btn" disabled={page === 1} onClick={() => setPage(1)}><ChevronsLeft size={14} /></button>
                            <button className="page-btn" disabled={page === 1} onClick={() => setPage(p => Math.max(1, p - 1))}><ChevronLeft size={14} /></button>
                            ...
                            <button className="page-btn" disabled={page === totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))}><ChevronRight size={14} /></button>
                            <button className="page-btn" disabled={page === totalPages} onClick={() => setPage(totalPages)}><ChevronsRight size={14} /></button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <SyncTriggerModal
                isOpen={isSyncTriggerOpen}
                onClose={() => {
                    if (!isSyncing || ['completed', 'failed', 'partially_completed'].includes(syncState.status)) {
                        setIsSyncTriggerOpen(false);
                    }
                }}
                onSync={handleSync}
                isSyncing={isSyncing}
