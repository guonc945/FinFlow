...
type SubjectCheckItem = {
    asstactitem_name?: string;
    asstactitem_number?: string;
};
type ConditionNode = {
    type?: string;
...
const parseJsonToRows = (jsonStr: string | null | undefined) => {
    ...
    try {
        ...
        return Object.entries(obj).map(([key, val]) => {
            const valObj = typeof val === 'object' && val !== null ? val as JsonObjectLike : {};
...
const rowsToJson = (rows: Array<{ key: string, prop: string, value: string }>) => {
    const obj: Record<string, JsonObjectLike> = {};
    rows.forEach(row => {
        if (row.key.trim()) {
...
const extractSaveErrorMessages = (err: unknown): string[] => {
    const detail = (err as ApiErrorLike)?.response?.data?.detail;

    if (detail && typeof detail === 'object') {
        const detailObject = detail as { errors?: unknown; message?: unknown };
        if (Array.isArray(detailObject.errors)) {
            const parsed = detailObject.errors
...
const normalizeSourceFields = (raw: unknown): SourceFieldOption[] => {
    ...
    raw.forEach((item: unknown) => {
        ...
        const itemObject = item as { value?: unknown; field?: unknown; label?: unknown; group?: unknown };
        const value = String(itemObject.value ?? itemObject.field ?? '').trim();
        if (!value) return;
        const label = getUnifiedSourceFieldLabel({
            label: String(itemObject.label ?? value).trim() || value,
            value,
        });
        const group = String(itemObject.group ?? '').trim() || '账单字段';
...
const normalizeTemplateFromApi = (t: Record<string, unknown>, categoryPathMap: Record<number, string>, voucherFieldModules: VoucherFieldModule[]): VoucherTemplate => {
    const sourceModule = String(t.source_module ?? '').trim();
    const sourceTypeRaw = String(t.source_type ?? '').trim();
    const categoryPath = String(t.category_path ?? '').trim();
    return {
        ...
        category_path: categoryPath || (Number.isFinite(Number(t.category_id)) ? categoryPathMap[Number(t.category_id)] : null),
        source_module: sourceModule || inferModuleIdFromSourceType(voucherFieldModules, sourceTypeRaw),
        source_type: (() => {
            const raw = sourceTypeRaw;
            if (raw) return raw;
            const mid = sourceModule || inferModuleIdFromSourceType(voucherFieldModules, sourceTypeRaw);
        ...
        trigger_condition: String(t.trigger_condition ?? ''),
        book_number_expr: String(t.book_number_expr ?? "{CURRENT_ACCOUNT_BOOK_NUMBER}"),
        vouchertype_number_expr: String(t.vouchertype_number_expr ?? "'0001'"),
        attachment_expr: String(t.attachment_expr ?? "0"),
        bizdate_expr: String(t.bizdate_expr ?? "{CURRENT_DATE}"),
        bookeddate_expr: String(t.bookeddate_expr ?? "{CURRENT_DATE}"),
        rules: Array.isArray(t.rules) ? t.rules as VoucherEntryRule[] : []
    };
};
...
const getInitialViewMode = (): 'card' | 'list' => {
    ...
    try {
        ...
        // Ignore local storage read failures and fall back to the default list view.
    }
    return 'list';
};
...
const VoucherTemplates = () => {
    ...
    const fetchTemplatesRef = useRef<() => Promise<void>>(async () => {});
    ...
    }, [closeConfirm, confirmState.loading, confirmState.onConfirm]);
    ...
    useEffect(() => {
        try {
            ...
            // Ignore local storage write failures; the UI can still work without persistence.
        }
    }, [viewMode]);
        ...
        openConfirm({
            ...
            onConfirm: async () => {
                try {
                    ...
                    setSelectedTemplateIds(new Set());
                    void fetchTemplatesRef.current();
                } catch (err) {
    ...
    }, [openConfirm, selectedTemplateIds, showToast]);

    const handleBatchSetActive = useCallback(async (active: boolean) => {
        if (selectedTemplateIds.size === 0) return;
            ...
            onConfirm: async () => {
                try {
                    ...
                    const payloads = resolvedTemplates.map(t => {
                        ...
                        const { category_path: _category_path, ...restTemplate } = t;
                        return {
                            ...restTemplate,
                            template_id: newId,
    ...
    }, [categoryPathMap, openConfirm, selectedTemplateIds, showToast, templates, voucherFieldModules]);
    ...
    const fetchTemplates = useCallback(async () => {
        ...
        try {
            ...
            const normalized = (res.data || []).map((t: Record<string, unknown>) =>
            ...
            const message = typeof detail === 'string'
                ? detail
                : ((typeof detail === 'object' && detail !== null && 'message' in detail ? detail.message : undefined) || (err as ApiErrorLike)?.message || '无法加载模板列表');
            setLoadError(String(message));
            setTemplates([]);
        } finally {
            setIsLoading(false);
        }
    }, [categoryPathMap, voucherFieldModules]);

    useEffect(() => {
        fetchTemplatesRef.current = fetchTemplates;
    }, [fetchTemplates]);

    const extractSourceFields = useCallback((modules: VoucherFieldModule[], sourceType: string): SourceFieldOption[] => {
    ...
    const fetchSourceFieldsByType = useCallback(async (sourceType: 'bills' | 'receipt_bills' | 'deposit_records') => {
    ...
    const fetchSourceFieldsFallback = useCallback(async () => {
    ...
    }, [applySourceMetadata, fetchSourceFieldsByType]);

    const fetchVoucherFieldModules = useCallback(async () => {
        try {
    ...
    }, [applySourceMetadata, fetchSourceFieldsFallback]);
    ...
    }, [fetchSubjects, fetchTemplateCategories, fetchTemplates, fetchVoucherFieldModules]);

    useEffect(() => {
        if (location.pathname.includes('/vouchers/templates')) {
            fetchTemplateCategories();
        }
    }, [fetchTemplateCategories, location.pathname]);
    ...
    const normalizeTemplateFieldBindings = (template: VoucherTemplate) => {
        ...
        (module?.sources || []).forEach(s => {
            ...
            const fieldKeys = new Set(
                (s?.fields || [])
                    .map((f: VoucherSourceFieldOption) => String(f?.value || '').trim())
                    .filter(Boolean)
            );
        ...
        const normalizeConditionTree = (trigger: string | null | undefined) => {
            ...
            try {
                ...
                const walk = (node: ConditionNode, currentSourceType: string) => {
                    if (!node || typeof node !== 'object') return;
                    const t = node.type || 'group';
                    if (t === 'group') {
                        const children = Array.isArray(node.children) ? node.children : [];
                        children.forEach((child: ConditionNode) => walk(child, currentSourceType));
                        return;
                    }
                    if (t === 'relation') {
                        const relationSourceType = String(node.target_source || '').trim() || currentSourceType;
                        if (Array.isArray(node.children)) {
                            node.children.forEach((child: ConditionNode) => walk(child, relationSourceType));
    ...
    const handleSave = async () => {
        ...
        const { category_path: _category_path, ...restTemplate } = workingTemplate;
            ...
            if (subject.check_items) {
                try {
                    ...
                    if (Array.isArray(checkItems) && checkItems.length > 0) {
                        ...
                        const newAuxItemsObj: Record<string, { number: string }> = {};
                        checkItems.forEach((item: SubjectCheckItem) => {
                            ...
                            <div className="field-grid-three">
                                    ...
                                    <div className="field-item">
                                        ...
                                        <Select
                                            ...
                                            onChange={(v) => {
                                                const nextModuleId = String(v || '').trim();
                                            ...
                                            options={voucherFieldModules.map(m => ({ value: String(m.id), label: m.label }))}
                                        />
                                    </div>
                                    <div className="field-item">
                                        ...
                                        {currentTemplate.rules.map((rule, idx) => (
                                            ...
                                            >
                                                ...
                                                <td>
                                                    <Select
                                                        value={rule.dr_cr}
                                                        onChange={(v) => updateRule(idx, { dr_cr: v as 'D' | 'C' })}
                                                    ...
                                                    if (subject?.check_items) {
                                                        try {
                                                            ...
                                                            if (Array.isArray(checkItems)) {
                                                                return checkItems.map((item: SubjectCheckItem) =>
