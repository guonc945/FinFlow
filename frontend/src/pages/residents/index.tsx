...
type ResidentQueryParams = {
    skip: number;
    limit: number;
    search?: string;
};

type ResidentHouseItem = {
    name?: string;
};
...
const Residents = () => {
    ...
    const fetchResidentsData = useCallback(async () => {
        setIsLoading(true);
        try {
            const params: ResidentQueryParams = {
    ...
    const getHousesText = (houses?: string | unknown) => {
        if (!houses || typeof houses !== 'string') return '-';
        try {
            const housesList = JSON.parse(houses);
            if (!Array.isArray(housesList)) return '-';
            const names = housesList.map((h: ResidentHouseItem) => h?.name).filter(Boolean);
    ...
    const columns = [
        ...
        },
        {
            ...
            title: '姓名',
            width: 130,
            render: (val: unknown, row: Resident): ReactNode => (
        ...
        },
        {
            ...
            title: '财务系统客户映射',
            width: 220,
            render: (_value: unknown, row: Resident): ReactNode => (
                <div
    ...
    return (
        <div className="page-container fade-in residents-page">
