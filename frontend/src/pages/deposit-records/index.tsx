...
import { getDepositRecordSyncStatus, getDepositRecords, getProjects, syncDepositRecords } from '../../services/api';
            ...
            <div className="sync-modal">
                <div className="sync-header">
                    <div className="sync-title">
                        ...
                        <div>
                            <h3>{isCompleted ? (isFailed ? '同步失败' : '同步完成') : '正在同步押金记录'}</h3>
                            <p className="text-secondary text-sm">
                                {isCompleted ? '押金记录同步任务已结束' : '正在后台拉取接口中心的押金变动记录'}
                            </p>
                        </div>
                    </div>
                </div>
...
const DepositRecords = () => {
    ...
    const startPolling = useCallback((taskId: string) => {
        ...
        pollingTimer.current = window.setInterval(async () => {
            try {
                const status = await getDepositRecordSyncStatus(taskId);
                setSyncState((prev) => ({
                    ...prev,
    ...
    const handleSync = useCallback(async () => {
        try {
            ...
            const result = await syncDepositRecords(ids);
            setSyncState({
    ...
    const columns = [
        ...
        },
        {
            ...
            title: '变动类型',
            render: (_: any, row: DepositRecord) => {
            ...
            <div className={`bills-filter-section ${isFilterCollapsed ? 'collapsed' : ''}`}>
                <div className="filter-header-row">
                    <div className="flex items-center gap-2">
                        <h3 className="font-bold text-slate-800">押金管理</h3>
