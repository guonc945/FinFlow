...
const Projects = () => {
    ...
    const columns = [
        ...
        { key: 'proj_name' as keyof Project, title: '园区名称' },
        {
            ...
            title: '财务系统管理项目映射',
            width: 250,
            render: (val: unknown) => val && typeof val === 'object' ? (
                <div className="flex flex-col">
                    <span className="text-sm font-medium">{String((val as { full_path?: unknown; name?: unknown }).full_path || (val as { name?: unknown }).name || '')}</span>
                    <span className="text-xs text-slate-400 font-mono">内码: {String((val as { number?: unknown }).number || '')}</span>
                </div>
        ...
        },
        {
            ...
            render: (val: unknown) => val && typeof val === 'object' ? (
                <div className="flex flex-col">
                    <span className="text-sm font-medium">{String((val as { name?: unknown }).name || '')}</span>
                    <span className="text-xs text-slate-400 font-mono">内码: {String((val as { number?: unknown }).number || '')}</span>
                </div>
            ) : <span className="text-slate-300">未设置</span>
        },
        {
            ...
            render: (val: unknown) => val && typeof val === 'object' ? (
                <div className="flex flex-col">
                    <span className="text-sm font-medium">{String((val as { name?: unknown }).name || '')}</span>
                    <span className="text-xs text-slate-400 font-mono">{String((val as { bankaccountnumber?: unknown }).bankaccountnumber || '')}</span>
                </div>
            ) : <span className="text-slate-300">未设置</span>
        },
        {
        ...
        },
        {
            ...
            title: '创建时间',
            width: 120,
            render: (val: unknown) => val ? new Date(String(val)).toLocaleDateString() : '-'
        },
        {
    ...
    return (
        <div className="page-container fade-in projects-page">
