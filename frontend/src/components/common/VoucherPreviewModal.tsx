...
type VoucherDimensionMap = Record<string, unknown>;

type VoucherEntry = Record<string, unknown> & {
  summary?: string;
  edescription?: string;
  account_code?: string;
  account_name?: string;
  ...
  currency?: string;
  localrate?: string | number;
  localrate_exact?: string;
  debit?: string | number;
  credit?: string | number;
  debit_exact?: string | number;
  credit_exact?: string | number;
  debit_formula_exact?: string | number;
  credit_formula_exact?: string | number;
  line_no?: number;
  assgrp?: VoucherDimensionMap;
  maincfassgrp?: VoucherDimensionMap;
  debit_decimal?: DecimalValue;
  credit_decimal?: DecimalValue;
  source_line_no_min?: number;
  source_index_min?: number;
  business_sort_bucket?: number;
};

type KingdeeEntry = Record<string, unknown> & {
  seq?: number;
  edescription?: string;
  account_number?: string;
  currency_number?: string;
  localrate?: string | number;
  debitori?: string | number;
  creditori?: string | number;
  debitlocal?: string | number;
  creditlocal?: string | number;
  assgrp?: VoucherDimensionMap;
  maincfassgrp?: VoucherDimensionMap;
};

type KingdeeVoucherData = Record<string, unknown> & {
  bookeddate?: string;
  period_number?: string;
  entries?: KingdeeEntry[];
};

type KingdeeJson = Record<string, unknown> & {
  data?: KingdeeVoucherData[];
};

type AccountingView = Record<string, unknown> & {
  entries?: VoucherEntry[];
};

type PushStatusSummaryLike = {
  total?: number;
  not_pushed?: number;
  pushing?: number;
  success?: number;
...
type VoucherPreviewData = Record<string, unknown> & {
  accounting_view?: AccountingView;
  kingdee_json?: KingdeeJson;
  source_bills?: unknown[];
  source_bill_push_summary?: PushStatusSummaryLike;
  skipped_bills?: Array<Record<string, unknown> & {
    bill_id?: string | number | null;
    community_id?: string | number | null;
    reason?: string | null;
  }>;
  push_blocked?: boolean;
  partial_matched?: boolean;
  matched?: boolean;
  message?: string;
  template_name?: string;
  template_id?: string | number;
  templates_checked?: string | number;
  push_block_reason?: string;
};

type PushVoucherResult = {
  success?: boolean;
  message?: string;
};
...
interface VoucherPreviewModalProps {
  ...
  isLoading: boolean;
  error: string | null;
  onPushVoucher?: (kingdeeJson: unknown) => Promise<PushVoucherResult>;
}
const VoucherPreviewModal = ({
  ...
  const showLegacyDatePreview = false;
  const showLegacyBalanceSummary = false;
  ...
  const getAssgrpDisplayValue = (conf: unknown): string => {
    ...
    const fallback = Object.values(config).find(
  ...
  const parseDecimal = (value: unknown): DecimalValue => {
  ...
  const getEntryDebitValue = (entry: VoucherEntry) => entry?.debit_exact ?? entry?.debit;
  const getEntryCreditValue = (entry: VoucherEntry) => entry?.credit_exact ?? entry?.credit;
  const getEntryDebitMergeValue = (entry: VoucherEntry) =>
    entry?.debit_formula_exact ?? entry?.debit_exact ?? entry?.debit;
  const getEntryCreditMergeValue = (entry: VoucherEntry) =>
    entry?.credit_formula_exact ?? entry?.credit_exact ?? entry?.credit;
  const getEntryLocalRateValue = (entry: VoucherEntry) => entry?.localrate_exact ?? entry?.localrate ?? "1";
  const getMoneyString = (value: unknown): string => decimalToFixedString(parseDecimal(value), 2);
  const getAmountDisplay = (value: unknown): string => getMoneyString(value);
  const hasPositiveAmount = (value: unknown): boolean => parseDecimal(value).int > 0n;
  const buildKingdeeJsonFromEntries = (entries: VoucherEntry[], nextBookedDate: string): KingdeeJson | undefined => {
  ...
  const normalizeAssgrp = (assgrp: VoucherDimensionMap | undefined): Array<[string, string]> => {
  ...
  const getAccountNameDisplay = (entry: VoucherEntry): string => {
  ...
  const normalizeEntryText = (...parts: unknown[]): string =>
    parts
      ...
      .trim()
      .toLowerCase();
  const isProfitLossEntry = (entry: VoucherEntry): boolean => {
  ...
  const isCarryForwardEntry = (entry: VoucherEntry): boolean => {
  ...
  const isTaxEntry = (entry: VoucherEntry): boolean => {
    const accountCode = String(entry?.account_code || "").trim();
    const summary = normalizeEntryText(entry?.summary, entry?.edescription);
    return (
  ...
  const getEntryBusinessSortBucket = (entry: VoucherEntry): number =>
  ...
  const mergeEntries = (entries: VoucherEntry[]) => {
    const merged: VoucherEntry[] = [];
  ...
  const displayedEntries = !acctView?.entries
    ? []
    : mergeEnabled
      ? mergeEntries(acctView.entries)
      : acctView.entries;
  const displayedDebitTotal = displayedEntries.reduce(
    (sum: DecimalValue, entry: VoucherEntry) =>
      addDecimal(sum, parseDecimal(getEntryDebitValue(entry))),
    ZERO_DECIMAL,
  );
  const displayedCreditTotal = displayedEntries.reduce(
    (sum: DecimalValue, entry: VoucherEntry) =>
      addDecimal(sum, parseDecimal(getEntryCreditValue(entry))),
    ZERO_DECIMAL,
  );
  const accountingViewBalanced =
    compareDecimal(displayedDebitTotal, displayedCreditTotal) === 0;
  const accountingViewDiffDisplay = decimalToFixedString({
  ...
  const effectiveKingdeeJson = !acctView?.entries || !kdJson
    ? kdJson
    : buildKingdeeJsonFromEntries(displayedEntries, bookedDate);
  const voucherAmountValidation = (() => {
    if (!effectiveKingdeeJson?.data?.[0]?.entries) {
      return { ok: true, message: "" };
    }

    const jsonEntries = effectiveKingdeeJson.data[0].entries || [];
    const displayedDebit = displayedDebitTotal;
    const displayedCredit = displayedCreditTotal;
    const jsonDebit = jsonEntries.reduce(
      (sum: DecimalValue, entry: KingdeeEntry) =>
        ...
        typeof candidate.response?.data?.detail === "string"
          ...
          : typeof candidate.response?.data?.message === "string"
            ...
            : typeof candidate.message === "string"
              ? candidate.message
              : "凭证推送失败";
      setPushFeedback({ type: "error", text: message });
    } finally {
      setIsPushing(false);
    }
  };
                ...
                <>
                  ...
                  {skippedBills.length > 0 && (
                    ...
                    >
                      ...
                      >
                        {skippedBills.slice(0, 10).map((item, index: number) => (
                          <div
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <tbody>
                        {displayedEntries.map((entry, idx: number) => (
                          <tr
                            key={idx}
                            style={{
...
// 简单 JSON 语法高亮组件
const JsonSyntaxHighlight = ({ json }: { json: unknown }) => {
