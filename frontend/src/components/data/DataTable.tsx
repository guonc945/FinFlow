...
type ColumnRender<T> = {
    // Preserve existing call-site flexibility for legacy tables while still typing the record shape.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    bivarianceHack: (value: any, record: T, index: number) => ReactNode;
}['bivarianceHack'];
...
const isSerialColumnDefinition = <T extends object>(column: Column<T>) => {
...
const resolveStickyWidth = <T extends object>(column: ManagedColumn<T>) => {
...
const DataTable = <T extends object>({
    ...
    const activeStoredSettings = settingsEnabled ? storedSettings : DEFAULT_COLUMN_SETTINGS;
    ...
    const normalizedColumns = useMemo<Column<T>[]>(() => {
        ...
        const serialColumn: Column<T> = {
            ...
            hideable: false,
            reorderable: false,
            render: (_value: unknown, _record: T, index: number) => serialStart + index,
        };
    ...
    const handleToggleColumn = (columnKey: string) => {
        const hidden = new Set(activeStoredSettings.hidden);
        if (hidden.has(columnKey)) {
            hidden.delete(columnKey);
        } else {
            hidden.add(columnKey);
        }
        persistSettings({ ...activeStoredSettings, hidden: Array.from(hidden) });
    };
    ...
    const handleMoveColumn = (columnKey: string, direction: 'up' | 'down') => {
        ...
        const currentOrder = activeStoredSettings.order.length
            ? activeStoredSettings.order.filter((key) => reorderableKeys.includes(key))
            : reorderableKeys;
        const completeOrder = [
            ...currentOrder,
        ...
        persistSettings({ ...activeStoredSettings, order: nextOrder });
    };
    ...
    const effectiveColumns = useMemo(() => {
        ...
        const reorderableKeys = middleColumns
            .filter((column) => column.reorderableResolved)
            .map((column) => column.normalizedKey);
        const savedOrder = activeStoredSettings.order.filter((key) => reorderableKeys.includes(key));
        const finalOrder = [
            ...savedOrder,
            ...reorderableKeys.filter((key) => !savedOrder.includes(key)),
        ];
        const orderMap = new Map(finalOrder.map((key, index) => [key, index]));
        const hiddenKeys = new Set(activeStoredSettings.hidden);
    ...
    }, [activeStoredSettings.hidden, activeStoredSettings.order, managedColumns]);
    ...
    }, [activeStoredSettings.order, configurableColumns]);
    ...
    return (
        <div className="data-table-wrapper glass">
            {(title || (settingsEnabled && configurableColumns.length > 0) || toolbar) && (
                <div className="table-toolbar">
                    {title ? <div className="table-header"><h3>{title}</h3></div> : <div />}
                    {toolbar && <div className="table-toolbar-extra">{toolbar}</div>}
                                ...
                                <div className="table-settings-panel">
                                    ...
                                    <div className="table-settings-list">
                                        {orderedConfigurableColumns.map((column, index) => {
                                            const isHidden = activeStoredSettings.hidden.includes(column.normalizedKey);
