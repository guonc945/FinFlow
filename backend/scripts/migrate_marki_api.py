    ...
    try:
        ...
        if not service:
            ...
            service = ExternalService(
                ...
                app_secret=encrypt_value(os.getenv("MARKI_PASSWORD", "")) if os.getenv("MARKI_PASSWORD", "") else None,
                auth_url="https://sttc-os-lgn.markiapp.com/lgn/login/authorize.do",
                base_url="https://charge-api.markiapp.com",
                auth_type="basic",
                is_active=True,
                extra_info=encrypt_value(os.getenv("MARKI_COOKIE", "")) if os.getenv("MARKI_COOKIE", "") else None
            )
            db.add(service)
            db.commit()
            db.refresh(service)
            ...
            if not service.app_secret and os.getenv("MARKI_PASSWORD"):
                service.app_secret = encrypt_value(os.getenv("MARKI_PASSWORD"))
            if not service.extra_info and os.getenv("MARKI_COOKIE"):
                 service.extra_info = encrypt_value(os.getenv("MARKI_COOKIE"))
            db.commit()
