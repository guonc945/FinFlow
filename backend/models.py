...
class Bill(Base):
    ...
    source_deleted = Column(Boolean, default=False)
...
class ReportingReportCategory(Base):
    __tablename__ = "reporting_report_categories"

    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(100), nullable=False)
    parent_id = Column(Integer, ForeignKey("reporting_report_categories.id"), nullable=True)
    sort_order = Column(Integer, default=0)
    status = Column(Integer, default=1)  # 1=active, 0=inactive
    description = Column(String(500))
    created_at = Column(DateTime, server_default=func.now())
    ...
    parent = relationship("ReportingReportCategory", remote_side=[id], backref="children")
...
class ReportingReport(Base):
    ...
    category_id = Column(Integer, ForeignKey("reporting_report_categories.id"), nullable=True)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())

    dataset = relationship("ReportingDataset", back_populates="reports")
    category = relationship("ReportingReportCategory", foreign_keys=[category_id])
...
class SyncModuleStatus(Base):
    __tablename__ = "sync_module_status"

    id = Column(Integer, primary_key=True, index=True)
    module_key = Column(String(100), unique=True, nullable=False, index=True)
    source_system = Column(String(50), nullable=False, default="kingdee")
    status = Column(String(20), nullable=False, default="idle")
    message = Column(String(500))
    started_at = Column(DateTime)
    finished_at = Column(DateTime)
    last_modifytime_sync_at = Column(DateTime)
...
class DataDictionary(Base):
    ...
    key = Column(String(100), unique=True, nullable=False, index=True)
    name = Column(String(120), nullable=False, index=True)
    dict_type = Column(String(20), nullable=False, default="enum")
    source_type = Column(String(30), nullable=False, default="static")
...
class BusinessDictionary(Base):
    ...
    key = Column(String(100), unique=True, nullable=False, index=True)
    name = Column(String(120), nullable=False, index=True)
    dict_type = Column(String(20), nullable=False, default="enum")  # enum / hierarchy
    category = Column(String(50), default="common")
    description = Column(String(500))
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, server_default=func.now())
    ...
    items = relationship(
        "BusinessDictionaryItem",
        back_populates="dictionary",
        cascade="all, delete-orphan",
        order_by="BusinessDictionaryItem.sort_order.asc(), BusinessDictionaryItem.id.asc()",
    )


class BusinessDictionaryItem(Base):
    __tablename__ = "business_dictionary_items"
    __table_args__ = (
        Index("ux_business_dictionary_items_dict_code", "dictionary_id", "code", unique=True),
        Index("ix_business_dictionary_items_dict_parent", "dictionary_id", "parent_id"),
    )

    id = Column(Integer, primary_key=True, index=True)
    dictionary_id = Column(Integer, ForeignKey("business_dictionaries.id", ondelete="CASCADE"), nullable=False, index=True)
    code = Column(String(100), nullable=False)
    label = Column(String(120), nullable=False)
    value = Column(String(200))
    parent_id = Column(Integer, ForeignKey("business_dictionary_items.id"), nullable=True)
    sort_order = Column(Integer, default=0)
    status = Column(Integer, default=1)  # 1=active, 0=inactive
    description = Column(String(500))
    ...
    dictionary = relationship("BusinessDictionary", back_populates="items")
    parent = relationship("BusinessDictionaryItem", remote_side=[id], backref="children")
