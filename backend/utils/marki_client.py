...
class MarkiClient:
    ...
    def _load_config(self):
        ...
        try:
            ...
            if service:
                ...
                self.password = decrypt_value(service.app_secret) if service.app_secret else None
                self.cookie = decrypt_value(service.extra_info) if service.extra_info else None
    ...
    def _save_cookie(self, new_cookie):
        ...
        try:
            service = db.query(ExternalService).filter_by(service_name="marki").first()
            if service:
                service.extra_info = encrypt_value(new_cookie) if new_cookie else None
                db.commit()
    ...
    def login(self):
        ...
        try:
            ...
            if str(rescode) == "0":
                ...
                try:
                    test_url = get_api_url_by_id(MARKI_PROJECT_API_ID)
                except Exception:
