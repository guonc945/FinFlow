...
class KingdeeAuthService:
    def __init__(self, db: Session = None):
        ...
        if self.service_record:
            ...
            self.app_secret = decrypt_value(self.service_record.app_secret) if self.service_record.app_secret else env_app_secret
    ...
    def _init_service_record(self):
        try:
            new_service = ExternalService(
                ...
                app_id=self.app_id,
                app_secret=encrypt_value(self.app_secret) if self.app_secret else None,
    ...
    def get_token(self) -> str:
        ...
        if token_record.access_token and token_record.expires_at:
            if token_record.expires_at > now + timedelta(minutes=5):
                return decrypt_value(token_record.access_token)
    ...
    def _save_token(self, access_token, refresh_token, expires_at, extra_data):
        try:
            ...
            token_record.access_token = encrypt_value(access_token) if access_token else None
            token_record.refresh_token = encrypt_value(refresh_token) if refresh_token else None
            token_record.expires_at = expires_at
            token_record.token_type = extra_data.get("token_type", "Bearer")
            token_record.scope = extra_data.get("scope", "")
            token_record.extra_info = encrypt_value(json.dumps(extra_data))
