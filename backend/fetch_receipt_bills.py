...
from models import ExternalApi, GlobalVariable, ProjectList, ReceiptBill, ReceiptBillUser
...
RECEIPT_BILL_API_ID = require_api_id("MARKI_RECEIPT_BILL_API_ID")
...
def _load_last_full_sync_time(db, community_id: int):
    key = f"RECEIPT_BILL_LAST_FULL_SYNC_{int(community_id)}"
...
def _save_last_full_sync_time(db, community_id: int):
    ...
    if row:
        row.value = now_value
        row.description = "Last successful full sync time for receipt bills"
        ...
        db.add(
            GlobalVariable(
                key=key,
                value=now_value,
                description="Last successful full sync time for receipt bills",
...
def _should_use_full_sync(db, community_id: int):
    if _parse_bool_env("MARKI_RECEIPT_BILL_FORCE_FULL_SYNC", False):
        return True, "force flag"

    interval_hours = _parse_positive_int_env("MARKI_RECEIPT_BILL_FULL_SYNC_INTERVAL_HOURS", 24 * 7)
    if interval_hours <= 0:
...
def sync_receipt_bills_for_community(community_id: int, task_id: str = None):
    ...
    try:
        ...
        overlap_seconds = _parse_positive_int_env("MARKI_RECEIPT_BILL_INCREMENTAL_OVERLAP_SECONDS", 600)
        ...
        sync_mode_msg = (
            f"Community {community_id}: using FULL receipt-bill sync (reason={full_sync_reason})."
            if use_full_sync or max_deal_time_local <= 0
            else (
                f"Community {community_id}: using INCREMENTAL receipt-bill sync "
                f"(minDealTime=max- {overlap_seconds}s)."
            )
        )
        print(sync_mode_msg)
        if task_id:
        ...
        while True:
            ...
            if not (use_full_sync or max_deal_time_local <= 0) and max_deal_time_local > 0:
                request_data["minDealTime"] = max(0, max_deal_time_local - overlap_seconds)
            else:
                request_data.pop("minDealTime", None)
        ...
        if (use_full_sync or max_deal_time_local <= 0) and not request_failed:
            try:
                _save_last_full_sync_time(db, int(community_id))
                db.commit()
            except Exception as exc:
                db.rollback()
                marker_msg = f"Community {community_id}: failed to persist receipt full-sync marker: {exc}"
                print(marker_msg)
                if task_id:
