...
def _run_finance_sync_task(
    ...
    try:
        ...
        if not definition:
            return {"status": "success", "message": str(message), "logs": [{"type": "info", "message": str(message)}]}

        status_db = database.SessionLocal()
        try:
            status_row = (
                status_db.query(models.SyncModuleStatus)
                .filter(
        ...
        sync_status = str(status_row.status or "").strip().lower()
        sync_message = str(status_row.message or message)
        if sync_status == "success":
            return {"status": "success", "message": sync_message, "logs": [{"type": "info", "message": sync_message}]}
        if sync_status == "failed":
            return {"status": "failed", "message": sync_message, "logs": [{"type": "error", "message": sync_message}]}
        if sync_status == "running":
            return {
                "status": "failed",
                "message": f"{definition['label']}同步任务执行后仍处于运行中，请检查任务状态。",
                "logs": [{"type": "error", "message": f"{definition['label']}同步状态异常，仍为 running。"}],
            }
        return {"status": "success", "message": sync_message, "logs": [{"type": "info", "message": sync_message}]}
