...
def _sanitize_service_secret(service: models.ExternalService) -> models.ExternalService:
    service.app_secret = None
    service.has_app_secret = bool(getattr(service, "app_secret", None) or getattr(service, "has_app_secret", False))
    return service
...
def get_external_services(
    ...
    for s in services:
        s.has_app_secret = bool(s.app_secret)
        if not can_manage_credentials:
            s.app_secret = None
        _sanitize_service_secret(s)
    return services
...
def create_external_service(
    ...
    new_service.has_app_secret = bool(new_service.app_secret)
    return _sanitize_service_secret(new_service)
...
def update_external_service(
    ...
    db_service.has_app_secret = bool(db_service.app_secret)
    return _sanitize_service_secret(db_service)
...
def update_marki_config(
    ...
    if req.app_secret and req.app_secret != "********":
        service.app_secret = encrypt_value(req.app_secret)
