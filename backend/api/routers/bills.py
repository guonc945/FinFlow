...
def query_voucher_by_id(
    ...
    """Query Kingdee voucher by internal id using configured ExternalApi id."""
    import requests
    ...
    if api_id is None:
        try:
            api_id = require_api_id("KINGDEE_VOUCHER_QUERY_API_ID")
        except ValueError:
            raise HTTPException(
                status_code=500,
                detail="Invalid KINGDEE_VOUCHER_QUERY_API_ID. Expected integer external_apis.id.",
            )

    api_record = db.query(models.ExternalApi).filter(
        models.ExternalApi.id == api_id,
        models.ExternalApi.is_active == True,
    ).first()
    if not api_record:
        raise HTTPException(status_code=404, detail=f"ExternalApi not found or inactive: id={api_id}")

    service = db.query(models.ExternalService).filter(models.ExternalService.id == api_record.service_id).first()
    if not service:
        raise HTTPException(status_code=404, detail=f"External service not found for api_id={api_id}")
...
def get_bills(
    ...
    if not include_deleted:
        query = query.filter(
            or_(models.Bill.source_deleted == False, models.Bill.source_deleted.is_(None))
        )
    ...
    return {
        ...
        "items": [{
            ...
            "last_seen_at": bill.last_seen_at,
            "source_deleted": bool(bill.source_deleted),
            "source_deleted_at": bill.source_deleted_at,
...
def export_bills(
    ...
    for bill, proj_name, customer_name_value in results:
        ...
        writer.writerow([
            ...
            _format_dt(bill.last_seen_at),
            "是" if bill.source_deleted else "否",
            _format_dt(bill.source_deleted_at),
