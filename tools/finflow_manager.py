...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--minimized", action="store_true", help=argparse.SUPPRESS)
...
def run_cli(args: argparse.Namespace) -> int:
    if args.host:
        append_text_log(SERVICE_HOST_LOG, "===== Unified Host Run Entry =====")
        append_text_log(SERVICE_HOST_LOG, build_runtime_snapshot_text())
        try:
            host = FinFlowRuntimeHost()
            return host.run()
    ...
    if not any(mode_flags):
        try:
            app = FinFlowManagerApp(start_minimized=args.minimized)
            app.run()
            return 0
    ...
    if args.start:
        auto_start = bool(read_state_file(STATE_PATH).get("launch_manager_on_startup", False))
        install_ok, install_message = install_windows_service(auto_start=auto_start)
        if not install_ok:
            print(install_message)
            return 1
        ok, message = start_windows_service()
        print(message)
        return 0 if ok else 1

    if args.stop:
        ok, message = stop_windows_service()
        print(message)
        return 0 if ok else 1

    if args.restart:
        ok, message = restart_windows_service()
        print(message)
        return 0 if ok else 1
...
def discover_root_dir() -> Path:
    if getattr(sys, "frozen", False):
        # 打包后的 EXE：从 EXE 所在目录查找项目目录
        # 不要使用 sys._MEIPASS，因为它是临时目录，每次运行都不同
...
STARTUP_TASK_NAME = "FinFlowManager"
def _resolve_runtime_dir() -> Path:
    """解析运行时目录，确保打包后的EXE和Python脚本使用相同的路径"""
    # 首先尝试使用项目目录下的运行时目录
    project_runtime = ROOT_DIR / "deploy" / "windows" / "runtime"
    
    # 如果在打包环境中，检查是否需要使用备选路径
    if getattr(sys, "frozen", False):
        # 如果项目目录下没有backend目录，说明EXE不在项目目录中
        if not (ROOT_DIR / "backend").exists():
            # 尝试从EXE所在目录向上查找项目目录
            exe_dir = Path(sys.executable).resolve().parent
            for parent in [exe_dir] + list(exe_dir.parents):
                if (parent / "backend" / "main.py").exists() and (parent / "frontend").exists():
                    return parent / "deploy" / "windows" / "runtime"
            
            # 如果找不到项目目录，使用LOCALAPPDATA
            local_appdata = os.environ.get("LOCALAPPDATA")
            if local_appdata:
                return Path(local_appdata) / "FinFlow" / "runtime"
...
BACKEND_START_REQUEST_PATH = RUNTIME_DIR / "backend.start"
FRONTEND_START_REQUEST_PATH = RUNTIME_DIR / "frontend.start"
FRONTEND_STOP_REQUEST_PATH = RUNTIME_DIR / "frontend.stop"
FRONTEND_RESTART_REQUEST_PATH = RUNTIME_DIR / "frontend.restart"
SERVICE_HOST_LOG = LOG_DIR / "manager.service.log"
ACTIONS_AUDIT_LOG = LOG_DIR / "manager.audit.jsonl"
...
ENV_SECTIONS: List[Tuple[str, List[Tuple[str, str, bool]]]] = [
    (
        "应用配置",
        [
            ("APP_HOST", "后端监听地址", False),
            ("APP_PORT", "后端监听端口", False),
            ("ALLOWED_ORIGINS", "后端允许来源", False),
            ("ALLOW_LAN_ORIGINS", "后端允许局域网来源", False),
        ],
    ),
    (
        "前端配置",
        [
            ("FRONTEND_HOST", "前端监听地址", False),
            ("FRONTEND_PORT", "前端监听端口", False),
            ("FRONTEND_ALLOWED_ORIGINS", "前端允许来源", False),
        ],
    ),
    (
        "数据库配置",
...
def build_backend_env_payload(runtime_config: Dict[str, str], extra_values: Dict[str, str] | None = None) -> Dict[str, str]:
    payload = read_env_file(ENV_EXAMPLE_PATH)
    payload.update(DEFAULT_ENV)
    payload.update(read_env_file(ENV_PATH))
    payload.update(extra_values or {})
    payload.update(runtime_config)
    payload["APP_RELOAD"] = "false"
    return payload


def validate_backend_env_payload(payload: Dict[str, str]) -> List[str]:
    issues: List[str] = []

    app_host = (payload.get("APP_HOST") or "").strip()
    app_port = (payload.get("APP_PORT") or "").strip()
    frontend_host = (payload.get("FRONTEND_HOST") or "").strip()
    frontend_port = (payload.get("FRONTEND_PORT") or "").strip()
    token_minutes = (payload.get("ACCESS_TOKEN_EXPIRE_MINUTES") or "").strip()
    secret_key = (payload.get("SECRET_KEY") or "").strip()
    encryption_key = (payload.get("ENCRYPTION_KEY") or "").strip()
    encryption_key_file = (payload.get("ENCRYPTION_KEY_FILE") or "").strip()
    database_url = (payload.get("DATABASE_URL") or "").strip()
    db_dialect = (payload.get("DB_DIALECT") or "").strip()

    if not app_host:
        issues.append("APP_HOST 未配置")
    if not app_port:
        issues.append("APP_PORT 未配置")
    elif not app_port.isdigit():
        issues.append("APP_PORT 必须是数字")
    
    # 前端配置验证
    if frontend_port and not frontend_port.isdigit():
        issues.append("FRONTEND_PORT 必须是数字")

    if token_minutes and not token_minutes.isdigit():
        issues.append("ACCESS_TOKEN_EXPIRE_MINUTES 必须是数字")

    if not secret_key:
        issues.append("SECRET_KEY 未配置")

    if not encryption_key and not encryption_key_file:
        issues.append("ENCRYPTION_KEY 与 ENCRYPTION_KEY_FILE 至少需要配置一个")

    if not database_url:
        if not db_dialect:
            issues.append("DB_DIALECT 未配置")
        for key in ("DB_HOST", "DB_PORT", "DB_NAME", "DB_USER"):
            if not (payload.get(key) or "").strip():
                issues.append(f"{key} 未配置")
        db_port = (payload.get("DB_PORT") or "").strip()
        if db_port and not db_port.isdigit():
            issues.append("DB_PORT 必须是数字")

    return issues


def save_backend_env_config(runtime_config: Dict[str, str], extra_values: Dict[str, str] | None = None) -> Tuple[Dict[str, str], bool]:
    payload = build_backend_env_payload(runtime_config, extra_values)
    issues = validate_backend_env_payload(payload)
    if issues:
        raise ValueError("；".join(issues))
    existed = ENV_PATH.exists()
    write_env_file(ENV_PATH, payload, extra_values or {})
...
def build_startup_sync_label(result: str, detail: str) -> str:
    result_text = (result or "").strip() or "unknown"
    detail_text = (detail or "").strip()
    if detail_text:
        return f"{result_text}: {detail_text}"
    return result_text
...
DEFAULT_RUNTIME_STATE = {
    ...
    "frontend_pid": 0,
    "host_started_at": 0.0,
    "host_last_heartbeat_at": 0.0,
    "host_last_transition": "",
    "host_stop_reason": "",
    "user_stopped": False,
...
def append_audit_log(action: str, status: str, detail: str = "", **extra: Any) -> None:
    try:
        ensure_log_dir()
        entry: Dict[str, Any] = {
        ...
        for key, value in extra.items():
            if value is None:
                continue
            entry[str(key)] = value
        with open(ACTIONS_AUDIT_LOG, "a", encoding="utf-8", errors="ignore") as handle:
            handle.write(json.dumps(entry, ensure_ascii=False) + "\n")
    except Exception:
        pass


def format_runtime_timestamp(timestamp: Any) -> str:
    try:
...
def get_last_audit_summary() -> str:
    if not ACTIONS_AUDIT_LOG.exists():
        return "-"
    try:
        with open(ACTIONS_AUDIT_LOG, "r", encoding="utf-8", errors="ignore") as handle:
    ...
    for raw_line in reversed(lines):
        ...
        summary = " ".join(part for part in (timestamp, action, status) if part)
        if detail:
...
def build_takeover_check_summary(app: Any) -> str:
    config = app.get_effective_config()
    lines: List[str] = ["FinFlow 首装 / 环境接管检查", ""]

    layout_ok, layout_issues = evaluate_manager_runtime_layout()
    lines.append(f"[{'OK' if layout_ok else 'WARN'}] 项目布局: {'正常' if layout_ok else '存在缺失'}")
    if layout_issues:
        lines.extend([f"  - {item}" for item in layout_issues[:6]])

    backend_python = get_backend_python_executable()
    lines.append(f"[{'OK' if backend_python.exists() else 'WARN'}] 后端虚拟环境: {backend_python}")
    lines.append(f"[{'OK' if ENV_PATH.exists() else 'WARN'}] 后端配置文件 backend/.env")
    lines.append(f"[{'OK' if KEY_PATH.exists() else 'WARN'}] 加密密钥 backend/.encryption.key")
    lines.append(f"[{'OK' if (DIST_DIR / 'index.html').exists() else 'WARN'}] 前端构建产物 frontend/dist")

    git_ok = app.check_git_available()
    lines.append(f"[{'OK' if git_ok else 'WARN'}] Git 命令")

    node_ok = is_command_available("node")
    lines.append(f"[{'OK' if node_ok else 'WARN'}] Node.js")

    sqlcmd_value = app.ops_vars.get("sqlcmd_path").get().strip() if app.ops_vars.get("sqlcmd_path") else "sqlcmd"
    sqlcmd_ok = is_command_available(sqlcmd_value or "sqlcmd")
    lines.append(f"[{'OK' if sqlcmd_ok else 'WARN'}] sqlcmd: {sqlcmd_value or 'sqlcmd'}")

    startup_enabled = bool(app.manager_state.get("launch_manager_on_startup", False))
    startup_ready = startup_task_exists() or STARTUP_SCRIPT_PATH.exists()
    lines.append(f"[{'OK' if (startup_enabled and startup_ready) else 'WARN'}] 开机自启: {app.get_startup_status_text()}")

    runtime_state = read_managed_runtime_state()
    host_alive = bool(runtime_state.get("service_host_alive"))
    lines.append(f"[{'OK' if host_alive else 'WARN'}] 后台宿主: {app.get_host_status_text()}")

    port = (config.get("APP_PORT") or "8100").strip() or "8100"
    if port.isdigit():
        owner = get_port_owner_info(int(port))
        if owner:
            lines.append(f"[WARN] 后端端口占用: {build_port_owner_label(owner)}")
        else:
            lines.append(f"[OK] 后端端口 {port}: 未检测到冲突")
    else:
        lines.append(f"[WARN] APP_PORT 无效: {port}")

    db_state, db_detail = describe_database_configuration(config)
    lines.append(f"[{'OK' if db_state == 'ready' else 'WARN'}] 数据库配置: {db_detail}")

    recommendations: List[str] = []
    if not ENV_PATH.exists():
        recommendations.append("先在“系统配置”页保存 backend/.env")
    if not KEY_PATH.exists():
        recommendations.append("生成或放入 backend/.encryption.key")
    if not backend_python.exists():
        recommendations.append("执行“同步后端依赖”以创建虚拟环境")
    if not (DIST_DIR / "index.html").exists():
        recommendations.append("执行“同步前端依赖”并构建前端")
    if not startup_ready:
        recommendations.append("启用开机自启，让服务器重启后自动拉起后台宿主")
    if not host_alive:
        recommendations.append("启动后台宿主，进入无人值守运行模式")
    ...
    if recommendations:
        ...
        lines.append("- 当前接管条件基本齐备，可直接进入运行与验活")
    return "\n".join(lines)
...
def create_backend_start_request() -> None:
    ensure_runtime_dir()
    BACKEND_START_REQUEST_PATH.write_text("start\n", encoding="utf-8")


def clear_backend_start_request() -> None:
    BACKEND_START_REQUEST_PATH.unlink(missing_ok=True)


def create_frontend_start_request() -> None:
    ensure_runtime_dir()
    FRONTEND_START_REQUEST_PATH.write_text("start\n", encoding="utf-8")


def clear_frontend_start_request() -> None:
    FRONTEND_START_REQUEST_PATH.unlink(missing_ok=True)


def create_frontend_stop_request() -> None:
    ensure_runtime_dir()
    FRONTEND_STOP_REQUEST_PATH.write_text("stop\n", encoding="utf-8")


def clear_frontend_stop_request() -> None:
    FRONTEND_STOP_REQUEST_PATH.unlink(missing_ok=True)


def create_frontend_restart_request() -> None:
    ensure_runtime_dir()
    FRONTEND_RESTART_REQUEST_PATH.write_text("restart\n", encoding="utf-8")


def clear_frontend_restart_request() -> None:
    FRONTEND_RESTART_REQUEST_PATH.unlink(missing_ok=True)


# 进程存活状态缓存
_process_alive_cache: Dict[int, Tuple[bool, float]] = {}
_process_alive_cache_ttl = 1.0  # 缓存有效期 1 秒

def is_process_alive(pid: int, use_cache: bool = True) -> bool:
    if pid <= 0:
        return False
    
    # 检查缓存
    if use_cache and pid in _process_alive_cache:
        result, timestamp = _process_alive_cache[pid]
        if time.time() - timestamp < _process_alive_cache_ttl:
            return result
    
    # 在 Windows 上，使用 ctypes 调用 OpenProcess 更可靠
    # 特别是在跨会话检测进程时（如 Windows 服务在 Session 0，GUI 在用户会话）
    result = False
    ...
    # 更新缓存
    if use_cache:
        _process_alive_cache[pid] = (result, time.time())
    return result
...
def get_backend_pythonw_executable() -> Path:
...
# 端口查询缓存
_port_cache: Dict[int, Tuple[List[int], float]] = {}
_port_cache_ttl = 3.0  # 缓存有效期 3 秒

def find_listening_pids(port: int, use_cache: bool = True) -> List[int]:
    """查找监听指定端口的进程 PID
    ...
    # 检查缓存
    if use_cache and port in _port_cache:
        pids, timestamp = _port_cache[port]
        if time.time() - timestamp < _port_cache_ttl:
            return pids
    ...
    # 更新缓存
    _port_cache[port] = (pids, time.time())
    return pids


# 进程信息缓存
_process_info_cache: Dict[int, Tuple[Dict[str, str], float]] = {}
_process_info_cache_ttl = 5.0  # 缓存有效期 5 秒
...
def get_port_owner_info(port: int, use_cache: bool = True, include_command_line: bool = False) -> Dict[str, str]:
    """获取端口占用信息
    ...
    Returns:
        端口占用信息字典
    """
    global _process_info_cache
    
    # 先获取端口对应的 PID
    pids = find_listening_pids(port, use_cache=use_cache)
    if not pids:
        return {}
    pid = pids[0]
    
    # 检查缓存
    if use_cache and pid in _process_info_cache:
        info, timestamp = _process_info_cache[pid]
        if time.time() - timestamp < _process_info_cache_ttl:
            return info
    ...
    info = {
        ...
        "command_line": get_process_commandline(pid) if include_command_line else "",
    }
    
    # 更新缓存
    _process_info_cache[pid] = (info, time.time())
    return info
...
def build_port_owner_label(owner: Dict[str, str], manager_pid: int | None = None) -> str:
    ...
    if manager_pid is not None and pid == str(manager_pid):
        return "当前服务实例"
    process_name = (owner.get("name", "") or "").strip().lower()
    if process_name in {"python.exe", "pythonw.exe"}:
        return f"Python 进程 (PID {pid})"
    if process_name in {"node.exe", "node"}:
        return f"Node 进程 (PID {pid})"
    if process_name:
        return f"{process_name} (PID {pid})"
    return f"其他进程 (PID {pid})"


def _format_port_owner_summary(owner: Dict[str, str] | None) -> str:
    if not owner:
        return "空闲"
    process_name = (owner.get("name", "") or "").strip().lower()
    pid = owner.get("pid", "") or "未知"
    if process_name in {"python.exe", "pythonw.exe"}:
        return f"Python 占用 (PID {pid})"
    if process_name in {"node.exe", "node"}:
        return f"Node 占用 (PID {pid})"
    if process_name:
        return f"{process_name} 占用 (PID {pid})"
    return f"已占用 (PID {pid})"
...
def read_file_tail_bytes(path: Path, max_bytes: int = 262144) -> bytes:
    if not path.exists():
        return b""
    try:
...
def tail_text_file(path: Path, max_lines: int = 40) -> str:
    if not path.exists():
        return ""
    try:
        lines = sanitize_console_text(decode_console_output(read_file_tail_bytes(path))).splitlines()
    except Exception:
        return ""
...
def read_log_since_marker(path: Path, marker: str, max_lines: int = 80) -> str:
    if not path.exists():
        return ""
    try:
        lines = sanitize_console_text(decode_console_output(read_file_tail_bytes(path, max_bytes=524288))).splitlines()
    except Exception:
        return ""
    if marker:
...
def resolve_frontend_service_settings(config: Dict[str, str] | None = None) -> Dict[str, str]:
    ...
    # 前端配置：优先使用 FRONTEND_HOST/FRONTEND_PORT，否则使用 APP_HOST 或默认值
    raw_frontend_host = (effective_config.get("FRONTEND_HOST") or "").strip()
    if raw_frontend_host:
        # 用户明确配置了前端监听地址
        frontend_bind_host = "0.0.0.0" if raw_frontend_host in {"0.0.0.0", "::"} else raw_frontend_host
    else:
        # 未配置时，根据 APP_HOST 自动判断
    ...
    frontend_port = (effective_config.get("FRONTEND_PORT") or "").strip()
    ...
    # 生产模式下，必须使用完整的后端地址，而不是相对路径 /api
    # 因为前端静态服务器不代理 API 请求
    # 相对路径（如 /api）或空值都需要替换为完整地址
    # 如果 VITE_API_BASE_URL 设置为 auto，则保持原样，让前端自动推断
    if not api_base_url or api_base_url == "/api":
        api_base_url = f"http://{backend_host}:{backend_port}/api"
    # 注意：如果 VITE_API_BASE_URL 设置为 auto，则保持原样，让前端自动推断

    return {
        "frontend_host": frontend_bind_host,  # 绑定地址：0.0.0.0 或指定地址
        "frontend_port": frontend_port,
        "frontend_url": f"http://{backend_host}:{frontend_port}/",  # 显示给用户的 URL
...
def sync_frontend_runtime_env(config: Dict[str, str]) -> Tuple[bool, str]:
    ...
    # 如果 VITE_API_BASE_URL 设置为 auto，则保持原样，不覆盖
    api_base_url = (current_values.get("VITE_API_BASE_URL") or "").strip()
    if api_base_url.lower() == "auto":
        current_values["VITE_API_BASE_URL"] = "auto"
    else:
        current_values["VITE_API_BASE_URL"] = settings["api_base_url"]
...
def resolve_manager_launcher(start_minimized: bool = False) -> List[str]:
    if getattr(sys, "frozen", False):
        launcher = [str(Path(sys.executable).resolve())]
        if start_minimized:
            launcher.append("--minimized")
        return launcher
    ...
    launcher = [str(python_exe), str(Path(__file__).resolve())]
    if start_minimized:
        launcher.append("--minimized")
    return launcher
...
def render_startup_script() -> str:
    launch_parts = " ".join(f'"{part}"' for part in resolve_manager_launcher(start_minimized=True))
    return f'@echo off\ncd /d "{ROOT_DIR}"\nstart "" {launch_parts}\n'


def _run_windows_command(command: List[str], timeout: int = 15) -> subprocess.CompletedProcess:
    creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
...
def get_startup_task_commandline() -> str:
    return subprocess.list2cmdline(resolve_background_host_launcher())
...
def startup_task_exists() -> bool:
    if os.name != "nt":
        return False
    try:
        result = _run_windows_command(["schtasks", "/Query", "/TN", STARTUP_TASK_NAME], timeout=10)
...
def create_startup_task() -> Tuple[bool, str]:
    if os.name != "nt":
        return False, "Task Scheduler is only supported on Windows"
    commandline = get_startup_task_commandline()
    command = [
        "schtasks",
        "/Create",
        "/TN",
    ...
    try:
        result = _run_windows_command(command, timeout=20)
    except Exception as exc:
        return False, str(exc)
    if result.returncode == 0:
        return True, "scheduled task"
    detail = sanitize_console_text(
        decode_console_output(result.stderr or b"") or decode_console_output(result.stdout or b"")
    ).strip()
    return False, detail or f"schtasks exited with code {result.returncode}"
...
def delete_startup_task() -> Tuple[bool, str]:
    ...
    if not startup_task_exists():
        return True, "scheduled task not found"
    try:
        result = _run_windows_command(["schtasks", "/Delete", "/TN", STARTUP_TASK_NAME, "/F"], timeout=15)
    except Exception as exc:
        return False, str(exc)
    if result.returncode == 0:
...
def get_startup_mode_label() -> str:
...
def get_startup_task_detail() -> str:
    if os.name != "nt":
        return "Task Scheduler is only supported on Windows"
    if not startup_task_exists():
        return "Task Scheduler entry was not found"
    try:
        result = _run_windows_command(
            ["schtasks", "/Query", "/TN", STARTUP_TASK_NAME, "/V", "/FO", "LIST"],
            timeout=20,
        )
    except Exception as exc:
        return str(exc)
    detail = sanitize_console_text(
        decode_console_output(result.stdout or b"") or decode_console_output(result.stderr or b"")
    ).strip()
    if not detail:
        return f"schtasks exited with code {result.returncode}"
    return detail
...
def resolve_python_launcher() -> List[str]:
    ...
    if "python" in exe_name and Path(sys.executable).exists():
        candidates.append(sys.executable)
    backend_python = get_backend_python_executable()
    if backend_python.exists():
        candidates.append(str(backend_python))
    ...
    if not unique:
        raise RuntimeError("未找到可用的 Python 解释器，无法执行更新打包脚本")
    return [unique[0]]
...
def record_backup_change(change_log: Dict[str, List[str]] | None, bucket: str, path: Path) -> None:
    if change_log is None:
        return
    change_log.setdefault(bucket, [])
    normalized = path.as_posix()
    if normalized not in change_log[bucket]:
        change_log[bucket].append(normalized)
...
def overlay_directory(
    ...
    *,
    ...
    change_log: Dict[str, List[str]] | None = None,
) -> int:
    copied = 0
    for source_file in source_dir.rglob("*"):
        ...
        if compare_existing and existed and files_are_identical(source_file, destination):
            continue
        if existed:
            backup_existing_path(destination, backup_root)
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source_file, destination)
        record_backup_change(change_log, "updated_files" if existed else "created_files", relative_to_root)
        copied += 1
    return copied
...
def replace_directory(
    ...
    record_backup_change(
        change_log,
        "replaced_dirs" if existed else "created_dirs",
...
def synchronize_directory(
    ...
    *,
    ...
    if not delete_missing or not target_dir.exists():
        return changed
    ...
    removed_files = 0
    for target_file in sorted(target_dir.rglob("*"), reverse=True):
        if not target_file.is_file():
            continue
        relative_to_target = target_file.relative_to(target_dir)
        relative_to_root = target_file.relative_to(ROOT_DIR)
        if should_skip_release_path(relative_to_root):
            continue
        if relative_to_target in source_files:
            continue
        backup_existing_path(target_file, backup_root)
        target_file.unlink(missing_ok=True)
        record_backup_change(change_log, "deleted_files", relative_to_root)
        removed_files += 1

    for directory in sorted([item for item in target_dir.rglob("*") if item.is_dir()], reverse=True):
        try:
            if not any(directory.iterdir()):
                directory.rmdir()
                relative_to_root = directory.relative_to(ROOT_DIR)
                record_backup_change(change_log, "deleted_dirs", relative_to_root)
        except Exception:
            continue
...
def copy_release_root_files(
    ...
    for item in release_root.iterdir():
        ...
        record_backup_change(change_log, "updated_files" if existed else "created_files", relative_path)
        copied += 1
    return copied


def write_upgrade_backup_manifest(
    backup_root: Path,
    action: str,
    package_label: str,
    change_log: Dict[str, List[str]] | None = None,
    ...
    try:
        ...
        payload = {
            ...
            "changes": change_log or {},
        }
        (backup_root / "_manifest.json").write_text(
...
def read_upgrade_backup_manifest(backup_root: Path) -> Dict[str, Any]:
...
def list_upgrade_backups() -> List[Path]:
    if not UPGRADE_BACKUP_DIR.exists():
        return []
    return sorted([item for item in UPGRADE_BACKUP_DIR.iterdir() if item.is_dir()], key=lambda p: p.name, reverse=True)


def summarize_upgrade_backup(backup_root: Path) -> str:
    manifest = read_upgrade_backup_manifest(backup_root)
    label = str(manifest.get("package_label", "") or backup_root.name).strip()
    action = str(manifest.get("action", "") or "backup").strip()
    created_at = str(manifest.get("created_at", "") or "").strip()
    changes = manifest.get("changes", {}) or {}
    total_items = 0
    if isinstance(changes, dict):
        total_items = sum(len(value or []) for value in changes.values() if isinstance(value, list))
    if total_items <= 0:
        try:
            total_items = sum(1 for item in backup_root.rglob("*") if item.is_file() and item.name != "_manifest.json")
        except Exception:
            total_items = 0
    summary = label
    if created_at:
...
def rollback_from_upgrade_backup(backup_root: Path) -> Tuple[bool, str]:
    if not backup_root.exists() or not backup_root.is_dir():
        return False, f"备份目录不存在: {backup_root}"

    manifest = read_upgrade_backup_manifest(backup_root)
    changes = manifest.get("changes", {}) if isinstance(manifest, dict) else {}
    created_files = [Path(item) for item in changes.get("created_files", []) if item]
    created_dirs = [Path(item) for item in changes.get("created_dirs", []) if item]
    replaced_dirs = [Path(item) for item in changes.get("replaced_dirs", []) if item]
    ...
    for relative_path in created_dirs:
        target = ROOT_DIR / relative_path
        if target.exists():
            remove_path(target)

    for relative_path in replaced_dirs:
        target = ROOT_DIR / relative_path
        if target.exists():
            remove_path(target)
    ...
    detail = f"已从备份恢复 {restored} 个文件: {backup_root}"
    if created_files or created_dirs or replaced_dirs:
        detail += (
            f"\n移除新增文件 {len(created_files)} 个"
            f"\n移除新增目录 {len(created_dirs)} 个"
            f"\n恢复替换目录 {len(replaced_dirs)} 个"
        )
    return True, detail
...
class BackendProcessController:
    def __init__(self) -> None:
        ...
        self.process: subprocess.Popen | SimpleNamespace | None = (
            SimpleNamespace(pid=backend_pid) if is_process_alive(backend_pid) else None
        )
        self.stdout_handle = None
        self.stderr_handle = None
        self.user_stopped = bool(state.get("user_stopped", False))
        self.last_start_attempt = float(state.get("last_start_attempt", 0.0) or 0.0)
        self.last_exit_code = state.get("last_exit_code")
        self.last_exit_at = float(state.get("last_exit_at", 0.0) or 0.0)
        self.last_started_at = float(state.get("last_started_at", 0.0) or 0.0)
        self.consecutive_failed_starts = int(state.get("consecutive_failed_starts", 0) or 0)
        self.auto_restart_suppressed = bool(state.get("auto_restart_suppressed", False))
        self.last_session_marker = str(state.get("last_session_marker", "") or "")
        self.last_session_started_label = str(state.get("last_session_started_label", "") or "")
        if backend_pid > 0 and self.process is None:
            write_runtime_state(backend_pid=0)
    ...
    def is_running(self) -> bool:
        if safe_process_is_running(self.process):
            return True
        state = read_runtime_state()
        backend_pid = int(state.get("backend_pid") or 0)
        if is_process_alive(backend_pid):
            self.process = SimpleNamespace(pid=backend_pid)
            return True
        return False
    ...
    def capture_exit(self) -> int | None:
        ...
        if self.last_started_at > 0:
            runtime_seconds = max(0.0, time.time() - self.last_started_at)
        ...
        append_text_log(
            SERVICE_HOST_LOG,
            f"[host] backend process exited: pid={pid} code={code} "
            f"user_stopped={self.user_stopped} runtime_seconds={runtime_seconds:.1f} "
            f"started_label={self.last_session_started_label or '-'}",
        )
        return code
    ...
    def start(self, config: Dict[str, str]) -> Tuple[bool, str]:
        ...
        # 使用 pythonw.exe 而不是 python.exe，避免创建控制台窗口
        python_exe = get_backend_pythonw_executable()

        # 使用 Windows API 标志确保后端进程：
        # 1. 在独立的会话中运行（RDP断开时不被终止）
        # 2. 不显示控制台窗口
        # 3. 创建新的进程组
        creationflags = 0
        creationflags |= 0x08000000  # CREATE_NO_WINDOW - 不创建控制台窗口
        creationflags |= 0x00000008  # DETACHED_PROCESS - 脱离父进程控制台
        creationflags |= 0x00000200  # CREATE_NEW_PROCESS_GROUP - 新进程组
...
def resolve_service_host_launcher() -> List[str]:
    return resolve_background_host_launcher()


def resolve_frozen_worker_executable() -> Path:
    current_exe = Path(sys.executable).resolve()
    worker_exe = current_exe.with_name("FinFlowManagerHost.exe")
    if worker_exe.exists():
        return worker_exe
    worker_dir_exe = current_exe.with_name("FinFlowManagerHost") / "FinFlowManagerHost.exe"
    if worker_dir_exe.exists():
        return worker_dir_exe
    return current_exe


def resolve_background_host_launcher() -> List[str]:
    if getattr(sys, "frozen", False):
        return [str(resolve_frozen_worker_executable()), "--host"]
    ...
    return [str(python_exe), str(Path(__file__).resolve()), "--host"]


def resolve_frontend_host_launcher() -> List[str]:
    if getattr(sys, "frozen", False):
        return [str(resolve_frozen_worker_executable()), "--frontend-run"]
...
def read_managed_runtime_state() -> Dict[str, Any]:
    ...
    # 不要因为进程检测失败就重置 PID，特别是 Windows 服务在不同会话中运行时
    # 只在确实需要时重置 PID（例如服务明确停止）
    # if not host_alive:
...
class ManagedBackendController:
    ...
    def is_running(self) -> bool:
        ...
        host_alive = is_process_alive(host_pid)
        backend_alive = is_process_alive(backend_pid)
        if host_alive or backend_alive:
            write_runtime_state(
                service_host_alive=host_alive,
...
class FinFlowRuntimeHost:
    def __init__(self) -> None:
        append_text_log(SERVICE_HOST_LOG, "===== FinFlowRuntimeHost.__init__ =====")
        self.backend = BackendProcessController()
        self.frontend = FrontendProcessController()
        self.stop_event = threading.Event()
        append_text_log(SERVICE_HOST_LOG, "===== FinFlowRuntimeHost.__init__ ready =====")

    def request_stop(self, *_args: Any) -> None:
        write_runtime_state(host_stop_reason="signal")
        self.stop_event.set()
    ...
    def _initialize_runtime_state(self) -> None:
        ...
        ensure_log_dir()
        clear_stop_request()
        clear_backend_start_request()
        clear_frontend_start_request()
        clear_frontend_stop_request()
        clear_frontend_restart_request()
        now = time.time()
        # 读取现有状态，而不是创建新状态，避免覆盖其他进程写入的数据
        state = read_runtime_state()
        state["service_host_pid"] = os.getpid()
        state["host_started_at"] = now
        state["host_last_heartbeat_at"] = now
        state["host_last_transition"] = "starting"
        state["host_stop_reason"] = ""
        state["user_stopped"] = False
        state["frontend_user_stopped"] = False
        RUNTIME_STATE_PATH.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    def _sync_runtime_state(self) -> None:
        write_runtime_state(
            service_host_pid=os.getpid(),
            backend_pid=safe_process_pid(self.backend.process) if self.backend.is_running() else 0,
            frontend_pid=safe_process_pid(self.frontend.process) if self.frontend.is_running() else 0,
            host_last_heartbeat_at=time.time(),
            ...
            last_session_marker=self.backend.last_session_marker,
            last_session_started_label=self.backend.last_session_started_label,
            frontend_user_stopped=self.frontend.user_stopped,
            frontend_last_start_attempt=self.frontend.last_start_attempt,
            frontend_last_exit_code=self.frontend.last_exit_code,
            frontend_last_exit_at=self.frontend.last_exit_at,
            frontend_last_started_at=self.frontend.last_started_at,
            frontend_consecutive_failed_starts=self.frontend.consecutive_failed_starts,
            frontend_auto_restart_suppressed=self.frontend.auto_restart_suppressed,
            frontend_last_session_marker=self.frontend.last_session_marker,
            frontend_last_session_started_label=self.frontend.last_session_started_label,
        )

    def _start_initial_services(self, config: Dict[str, str], state: Dict[str, Any]) -> None:
        if state.get("start_backend_on_launch", False):
            ok, message = self.backend.start(config)
            append_text_log(SERVICE_HOST_LOG, f"[host] backend initial start: {message}")
            if not ok:
                append_text_log(SERVICE_HOST_LOG, f"[host] backend initial start failed: {message}")
        if state.get("start_frontend_on_launch", False):
            ok, message = self.frontend.start(config)
            append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] initial start: {message}")
            if not ok:
                append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] initial start failed: {message}")

    def _maintain_backend(self, config: Dict[str, str], state: Dict[str, Any]) -> None:
        self.backend.capture_exit()
        auto_restart_enabled = bool(state.get("auto_restart_backend", True))
        backend_running = self.backend.is_running()
        seconds_since_last_start = time.time() - self.backend.last_start_attempt if self.backend.last_start_attempt > 0 else -1
        restart_due = (
            auto_restart_enabled
            and not backend_running
            and not self.backend.user_stopped
            and not self.backend.auto_restart_suppressed
            and seconds_since_last_start > 8
            and self.backend.last_start_attempt > 0
        )
        if restart_due:
            append_text_log(
                SERVICE_HOST_LOG,
                "[host] backend restart trigger: "
                f"running={backend_running} "
                f"user_stopped={self.backend.user_stopped} "
                f"auto_restart_enabled={auto_restart_enabled} "
                f"auto_restart_suppressed={self.backend.auto_restart_suppressed} "
                f"last_exit_code={self.backend.last_exit_code} "
                f"seconds_since_last_start={seconds_since_last_start:.1f}",
            )
            self.backend.register_failed_start_if_needed()
            ok, message = self.backend.start(config)
            append_text_log(SERVICE_HOST_LOG, f"[host] backend restart: {message}")
            if not ok:
                append_text_log(SERVICE_HOST_LOG, f"[host] backend restart failed: {message}")
        if self.backend.is_running():
            self.backend.consecutive_failed_starts = 0

    def _maintain_frontend(self, config: Dict[str, str], state: Dict[str, Any]) -> None:
        self.frontend.capture_exit()
        if (
            state.get("auto_restart_frontend", True)
            ...
            self.frontend.register_failed_start_if_needed()
            ok, message = self.frontend.start(config)
            append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] auto-restart: {message}")
            if not ok:
                append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] auto-restart failed: {message}")
    ...
    def run(self) -> int:
        append_text_log(SERVICE_HOST_LOG, "===== FinFlowRuntimeHost.run begin =====")
        ...
        config = load_effective_config_from_disk()
        state = read_state_file(STATE_PATH)
        self._start_initial_services(config, state)
        self._sync_runtime_state()
        append_text_log(SERVICE_HOST_LOG, "===== FinFlowRuntimeHost.run loop enter =====")

        while not self.stop_event.is_set():
            if STOP_REQUEST_PATH.exists():
                self.backend.user_stopped = True
                self.frontend.user_stopped = True
                write_runtime_state(host_stop_reason="stop_requested")
                self.stop_event.set()
                break

            config = load_effective_config_from_disk()
            state = read_state_file(STATE_PATH)
            if BACKEND_START_REQUEST_PATH.exists():
                self.backend.user_stopped = False
                ok, message = self.backend.start(config)
                append_text_log(SERVICE_HOST_LOG, f"[host] backend requested start: {message}")
                if not ok:
                    append_text_log(SERVICE_HOST_LOG, f"[host] backend requested start failed: {message}")
                clear_backend_start_request()
            if FRONTEND_START_REQUEST_PATH.exists():
                self.frontend.user_stopped = False
                ok, message = self.frontend.start(config)
                append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] request=start result: {message}")
                if not ok:
                    append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] request=start failed: {message}")
                clear_frontend_start_request()
            if FRONTEND_STOP_REQUEST_PATH.exists():
                self.frontend.user_stopped = True
                ok, message = self.frontend.stop()
                append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] request=stop result: {message}")
                if not ok:
                    append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] request=stop failed: {message}")
                clear_frontend_stop_request()
            if FRONTEND_RESTART_REQUEST_PATH.exists():
                self.frontend.user_stopped = False
                ok, message = self.frontend.restart(config)
                append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] request=restart result: {message}")
                if not ok:
                    append_text_log(SERVICE_HOST_LOG, f"[host-managed frontend] request=restart failed: {message}")
                clear_frontend_restart_request()
            self._maintain_backend(config, state)
            self._maintain_frontend(config, state)
            self._sync_runtime_state()
            time.sleep(1)
        ...
        clear_frontend_stop_request()
        clear_frontend_restart_request()
        previous_started_at = read_runtime_state().get("host_started_at", 0.0)
        previous_stop_reason = str(read_runtime_state().get("host_stop_reason", "") or "stopped")
        self._initialize_runtime_state()
        write_runtime_state(
            service_host_pid=0,
            backend_pid=0,
            frontend_pid=0,
            host_started_at=previous_started_at,
            host_last_heartbeat_at=time.time(),
            host_last_transition="stopped",
            host_stop_reason=previous_stop_reason,
        )
        append_text_log(SERVICE_HOST_LOG, "===== FinFlowRuntimeHost.run end =====")
        return 0
...
class FinFlowManagerApp:
    def __init__(self, start_minimized: bool = False) -> None:
        ...
        self.status_nav_items: Dict[str, Dict[str, Any]] = {}
        self.status_section_frames: Dict[str, ttk.Frame] = {}
    ...
    def save_config_values(self) -> None:
        ...
        payload, _existed = save_backend_env_config(data, self.extra_env_values)
        self.config_values = payload
    ...
    def show_status_section(self) -> None:
        active_key = getattr(self, "status_section_var", tk.StringVar(value="overview")).get()
        for section_key, frame in self.status_section_frames.items():
            if section_key == active_key:
                frame.pack(fill="both", expand=True)
            else:
                frame.pack_forget()
        self.refresh_side_nav_styles(self.status_nav_items, active_key)
    ...
    def refresh_takeover_guide(self) -> None:
        summary = build_takeover_check_summary(self)
        if hasattr(self, "takeover_summary_text"):
            self.set_readonly_text(self.takeover_summary_text, summary)
        if hasattr(self, "takeover_status_var"):
            self.takeover_status_var.set(f"最近检查: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        append_audit_log("takeover_check", "success", "environment takeover checklist refreshed")
    ...
    def get_startup_status_text(self) -> str:
        ...
        task_exists = startup_task_exists()
        exists = STARTUP_SCRIPT_PATH.exists()
        if enabled and task_exists:
            return "\u5df2\u542f\u7528\uff08\u8ba1\u5212\u4efb\u52a1\uff09"
        if enabled and exists:
            return "\u5df2\u542f\u7528\uff08\u542f\u52a8\u76ee\u5f55\uff09"
        if enabled and not exists and not task_exists:
            return "\u5f85\u5199\u5165"
        if task_exists:
            return "\u5df2\u5b58\u5728\u8ba1\u5212\u4efb\u52a1"
        if exists:
    ...
    def get_host_status_text(self) -> str:
        state = read_managed_runtime_state()
        host_pid = int(state.get("service_host_pid") or 0)
        backend_pid = int(state.get("backend_pid") or 0)
        frontend_pid = int(state.get("frontend_pid") or 0)
        ...
        service_running = service_info.get("running", False)
        service_pid = int(service_info.get("pid", 0))
        
        if host_alive or service_running:
            details: List[str] = []
            if host_alive:
                details.append(f"PID {host_pid}")
            if service_running:
                details.append(f"Windows 服务 PID {service_pid}")
            ...
            return "\u8fd0\u884c\u4e2d (" + ", ".join(details) + ")"
        if backend_alive or frontend_alive or backend_pid > 0 or frontend_pid > 0:
            fragments: List[str] = []
            if backend_alive or backend_pid > 0:
                fragments.append(f"backend {backend_pid}")
            if frontend_alive or frontend_pid > 0:
                fragments.append(f"frontend {frontend_pid}")
            return "\u5bbf\u4e3b\u672a\u8fd0\u884c\uff0c\u4f46\u5b50\u8fdb\u7a0b\u5b58\u6d3b (" + ", ".join(fragments) + ")"
        return "\u672a\u8fd0\u884c"

    def get_host_diagnostics_text(self) -> str:
        state = read_managed_runtime_state()
        parts = [
            f"started={format_runtime_timestamp(state.get('host_started_at'))}",
            f"heartbeat={format_runtime_timestamp(state.get('host_last_heartbeat_at'))}",
            f"transition={str(state.get('host_last_transition', '') or '-').strip() or '-'}",
            f"stop={str(state.get('host_stop_reason', '') or '-').strip() or '-'}",
            f"backend_failures={int(state.get('consecutive_failed_starts', 0) or 0)}",
            f"frontend_failures={int(state.get('frontend_consecutive_failed_starts', 0) or 0)}",
        ]
        return " | ".join(parts)

    def get_audit_status_text(self) -> str:
        return get_last_audit_summary()

    def get_startup_mode_text(self) -> str:
        return get_startup_mode_label()

    def get_startup_detail_text(self) -> str:
        if startup_task_exists():
            return "Task Scheduler entry is active"
        if STARTUP_SCRIPT_PATH.exists():
            return str(STARTUP_SCRIPT_PATH)
        return "-"

    def get_startup_last_result_text(self) -> str:
        result = str(self.manager_state.get("startup_last_result", "") or "").strip()
        detail = str(self.manager_state.get("startup_last_sync_at", "") or "").strip()
        if result and detail:
            return f"{result} @ {detail}"
        return result or "-"

    def get_startup_last_error_text(self) -> str:
        return str(self.manager_state.get("startup_last_error", "") or "").strip() or "-"

    def get_manager_runtime_mode_text(self) -> str:
        runtime_state = read_managed_runtime_state()
        host_pid = int(runtime_state.get("service_host_pid") or 0)
        if runtime_state.get("service_host_alive"):
            return f"\u540e\u53f0\u5bbf\u4e3b\u6a21\u5f0f (PID {host_pid})"
        if self.start_minimized:
            return "\u540e\u53f0\u6258\u76d8\u6a21\u5f0f"
        return "\u684c\u9762\u7ba1\u7406\u6a21\u5f0f"

    def handle_start_host(self, show_dialog: bool = True) -> Tuple[bool, str]:
        ok, message = self.backend.start(self.get_effective_config())
        append_audit_log("host_start", "success" if ok else "failed", message)
        self.refresh_status()
        if show_dialog:
            dialog = messagebox.showinfo if ok else messagebox.showwarning
            dialog("\u540e\u53f0\u5bbf\u4e3b", message)
        return ok, message

    def handle_stop_host(self, show_dialog: bool = True) -> Tuple[bool, str]:
        ok, message = self.backend.stop()
        append_audit_log("host_stop", "success" if ok else "failed", message)
        self.refresh_status()
        if show_dialog:
            dialog = messagebox.showinfo if ok else messagebox.showwarning
            dialog("\u540e\u53f0\u5bbf\u4e3b", message)
    ...
    def handle_restart_host(self, show_dialog: bool = True) -> Tuple[bool, str]:
        ok, message = self.backend.restart(self.get_effective_config())
        append_audit_log("host_restart", "success" if ok else "failed", message)
        self.refresh_status()
        if show_dialog:
            dialog = messagebox.showinfo if ok else messagebox.showwarning
            dialog("\u540e\u53f0\u5bbf\u4e3b", message)
    ...
    def open_host_log(self) -> None:
        if not SERVICE_HOST_LOG.exists():
            messagebox.showwarning("\u540e\u53f0\u5bbf\u4e3b\u65e5\u5fd7", "\u5c1a\u672a\u751f\u6210\u5bbf\u4e3b\u65e5\u5fd7")
            return
        try:
            os.startfile(str(SERVICE_HOST_LOG))
        except Exception as exc:
            messagebox.showerror("\u6253\u5f00\u65e5\u5fd7\u5931\u8d25", str(exc))

    def open_audit_log(self) -> None:
        if not ACTIONS_AUDIT_LOG.exists():
            messagebox.showwarning("审计日志", "尚未生成审计日志")
            return
        try:
            os.startfile(str(ACTIONS_AUDIT_LOG))
        except Exception as exc:
            messagebox.showerror("打开审计日志失败", str(exc))

    def record_startup_sync_result(self, result: str, detail: str = "", error: str = "") -> None:
        self.manager_state["startup_last_result"] = build_startup_sync_label(result, detail)
        self.manager_state["startup_last_error"] = (error or "").strip()
        self.manager_state["startup_last_sync_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        write_state_file(STATE_PATH, self.manager_state)

    def enable_startup_now(self) -> None:
        self.manager_state["launch_manager_on_startup"] = True
        if "launch_manager_on_startup" in self.manager_option_vars:
            self.manager_option_vars["launch_manager_on_startup"].set(True)
        write_state_file(STATE_PATH, self.manager_state)
        self.sync_startup_entry(show_message=True)
        self.refresh_status()

    def disable_startup_now(self) -> None:
    ...
    def rebuild_startup_now(self) -> None:
        self.manager_state["launch_manager_on_startup"] = True
        if "launch_manager_on_startup" in self.manager_option_vars:
            self.manager_option_vars["launch_manager_on_startup"].set(True)
        write_state_file(STATE_PATH, self.manager_state)
    ...
    def show_startup_detail(self) -> None:
        if startup_task_exists():
            detail = get_startup_task_detail()
            title = "\u8ba1\u5212\u4efb\u52a1\u8be6\u60c5"
        elif STARTUP_SCRIPT_PATH.exists():
            detail = render_startup_script()
            title = "\u542f\u52a8\u76ee\u5f55\u811a\u672c"
        else:
            detail = "\u5f53\u524d\u672a\u914d\u7f6e\u5f00\u673a\u81ea\u542f"
            title = "\u5f00\u673a\u81ea\u542f\u8be6\u60c5"
    ...
    def sync_startup_entry(self, show_message: bool = False) -> None:
        ...
        try:
            if enabled:
                task_ok, task_detail = create_startup_task()
                if task_ok:
                    if STARTUP_SCRIPT_PATH.exists():
                        STARTUP_SCRIPT_PATH.unlink()
                    self.record_startup_sync_result("\u8ba1\u5212\u4efb\u52a1\u5df2\u751f\u6548", task_detail, "")
                else:
                    STARTUP_DIR.mkdir(parents=True, exist_ok=True)
                    STARTUP_SCRIPT_PATH.write_text(render_startup_script(), encoding="utf-8")
                    self.record_startup_sync_result("\u5df2\u56de\u9000\u542f\u52a8\u76ee\u5f55", str(STARTUP_SCRIPT_PATH), task_detail)
                    task_detail = f"\u8ba1\u5212\u4efb\u52a1\u521b\u5efa\u5931\u8d25\uff0c\u5df2\u56de\u9000\u5230\u542f\u52a8\u76ee\u5f55\uff1a{task_detail}"
            else:
                task_ok, task_detail = delete_startup_task()
                if STARTUP_SCRIPT_PATH.exists():
                    STARTUP_SCRIPT_PATH.unlink()
                self.record_startup_sync_result("\u5df2\u5173\u95ed\u5f00\u673a\u81ea\u542f", task_detail, "")
        except Exception as exc:
            self.manager_state["launch_manager_on_startup"] = False
            if "launch_manager_on_startup" in self.manager_option_vars:
                self.manager_option_vars["launch_manager_on_startup"].set(False)
            self.record_startup_sync_result("\u81ea\u542f\u540c\u6b65\u5931\u8d25", "", str(exc))
            write_state_file(STATE_PATH, self.manager_state)
            if show_message:
        ...
        if show_message:
            if enabled:
                messagebox.showinfo("\u8bbe\u7f6e\u6210\u529f", f"\u5df2\u66f4\u65b0\u7ba1\u7406\u5668\u5f00\u673a\u81ea\u542f\u8bbe\u7f6e\u3002\n{task_detail}")
            else:
                messagebox.showinfo("\u8bbe\u7f6e\u6210\u529f", "\u5df2\u5173\u95ed\u7ba1\u7406\u5668\u5f00\u673a\u81ea\u542f\u8bbe\u7f6e")
    ...
    def select_path_for_var(self, key: str, mode: str) -> None:
        ...
        if selected:
            ...
            if key == "release_package_path":
                if hasattr(self, "zip_info_var"):
                    self.zip_info_var.set(f"已选择更新 ZIP：{selected}")
                try:
                    self.preview_release_package()
                except Exception:
                    pass
    ...
    def handle_start_all(self, show_dialog: bool = True) -> Tuple[bool, str]:
        ...
        append_audit_log("services_start_all", "success" if all_started else "failed", message)
        if all_started:
    ...
    def handle_stop_all(self, show_dialog: bool = True) -> Tuple[bool, str]:
        ...
        append_audit_log("services_stop_all", "success" if all_stopped else "failed", message)
        if all_stopped:
    ...
    def handle_restart_all(self, show_dialog: bool = True) -> Tuple[bool, str]:
        ...
        self.last_backend_health_state = "unknown"
        self.last_frontend_health_state = "unknown"
        append_audit_log("services_restart_all", "success" if ok else "failed", message)
    ...
    def build_update_package(self) -> None:
        self.save_manager_state()
        script_path = ROOT_DIR / "tools" / "pack_update.py"
        if not script_path.exists():
            messagebox.showerror("打包失败", f"未找到更新打包脚本：{script_path}")
            return

        current_value = self.ops_vars["release_package_path"].get().strip()
        suggested_name = f"FinFlow_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
        initial_dir = str(Path(current_value).parent) if current_value else str(ROOT_DIR)
        initial_file = Path(current_value).name if current_value.lower().endswith(".zip") else suggested_name
        ...
        self.ops_vars["release_package_path"].set(target_path)
        self.save_manager_state()

        try:
            command = resolve_python_launcher() + [str(script_path), "-o", target_path]
            result = subprocess.run(
                command,
                cwd=ROOT_DIR,
                capture_output=True,
            ...
            messagebox.showerror("打包失败", f"执行更新打包脚本失败：{exc}")
            return

        output = "\n".join(part for part in [result.stdout.strip(), result.stderr.strip()] if part).strip()
        if result.returncode != 0:
            messagebox.showerror("打包失败", output or "更新打包脚本执行失败")
            return

        self.preview_release_package()
        messagebox.showinfo("打包完成", output or f"更新 ZIP 已生成：{target_path}")

    def apply_release_package(self) -> None:
        self.save_manager_state()
        package_path = Path(self.ops_vars["release_package_path"].get().strip())
        if not package_path.exists() or not package_path.is_file():
            messagebox.showerror("更新失败", "请选择有效的更新 ZIP 文件")
            return
        if package_path.suffix.lower() != ".zip":
            messagebox.showerror("更新失败", "更新包必须是 .zip 文件")
            return

        proceed = messagebox.askyesno(
            "确认应用更新",
            "更新过程会自动停止前后端，先备份当前版本，再按差异替换 ZIP 中的后端文件和前端 dist，同时保留本机配置、密钥、虚拟环境与日志。是否继续？",
        )
        if not proceed:
            return
        ...
        try:
            ...
            messagebox.showerror("更新失败", "更新包不是有效的 ZIP 文件")
            return
        except Exception as exc:
            shutil.rmtree(extract_dir, ignore_errors=True)
            messagebox.showerror("更新失败", f"解压更新包失败：{exc}")
            return
        ...
        try:
            ...
            change_log: Dict[str, List[str]] = {}
            if has_backend:
                copied_counts["backend"] = overlay_directory(
            ...
            if has_tools:
                copied_counts["tools"] = overlay_directory(
                    release_root / "tools",
                    ROOT_DIR / "tools",
                    backup_root,
                    compare_existing=False,
            ...
            if has_deploy:
                copied_counts["deploy"] = overlay_directory(
                    release_root / "deploy",
                    ROOT_DIR / "deploy",
                    backup_root,
                    compare_existing=False,
            ...
            if has_root_files:
                copied_counts["root"] = copy_release_root_files(release_root, backup_root, change_log)
            if has_frontend_dist:
                copied_counts["frontend_dist"] = overlay_directory(
            ...
            write_upgrade_backup_manifest(backup_root, "update_zip", package_path.name, change_log)
            ...
            if backend_was_running:
                ...
                if not ok:
                    raise RuntimeError(f"更新完成，但后端自动重启失败：{restart_message}")
            if frontend_was_running:
                ok, restart_message = self.frontend.start(self.get_effective_config())
                restart_messages.append(f"前端：{restart_message}")
                if not ok:
                    raise RuntimeError(f"更新完成，但前端自动重启失败：{restart_message}")

            self.last_backend_health_state = "unknown"
            self.last_frontend_health_state = "unknown"
            self.refresh_status()
            self.notify_tray("更新 ZIP 已应用", f"已应用：{package_path.name}")
            summary = "，".join(f"{key}={value}" for key, value in copied_counts.items() if value)
            total_written = sum(int(value or 0) for value in copied_counts.values())
            detail = (
                f"更新完成。\n"
                f"备份目录：{backup_root}\n"
                f"写入文件：{total_written} 个\n"
                f"覆盖明细：{summary or '无文件写入'}"
            )
            if restart_messages:
            ...
            if restored_services:
                self.notify_tray("更新 ZIP 应用失败", f"已尝试恢复服务：{', '.join(restored_services)}")
            messagebox.showerror("更新失败", detail)
        finally:
            shutil.rmtree(extract_dir, ignore_errors=True)
            self.refresh_status()

    def open_upgrade_backup_folder(self) -> None:
        UPGRADE_BACKUP_DIR.mkdir(parents=True, exist_ok=True)
        os.startfile(str(UPGRADE_BACKUP_DIR))

    def rollback_release_backup(self) -> None:
        self.save_manager_state()
        backups = list_upgrade_backups()
        if not backups:
            messagebox.showinfo("回滚备份", f"未找到升级备份目录：{UPGRADE_BACKUP_DIR}")
            return

        rollback_window = tk.Toplevel(self.root)
        rollback_window.title("选择升级回滚点")
        rollback_window.geometry("780x420")
        ttk.Label(
            rollback_window,
            text="选择要恢复的升级备份。回滚会停止当前前后端，按备份内容恢复被覆盖文件，并尝试恢复先前运行状态。",
            justify="left",
            wraplength=740,
        ).pack(anchor="w", padx=12, pady=(12, 8))

        listbox = tk.Listbox(rollback_window, height=14, font=("Consolas", 9))
        listbox.pack(fill="both", expand=True, padx=12, pady=8)
        backup_map: List[Path] = []
        for backup_root in backups:
            backup_map.append(backup_root)
            listbox.insert("end", summarize_upgrade_backup(backup_root))

        detail_var = tk.StringVar(value="请选择一个备份目录")
        ttk.Label(rollback_window, textvariable=detail_var, justify="left", wraplength=740).pack(
            anchor="w", padx=12, pady=(0, 8)
        )

        def _update_detail(_event: Any = None) -> None:
            selection = listbox.curselection()
            if not selection:
                detail_var.set("请选择一个备份目录")
                return
            backup_root = backup_map[selection[0]]
            manifest = read_upgrade_backup_manifest(backup_root)
            package_label = str(manifest.get("package_label", "") or backup_root.name)
            action = str(manifest.get("action", "") or "backup")
            created_at = str(manifest.get("created_at", "") or "-")
            detail_var.set(f"目录：{backup_root} | 时间：{created_at} | 来源：{action} | 标签：{package_label}")

        listbox.bind("<<ListboxSelect>>", _update_detail)

        actions = ttk.Frame(rollback_window)
        actions.pack(fill="x", padx=12, pady=(0, 12))
        ttk.Button(
            actions,
            text="执行回滚",
            command=lambda: self.execute_release_rollback(backup_map, listbox, rollback_window),
        ).pack(side="left", padx=(0, 8))
        ttk.Button(actions, text="打开备份目录", command=self.open_upgrade_backup_folder).pack(side="left", padx=(0, 8))
        ttk.Button(actions, text="关闭", command=rollback_window.destroy).pack(side="right")

    def execute_release_rollback(self, backup_map: List[Path], listbox: tk.Listbox, window: tk.Toplevel) -> None:
        selection = listbox.curselection()
        if not selection:
            messagebox.showwarning("回滚备份", "请先选择一个升级备份")
            return

        backup_root = backup_map[selection[0]]
        proceed = messagebox.askyesno(
            "确认回滚",
            f"确定要从以下备份回滚当前项目吗？\n\n{backup_root}\n\n这会停止当前前后端服务，并恢复备份中的旧版本文件。",
        )
        if not proceed:
            append_audit_log("release_rollback", "cancelled", str(backup_root))
            return
        ...
        try:
            ...
            ok, detail = rollback_from_upgrade_backup(backup_root)
            if not ok:
                append_audit_log("release_rollback", "failed", detail, backup_root=str(backup_root))
                messagebox.showerror("回滚失败", detail)
                return

            if backend_was_running:
                backend_ok, backend_msg = self.backend.start(self.get_effective_config())
                restart_messages.append(f"后端：{backend_msg}")
                if not backend_ok:
                    raise RuntimeError(f"回滚后后端恢复失败：{backend_msg}")
            if frontend_was_running:
                frontend_ok, frontend_msg = self.frontend.start(self.get_effective_config())
                restart_messages.append(f"前端：{frontend_msg}")
                if not frontend_ok:
                    raise RuntimeError(f"回滚后前端恢复失败：{frontend_msg}")
            ...
            if restart_messages:
                final_detail += "\n服务恢复：\n" + "\n".join(restart_messages)
            append_audit_log("release_rollback", "success", final_detail, backup_root=str(backup_root))
            self.notify_tray("升级回滚完成", backup_root.name)
            messagebox.showinfo("回滚完成", final_detail)
            window.destroy()
        except Exception as exc:
            recovery_messages: List[str] = []
            if backend_was_running and not self.backend.is_running():
                _, msg = self.backend.start(self.get_effective_config())
                recovery_messages.append(f"后端：{msg}")
            if frontend_was_running and not self.frontend.is_running():
                _, msg = self.frontend.start(self.get_effective_config())
                recovery_messages.append(f"前端：{msg}")
            self.refresh_status()
            detail = str(exc)
    ...
    def backup_database(self) -> None:
        ...
        append_audit_log("database_backup", "success" if ok else "failed", detail, backup_dir=str(backup_dir))
        if ok:
    ...
    def _on_deploy_mode_change(self) -> None:
        """切换部署模式时控制 ZIP 选择区域的显隐"""
        if self.deploy_mode.get() == "zip":
            self.zip_select_frame.pack(fill="x", pady=(0, 8), after=self.deploy_type_frame if hasattr(self, 'deploy_type_frame') else None)
        else:
            self.zip_select_frame.pack_forget()
            self.zip_info_var.set("")

    def preview_release_package(self) -> None:
        """预检更新 ZIP：校验完整性、显示包内目录结构和文件统计"""
        package_path_str = self.ops_vars["release_package_path"].get().strip()
        if not package_path_str:
            self.zip_info_var.set("\u26a0 \u8bf7\u5148\u9009\u62e9更新 ZIP 文件")
            return
        package_path = Path(package_path_str)
        if not package_path.exists() or not package_path.is_file():
            self.zip_info_var.set("\u26a0 文件不存在，请选择有效的 ZIP 文件")
            return
        if package_path.suffix.lower() != ".zip":
            self.zip_info_var.set("\u26a0 文件不是 .zip 格式")
            return
        ...
        try:
            with zipfile.ZipFile(package_path, "r") as archive:
                bad = archive.testzip()
                if bad is not None:
                    self.zip_info_var.set(f"\u274c 文件损坏：{bad}")
                    return

                namelist = archive.namelist()
                file_count = sum(1 for n in namelist if not n.endswith("/"))
                total_size = sum(info.file_size for info in archive.infolist() if not info.is_dir())
                package_size = package_path.stat().st_size
                manifest = None
                manifest_name = next((name for name in namelist if name.endswith("release_manifest.json")), "")
                if manifest_name:
                    try:
                ...
                for name in namelist:
                    ...
                    if len(parts) >= 2:
                        root_dirs.add(parts[-2] if parts[-1] == "" or name.endswith("/") else parts[0])
                    elif len(parts) == 1 and parts[0] and not name.endswith("/"):
                        root_dirs.add("(根文件)")

                has_finflow_prefix = any(n.startswith("FinFlow/") for n in namelist)
                check_dirs = ["backend", "frontend", "deploy"]
                found_dirs = []
                for d in check_dirs:
                    pattern_in_root = f"{d}/"
                    pattern_in_finflow = f"FinFlow/{d}/"
                    if any(n.startswith(pattern_in_root) or n.startswith(pattern_in_finflow) for n in namelist):
                        found_dirs.append(d)

                size_str = f"{total_size / 1024 / 1024:.2f}" if total_size > 1024 * 1024 else f"{total_size / 1024:.1f} KB"
                zip_size_str = f"{package_size / 1024 / 1024:.2f}" if package_size > 1024 * 1024 else f"{package_size / 1024:.1f} KB"

                info_parts = ["\u2705 完整性校验通过"]
                info_parts.append(f"文件数：{file_count}，原始大小：{size_str}，压缩大小：{zip_size_str}")
                if isinstance(manifest, dict):
                    package_type = str(manifest.get("package_type", "") or "").strip()
                    created_at = str(manifest.get("created_at", "") or "").strip()
                    if package_type:
                        info_parts.append(f"类型：{package_type}")
                    if created_at:
                        info_parts.append(f"生成时间：{created_at}")
                if has_finflow_prefix:
                    info_parts.append("包含 FinFlow/ 前缀目录")
                if found_dirs:
                    info_parts.append(f"包含目录：{', '.join(found_dirs)}")
                else:
                    info_parts.append("\u26a0 未检测到 backend/frontend/deploy 等关键目录")

                self.zip_info_var.set("  |  ".join(info_parts))
        except zipfile.BadZipFile:
            self.zip_info_var.set("\u274c 不是有效的 ZIP 文件")
        except Exception as exc:
            self.zip_info_var.set(f"\u274c 预检失败：{exc}")

    def start_one_click_deploy(self) -> None:
        if self.deploy_running:
            append_audit_log("one_click_deploy", "ignored", "deploy already running")
        ...
        if mode == "git" and not repo_url:
            append_audit_log("one_click_deploy", "failed", "missing git repo", mode=mode)
            messagebox.showerror("\u9519\u8bef", "Git \u90e8\u7f72\u6a21\u5f0f\u9700\u8981\u5148\u914d\u7f6e\u4ed3\u5e93\u5730\u5740")
            return
        if mode == "zip":
            pkg = Path(release_package_path)
            if not pkg.exists():
                append_audit_log("one_click_deploy", "failed", "missing zip package", mode=mode)
        ...
        proceed = messagebox.askyesno(
            ...
            "\u4e00\u952e\u90e8\u7f72\u5c06\u81ea\u52a8\u6267\u884c\u4ee5\u4e0b\u6d41\u7a0b\uff1a\n\n"
            "1. \u505c\u6b62\u524d\u540e\u7aef\u670d\u52a1\n"
            "2. \u4ee3\u7801\u90e8\u7f72\uff08Git \u62c9\u53d6 \u6216 ZIP \u89e3\u538b\u8986\u76d6\uff09\n"
            "3. \u73af\u5883\u68c0\u67e5\u4e0e\u51c6\u5907\uff08Python/venv/Node/sqlcmd\uff09\n"
            "4. \u8fd0\u884c\u914d\u7f6e\u5199\u5165\n"
            "5. \u4f9d\u8d56\u540c\u6b65\u3001\u524d\u7aef\u6784\u5efa\u4e0e\u8fd0\u884c\u914d\u7f6e\u540c\u6b65\n"
            "6. \u90e8\u7f72\u540e\u5065\u5eb7\u4e0e\u540e\u7f6e\u68c0\u67e5\n"
        ...
        if not proceed:
            append_audit_log("one_click_deploy", "cancelled", f"mode={mode}")
            return

        self.root.config(cursor="watch")
        self.deploy_running = True
        append_audit_log("one_click_deploy", "started", f"mode={mode}", mode=mode)
    ...
    def _run_one_click_deploy(
        ...
        def save_runtime_config(runtime_config: Dict[str, str]) -> None:
            payload, existed = save_backend_env_config(runtime_config, self.extra_env_values)
            self.config_values = dict(payload)
            if existed:
                self.append_deploy_log("  [INFO] 检测到已有 backend/.env，已在保留现有扩展项的基础上同步更新")
            else:
                self.append_deploy_log("  [INFO] 未检测到 backend/.env，已根据模板和当前配置自动创建")
        ...
        def abort_deploy(reason: str) -> None:
            ...
            append_audit_log("one_click_deploy", "failed", detail, mode=mode)
        ...
        if mode == "git":
            if not (ROOT_DIR / ".git").exists():
                ...
                try:
                    ...
                    for item_name in ("backend", "frontend", "tools", "deploy"):
                        ...
                        if src.exists():
                            overlay_directory(src, ROOT_DIR / item_name, backup_root, change_log)
                    write_upgrade_backup_manifest(backup_root, "one_click_deploy_git_clone", repo_url, change_log)
            ...
            try:
                ...
                if (release_root / "backend").exists():
                    overlay_directory(release_root / "backend", ROOT_DIR / "backend", backup_root, change_log)
                if (release_root / "frontend").exists():
                    overlay_directory(release_root / "frontend", ROOT_DIR / "frontend", backup_root, change_log)
                if (release_root / "tools").exists():
                    overlay_directory(release_root / "tools", ROOT_DIR / "tools", backup_root, change_log)
                if (release_root / "deploy").exists():
                    overlay_directory(release_root / "deploy", ROOT_DIR / "deploy", backup_root, change_log)
                write_upgrade_backup_manifest(backup_root, "one_click_deploy_zip", package_path.name, change_log)

                self.append_deploy_log("  [OK] ZIP \u53d1\u5e03\u5305\u5df2\u89e3\u538b\u5e76\u5e94\u7528")
            finally:
                shutil.rmtree(extract_dir, ignore_errors=True)

        steps_passed += 1

        self.append_deploy_log(f"[4/{total_steps}] \u8fd0\u884c\u914d\u7f6e\u5199\u5165\u3001\u524d\u7aef\u6784\u5efa\u4e0e\u8fd0\u884c\u914d\u7f6e\u540c\u6b65...")
        ...
        append_audit_log("one_click_deploy", "success", "\n".join(deploy_lines), mode=mode, title=deploy_title)
        if deploy_title == "\u4e00\u952e\u90e8\u7f72\u5b8c\u6210":
...
def _managed_backend_poll_status(self: Any) -> str:
    ...
    if host_alive and not backend_alive:
        return f"\u4e3b\u63a7\u8fdb\u7a0b\u8fd0\u884c\u4e2d (PID {host_pid})\uff0c\u7b49\u5f85\u540e\u7aef\u5c31\u7eea"
    if backend_alive and not host_alive:
...
def _ff_build_status_tab(self: Any, parent: ttk.Frame) -> None:
    ...
    status_items = {
        ...
        "manager_runtime_mode": self.get_manager_runtime_mode_text(),
        "startup_mode": self.get_startup_mode_text(),
        "startup_detail": self.get_startup_detail_text(),
        "startup_last_result": self.get_startup_last_result_text(),
        "startup_last_error": self.get_startup_last_error_text(),
        "host_diagnostics": self.get_host_diagnostics_text(),
        "audit_status": self.get_audit_status_text(),
    ...
    overview_fields = [
        ...
        ("\u81ea\u542f\u65b9\u5f0f", "startup_mode", 3, 0, 180),
        ("\u7ba1\u7406\u6a21\u5f0f", "manager_runtime_mode", 3, 2, 180),
        ("\u81ea\u542f\u8be6\u60c5", "startup_detail", 4, 0, 420),
        ("\u4e0a\u6b21\u81ea\u542f\u7ed3\u679c", "startup_last_result", 5, 0, 420),
        ("\u4e0a\u6b21\u9519\u8bef\u6458\u8981", "startup_last_error", 6, 0, 420),
        ("\u540e\u53f0\u5bbf\u4e3b", "host_status", 7, 0, 420),
        ("Host诊断", "host_diagnostics", 8, 0, 760),
        ("最近审计", "audit_status", 9, 0, 760),
    ]
    ...
    for title, fields, row, column in sections:
        ...
        if fields:
            ...
            action_groups = [
                ...
                ("\u7ba1\u7406\u5668", [("\u542f\u52a8\u540e\u53f0\u5bbf\u4e3b", self.handle_start_host), ("\u505c\u6b62\u540e\u53f0\u5bbf\u4e3b", self.handle_stop_host), ("\u91cd\u542f\u540e\u53f0\u5bbf\u4e3b", self.handle_restart_host), ("\u6253\u5f00\u5bbf\u4e3b\u65e5\u5fd7", self.open_host_log), ("\u542f\u7528\u5f00\u673a\u81ea\u542f", self.enable_startup_now), ("\u5173\u95ed\u5f00\u673a\u81ea\u542f", self.disable_startup_now), ("\u91cd\u5efa\u5f00\u673a\u81ea\u542f", self.rebuild_startup_now), ("\u67e5\u770b\u81ea\u542f\u8be6\u60c5", self.show_startup_detail), ("\u6253\u5f00\u7ba1\u7406\u5668", self.show_window)]),
            ]
...
def _ff_refresh_status(self: Any) -> None:
    ...
    self.status_vars["host_status"].set(self.get_host_status_text())
    self.status_vars["manager_runtime_mode"].set(self.get_manager_runtime_mode_text())
    self.status_vars["startup_mode"].set(self.get_startup_mode_text())
    self.status_vars["startup_detail"].set(self.get_startup_detail_text())
    self.status_vars["startup_last_result"].set(self.get_startup_last_result_text())
    self.status_vars["startup_last_error"].set(self.get_startup_last_error_text())
    self.status_vars["host_diagnostics"].set(self.get_host_diagnostics_text())
    self.status_vars["audit_status"].set(self.get_audit_status_text())
    if "host_status" in self.status_vars:
        self.status_vars["host_status"].set(self.get_host_status_text())
    if "manager_runtime_mode" in self.status_vars:
        self.status_vars["manager_runtime_mode"].set(self.get_manager_runtime_mode_text())
    if "startup_mode" in self.status_vars:
        self.status_vars["startup_mode"].set(self.get_startup_mode_text())
    if "startup_detail" in self.status_vars:
        self.status_vars["startup_detail"].set(self.get_startup_detail_text())
    if "startup_last_result" in self.status_vars:
        self.status_vars["startup_last_result"].set(self.get_startup_last_result_text())
    if "startup_last_error" in self.status_vars:
        self.status_vars["startup_last_error"].set(self.get_startup_last_error_text())
    if "host_diagnostics" in self.status_vars:
        self.status_vars["host_diagnostics"].set(self.get_host_diagnostics_text())
    if "audit_status" in self.status_vars:
        self.status_vars["audit_status"].set(self.get_audit_status_text())
...
def _ff_refresh_health_status(self: Any) -> None:
    ...
    active_jobs = getattr(self, "_background_refresh_jobs", None)
    if active_jobs is None:
        active_jobs = {}
        self._background_refresh_jobs = active_jobs
    if active_jobs.get("health"):
        self.schedule_health_refresh()
        return

    active_jobs["health"] = True
    self.status_vars["backend_health_status"].set("\u68c0\u67e5\u4e2d...")
    self.status_vars["frontend_health_status"].set("\u68c0\u67e5\u4e2d...")

    backend_url = self.get_backend_url()
    frontend_url = self.get_frontend_url()
    backend_running = self.backend.is_running()
    frontend_running = self.frontend.is_running()
    webhook_url = self.manager_state.get("webhook_url", "")
    ...
    def worker() -> Dict[str, Any]:
        ...
        if backend_running:
            backend_ok, backend_detail = probe_backend_health(backend_url)
        ...
        if frontend_running:
            frontend_ok, frontend_detail = probe_http(frontend_url)
    ...
    def finalize(result: Dict[str, Any] | None = None, error: Exception | None = None) -> None:
        active_jobs["health"] = False
        if self.exiting:
            return
        if error is not None:
            detail = str(error)
            self.status_vars["backend_health_status"].set(f"\u5f02\u5e38 ({detail})")
            self.status_vars["frontend_health_status"].set(f"\u5f02\u5e38 ({detail})")
            self.refresh_status_badges()
            self.schedule_health_refresh()
            return

        assert result is not None
        backend_ok = bool(result["backend_ok"])
        backend_detail = str(result["backend_detail"])
        backend_state = str(result["backend_state"])
        self.status_vars["backend_health_status"].set(
            f"\u6b63\u5e38 ({backend_detail})" if backend_ok else f"\u5f02\u5e38 ({backend_detail})" if backend_state == "unhealthy" else "\u672a\u68c0\u67e5"
        )
        if backend_running and self.last_backend_health_state not in {"unknown", backend_state}:
            if backend_ok:
                message = f"{backend_url} \u5df2\u6062\u590d\u6b63\u5e38\u54cd\u5e94"
                self.notify_tray("\u540e\u7aef\u5065\u5eb7\u68c0\u67e5\u5df2\u6062\u590d", message)
                send_webhook_notification(webhook_url, "\u540e\u7aef\u5065\u5eb7\u68c0\u67e5\u5df2\u6062\u590d", message)
            else:
                message = f"{backend_url} \u5065\u5eb7\u68c0\u67e5\u5f02\u5e38: {backend_detail}"
        ...
        frontend_ok = bool(result["frontend_ok"])
        frontend_detail = str(result["frontend_detail"])
        frontend_state = str(result["frontend_state"])
        self.status_vars["frontend_health_status"].set(
            f"\u6b63\u5e38 ({frontend_detail})" if frontend_ok else f"\u5f02\u5e38 ({frontend_detail})" if frontend_state == "unhealthy" else "\u672a\u68c0\u67e5"
        )
        if frontend_running and self.last_frontend_health_state not in {"unknown", frontend_state}:
            if frontend_ok:
                message = f"{frontend_url} \u5df2\u6062\u590d\u6b63\u5e38\u54cd\u5e94"
                self.notify_tray("\u524d\u7aef\u5065\u5eb7\u68c0\u67e5\u5df2\u6062\u590d", message)
                send_webhook_notification(webhook_url, "\u524d\u7aef\u5065\u5eb7\u68c0\u67e5\u5df2\u6062\u590d", message)
            else:
                message = f"{frontend_url} \u5065\u5eb7\u68c0\u67e5\u5f02\u5e38: {frontend_detail}"
    ...
    threading.Thread(target=run_worker, name="finflow-health-refresh", daemon=True).start()
...
def _ff_refresh_database_status(self: Any, force_check: bool = False, show_dialog: bool = False, sqlcmd_override: str | None = None, schedule_next: bool = True) -> Tuple[str, str]:
    ...
    if active_jobs is None:
        active_jobs = {}
        self._background_refresh_jobs = active_jobs
    if active_jobs.get("database"):
        if show_dialog:
            messagebox.showinfo("\u6570\u636e\u5e93\u8fde\u63a5\u68c0\u67e5", "\u6570\u636e\u5e93\u68c0\u67e5\u6b63\u5728\u8fdb\u884c\u4e2d\uff0c\u8bf7\u7a0d\u5019\u67e5\u770b\u7ed3\u679c\u3002")
        if schedule_next:
            self.schedule_db_refresh()
        return "running", "\u6570\u636e\u5e93\u68c0\u67e5\u6b63\u5728\u8fdb\u884c\u4e2d"

    sqlcmd = (sqlcmd_override if sqlcmd_override is not None else self.ops_vars["sqlcmd_path"].get().strip() or "sqlcmd").strip()
    active_jobs["database"] = True
    self.status_vars["db_connection_status"].set("\u68c0\u67e5\u4e2d...")
    self.status_vars["db_monitor_status"].set("\u624b\u52a8\u68c0\u67e5\u4e2d" if force_check else "\u81ea\u52a8\u68c0\u67e5\u4e2d")

    def worker() -> Dict[str, str]:
        state, detail = evaluate_database_runtime_status(sqlcmd)
        checked_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        return {"state": state, "detail": detail, "checked_at": checked_at}

    def finalize(result: Dict[str, str] | None = None, error: Exception | None = None) -> None:
        active_jobs["database"] = False
        if self.exiting:
            return
    ...
    threading.Thread(target=run_worker, name="finflow-db-refresh", daemon=True).start()
    return "running", "\u6570\u636e\u5e93\u68c0\u67e5\u5df2\u5728\u540e\u53f0\u542f\u52a8"
...
def _ff_build_manager_tab(self: Any, parent: ttk.Frame) -> None:
    ...
    startup_actions = ttk.Frame(options)
    startup_actions.grid(row=(len(option_items) + 1) // 2, column=0, columnspan=2, sticky="w", padx=10, pady=(10, 0))
    ttk.Button(startup_actions, text="\u542f\u52a8\u540e\u53f0\u5bbf\u4e3b", command=self.handle_start_host).pack(side="left", padx=(0, 8))
    ttk.Button(startup_actions, text="\u505c\u6b62\u540e\u53f0\u5bbf\u4e3b", command=self.handle_stop_host).pack(side="left", padx=(0, 8))
    ttk.Button(startup_actions, text="\u91cd\u542f\u540e\u53f0\u5bbf\u4e3b", command=self.handle_restart_host).pack(side="left", padx=(0, 8))
    ttk.Button(startup_actions, text="\u6253\u5f00\u5bbf\u4e3b\u65e5\u5fd7", command=self.open_host_log).pack(side="left", padx=(0, 8))
    ttk.Button(startup_actions, text="打开审计日志", command=self.open_audit_log).pack(side="left", padx=(0, 8))
    ttk.Label(startup_actions, textvariable=self.status_vars["host_status"]).pack(side="left", padx=(8, 12))

    startup_actions_2 = ttk.Frame(options)
    startup_actions_2.grid(row=(len(option_items) + 3) // 2, column=0, columnspan=2, sticky="w", padx=10, pady=(8, 0))
    ttk.Button(startup_actions_2, text="\u7acb\u5373\u542f\u7528\u5f00\u673a\u81ea\u542f", command=self.enable_startup_now).pack(side="left", padx=(0, 8))
    ttk.Button(startup_actions_2, text="\u7acb\u5373\u5173\u95ed\u5f00\u673a\u81ea\u542f", command=self.disable_startup_now).pack(side="left", padx=(0, 8))
    ttk.Button(startup_actions_2, text="\u91cd\u5efa\u81ea\u542f\u914d\u7f6e", command=self.rebuild_startup_now).pack(side="left", padx=(0, 8))
    ttk.Button(startup_actions_2, text="\u67e5\u770b\u81ea\u542f\u8be6\u60c5", command=self.show_startup_detail).pack(side="left", padx=(0, 8))
    ttk.Label(startup_actions_2, textvariable=self.status_vars["startup_status"]).pack(side="left", padx=(8, 0))

    diagnostics = ttk.LabelFrame(parent, text="宿主诊断与审计", padding=16)
    diagnostics.pack(fill="x", padx=10, pady=(0, 10))
    ttk.Label(diagnostics, text="宿主诊断：", width=14).grid(row=0, column=0, sticky="nw", padx=6, pady=4)
    ttk.Label(diagnostics, textvariable=self.status_vars["host_diagnostics"], justify="left", wraplength=860).grid(row=0, column=1, sticky="w", padx=6, pady=4)
    ttk.Label(diagnostics, text="最近审计：", width=14).grid(row=1, column=0, sticky="nw", padx=6, pady=4)
    ttk.Label(diagnostics, textvariable=self.status_vars["audit_status"], justify="left", wraplength=860).grid(row=1, column=1, sticky="w", padx=6, pady=4)
...
def _ff_build_ops_tab(self: Any, parent: ttk.Frame) -> None:
    ...
    ops_nav_items = [
        ("takeover", "首装接管", "检查服务器运行条件，并提供首装与接管阶段的常用修复入口"),
        ("one_click_deploy", "\u4e00\u952e\u90e8\u7f72", "\u7edf\u4e00\u5b8c\u6210\u914d\u7f6e\u5199\u5165\u3001\u4f9d\u8d56\u540c\u6b65\u3001\u6784\u5efa\u4e0e\u540e\u7f6e\u68c0\u67e5"),
        ("frontend", "\u524d\u7aef\u90e8\u7f72", "\u5355\u72ec\u90e8\u7f72 frontend/dist \u5230\u670d\u52a1\u73af\u5883"),
        ("release", "更新 ZIP", "生成或应用更新 ZIP，按差异更新后端与前端 dist"),
    ...
    takeover_frame = ttk.LabelFrame(self.ops_content_host, text="首装 / 环境接管向导", padding=16)
    ttk.Label(
        takeover_frame,
        text="用于服务器首次部署、打包 EXE 初次落地或已有环境接管。这里会汇总关键条件，并给出最常用的修复按钮。",
        justify="left",
        wraplength=760,
    ).pack(anchor="w", pady=(0, 10))
    takeover_actions = ttk.Frame(takeover_frame)
    takeover_actions.pack(fill="x", pady=(0, 10))
    ttk.Button(takeover_actions, text="重新检查", command=self.refresh_takeover_guide).pack(side="left", padx=4)
    ttk.Button(takeover_actions, text="同步后端依赖", command=self.sync_backend_deps).pack(side="left", padx=4)
    ttk.Button(takeover_actions, text="同步前端依赖", command=self.sync_frontend_deps).pack(side="left", padx=4)
    ttk.Button(takeover_actions, text="生成加密密钥", command=self.generate_encryption_key).pack(side="left", padx=4)
    ttk.Button(takeover_actions, text="启用开机自启", command=self.enable_startup_now).pack(side="left", padx=4)
    ttk.Button(takeover_actions, text="启动后台宿主", command=self.handle_start_host).pack(side="left", padx=4)

    takeover_actions_2 = ttk.Frame(takeover_frame)
    takeover_actions_2.pack(fill="x", pady=(0, 10))
    ttk.Button(takeover_actions_2, text="接管后端端口", command=self.handle_takeover_backend).pack(side="left", padx=4)
    ttk.Button(takeover_actions_2, text="释放后端端口", command=self.handle_force_release_port).pack(side="left", padx=4)
    ttk.Button(takeover_actions_2, text="检查数据库", command=self.check_database_connection).pack(side="left", padx=4)
    ttk.Button(takeover_actions_2, text="打开日志目录", command=self.open_logs_folder).pack(side="left", padx=4)
    ttk.Button(takeover_actions_2, text="打开审计日志", command=self.open_audit_log).pack(side="left", padx=4)

    self.takeover_status_var = tk.StringVar(value="尚未检查")
    ttk.Label(takeover_frame, textvariable=self.takeover_status_var, foreground="#4b5f73").pack(anchor="w", pady=(0, 8))

    self.takeover_summary_text = scrolledtext.ScrolledText(takeover_frame, wrap="word", font=("Consolas", 10), height=20)
    self.takeover_summary_text.pack(fill="both", expand=True)
    self.takeover_summary_text.configure(state="disabled")
    self.ops_section_frames["takeover"] = takeover_frame
    ...
    ttk.Radiobutton(deploy_type_frame, text="Git \u66f4\u65b0\u90e8\u7f72", variable=self.deploy_mode, value="git", command=self._on_deploy_mode_change).pack(side="left", padx=20)
    ttk.Radiobutton(deploy_type_frame, text="ZIP \u53d1\u5e03\u5305\u90e8\u7f72", variable=self.deploy_mode, value="zip", command=self._on_deploy_mode_change).pack(side="left", padx=20)

    # ZIP 文件选择区域（仅 ZIP 模式可见）
    self.zip_select_frame = ttk.LabelFrame(one_click_frame, text="ZIP \u53d1\u5e03\u5305", padding=8)
    zip_path_row = ttk.Frame(self.zip_select_frame)
    zip_path_row.pack(fill="x", pady=(0, 4))
    ttk.Label(zip_path_row, text="\u53d1\u5e03\u5305\u8def\u5f84\uff1a", width=10).pack(side="left", padx=(0, 4))
    self.zip_package_entry = ttk.Entry(zip_path_row, textvariable=self.ops_vars["release_package_path"], width=68)
    self.zip_package_entry.pack(side="left", fill="x", expand=True, padx=(0, 4))
    ttk.Button(zip_path_row, text="\u9009\u62e9\u6587\u4ef6", command=lambda: self.select_path_for_var("release_package_path", "zip")).pack(side="left", padx=(0, 4))
    ttk.Button(zip_path_row, text="\u9884\u68c0\u53d1\u5e03\u5305", command=self.preview_release_package).pack(side="left")

    self.zip_info_var = tk.StringVar(value="")
    ttk.Label(self.zip_select_frame, textvariable=self.zip_info_var, foreground="#4b5f73", justify="left", wraplength=720).pack(anchor="w", pady=(0, 2))
    # 初始隐藏（Git 模式下不显示）
    # _on_deploy_mode_change 会控制显隐
    ...
    path_fields = [
        ...
        ("release_package_path", "更新 ZIP", "zip"),
    ...
    release_frame = ttk.LabelFrame(self.ops_content_host, text="更新 ZIP", padding=16)
    ttk.Label(release_frame, text="开发机可生成轻量更新 ZIP，只包含 backend 必要文件、frontend/dist 和必要运行脚本。服务器端应用时会先备份，再按差异定向替换。", justify="left", wraplength=760).pack(anchor="w", pady=(0, 8))
    release_path_row = ttk.Frame(release_frame)
    release_path_row.pack(fill="x", pady=(0, 8))
    ttk.Label(release_path_row, text="当前 ZIP：", width=10).pack(side="left", padx=(0, 4))
    ttk.Entry(release_path_row, textvariable=self.ops_vars["release_package_path"], width=88).pack(side="left", fill="x", expand=True, padx=(0, 4))
    release_actions = ttk.Frame(release_frame)
    release_actions.pack(fill="x")
    ttk.Button(release_actions, text="生成更新 ZIP", command=self.build_update_package).pack(side="left", padx=4)
    ttk.Button(release_actions, text="\u9009\u62e9更新 ZIP", command=lambda: self.select_path_for_var("release_package_path", "zip")).pack(side="left", padx=4)
    ttk.Button(release_actions, text="预检更新 ZIP", command=self.preview_release_package).pack(side="left", padx=4)
    ttk.Button(release_actions, text="应用更新 ZIP", command=self.apply_release_package).pack(side="left", padx=4)
    ttk.Button(release_actions, text="回滚到升级备份", command=self.rollback_release_backup).pack(side="left", padx=4)
    ttk.Button(release_actions, text="打开升级备份目录", command=self.open_upgrade_backup_folder).pack(side="left", padx=4)
    ttk.Button(release_actions, text="\u6253\u5f00\u9879\u76ee\u76ee\u5f55", command=self.open_project_root).pack(side="left", padx=4)
    ttk.Label(release_frame, textvariable=self.zip_info_var, foreground="#4b5f73", justify="left", wraplength=760).pack(anchor="w", pady=(8, 0))
    ...
    ttk.Button(maint_actions, text="打开审计日志", command=self.open_audit_log).pack(side="left", padx=4)
...
def _ff_refresh_log_view(self: Any, force: bool = False) -> None:
    ...
    if path.exists():
        try:
            raw_bytes = read_file_tail_bytes(path, max_bytes=524288)
...
def _ff_git_pull_update(self: Any) -> None:
    self.save_manager_state()
    if not self.check_git_available():
        append_audit_log("git_pull_update", "failed", "git unavailable")
        messagebox.showerror("\u9519\u8bef", "\u672a\u68c0\u6d4b\u5230 Git\uff0c\u8bf7\u5148\u5b89\u88c5 Git \u5e76\u52a0\u5165\u7cfb\u7edf PATH")
        return

    repo_url = self.ops_vars["git_repo_url"].get().strip()
    if not repo_url:
        append_audit_log("git_pull_update", "failed", "missing repo url")
        messagebox.showerror("\u9519\u8bef", "\u8bf7\u5148\u914d\u7f6e Git \u4ed3\u5e93\u5730\u5740")
        return
    ...
    if not (ROOT_DIR / ".git").exists():
        ...
        if not proceed:
            append_audit_log("git_pull_update", "cancelled", "init repo declined", branch=branch)
            return
        try:
            ...
            append_audit_log("git_pull_update", "failed", f"git clone exception: {exc}", branch=branch)
            messagebox.showerror("\u5931\u8d25", f"Git clone \u5931\u8d25\uff1a{exc}")
            return
        if clone_result.returncode != 0:
            detail = (clone_result.stderr or clone_result.stdout or "\u672a\u77e5\u9519\u8bef").strip()
            append_audit_log("git_pull_update", "failed", detail, branch=branch)
            messagebox.showerror("\u5931\u8d25", f"Git clone \u5931\u8d25\uff1a\n{detail}")
            return
        post_ok, post_detail = self.run_git_post_update_tasks()
        if not post_ok:
            append_audit_log("git_pull_update", "failed", post_detail, branch=branch)
        ...
        if self.manager_state.get("git_auto_build_frontend", True):
            detail += f"\n\n{post_detail}"
        append_audit_log("git_pull_update", "success", detail, branch=branch)
    ...
    try:
        ...
        if "Already up to date" in stdout_text:
            append_audit_log("git_pull_update", "noop", stdout_text or "already up to date", branch=branch)
        ...
        append_audit_log("git_pull_update", "failed", str(exc), branch=branch)
...
def _ff_restore_database(self: Any) -> None:
    ...
    bak_files = sorted(backup_dir.glob("*.bak"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not bak_files:
        append_audit_log("database_restore", "failed", f"no backup files in {backup_dir}")
    ...
    def do_restore() -> None:
        ...
        if not proceed:
            append_audit_log("database_restore", "cancelled", selected_file.name)
            return
        ok, detail = restore_database_from_backup(self.get_effective_config(), selected_file, sqlcmd)
        choice_window.destroy()
        append_audit_log("database_restore", "success" if ok else "failed", detail or selected_file.name, backup_file=str(selected_file))
        if ok:
...
def _handle_start_all_override(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    ...
    if show_dialog:
        ...
        dialog("启动结果", message)
    append_audit_log("services_start_all", "success" if (backend_started and frontend_ok) else "failed", message)
...
def _backend_process_start_verified(self: BackendProcessController, config: Dict[str, str]) -> Tuple[bool, str]:
    ...
    while time.time() < deadline:
        ...
        if exit_code is not None:
            if port.isdigit():
                port_num = int(port)
                port_ready = is_port_open(browser_host, port_num) or (host == "0.0.0.0" and is_port_open("127.0.0.1", port_num))
                if port_ready:
                    owner = get_port_owner_info(port_num) or {}
                    owner_pid = int(owner.get("pid") or 0)
                    if owner_pid > 0:
                        self.process = SimpleNamespace(pid=owner_pid)
                    return True, f"后端已启动，PID={safe_process_pid(self.process)}，监听 {browser_host}:{port}"
            log_tail = read_log_since_marker(STDERR_LOG, self.last_session_marker) or read_log_since_marker(STDOUT_LOG, self.last_session_marker)
...
def _frontend_process_start_verified(self: FrontendProcessController, config: Dict[str, str]) -> Tuple[bool, str]:
    ...
    # 使用 DETACHED_PROCESS 标志确保前端进程在独立的会话中运行
    # 这样在RDP断开时不会被终止
    # 使用 shell 启动方式，通过重定向符号将输出写入日志文件
    creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
    ...
    # 构建命令行，使用 shell 重定向而不是传递文件句柄
    if use_vite_dev_server:
        cmd_str = f'"{node_exe}" "{vite_cli}" --host 0.0.0.0 --port {frontend_port} >> "{FRONTEND_STDOUT_LOG}" 2>> "{FRONTEND_STDERR_LOG}"'
    else:
        # 对于非 vite dev server 的情况，构建命令行
        launcher = resolve_frontend_host_launcher()
        cmd_str = subprocess.list2cmdline(launcher) + f' >> "{FRONTEND_STDOUT_LOG}" 2>> "{FRONTEND_STDERR_LOG}"'
...
def _managed_backend_start_sessioned(self: ManagedBackendController, _config: Dict[str, str] | None = None) -> Tuple[bool, str]:
    ...
    if os.name == "nt":
        try:
            invalidate_windows_service_info_cache()
            service_info = query_windows_service_info(force_refresh=True)
            if service_info.get("running"):
                probe = _wait_for_managed_backend_ready(timeout_seconds=2.0)
                if probe.get("backend_alive") or probe.get("port_open") or probe.get("http_ok"):
                    service_pid = int(service_info.get("pid") or 0)
                    self.process = SimpleNamespace(pid=service_pid) if service_pid > 0 else None
                    return True, f"Windows service is already running (service PID {service_pid}, backend ready)"
                create_backend_start_request()
                probe = _wait_for_managed_backend_ready(timeout_seconds=12.0)
                if probe.get("backend_alive") or probe.get("port_open") or probe.get("http_ok"):
                    service_pid = int(service_info.get("pid") or 0)
                    self.process = SimpleNamespace(pid=service_pid) if service_pid > 0 else None
                    return True, f"Windows service accepted backend start request (service PID {service_pid})"
                clear_backend_start_request()
                ok, message = restart_windows_service()
                if ok:
                    running_info = query_windows_service_info(force_refresh=True)
                    service_pid = int(running_info.get("pid") or 0)
                    self.process = SimpleNamespace(pid=service_pid) if service_pid > 0 else None
                    return True, message
                return False, f"Windows 服务已运行，但后端未就绪，自动修复失败：{message}"
            auto_start = bool(read_state_file(STATE_PATH).get("launch_manager_on_startup", False))
            install_ok, install_message = install_windows_service(auto_start=auto_start)
            if install_ok:
                service_ok, service_message = start_windows_service()
                if service_ok:
                    running_info = query_windows_service_info(force_refresh=True)
                    service_pid = int(running_info.get("pid") or 0)
                    self.process = SimpleNamespace(pid=service_pid) if service_pid > 0 else None
                ...
                service_detail = install_message
        except Exception as exc:
            service_detail = f"Windows 服务启动异常：{exc}"
        if service_detail:
            append_text_log(SERVICE_HOST_LOG, f"[manager] Windows service unavailable, falling back to session host: {service_detail}")
    ...
    launcher = resolve_service_host_launcher()
    creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
    launcher_target = Path(launcher[0]).resolve() if launcher else None
    current_exe = Path(sys.executable).resolve()
    frozen_self_launch = bool(getattr(sys, "frozen", False) and launcher_target == current_exe)
    if not frozen_self_launch:
        for flag_name in ("DETACHED_PROCESS", "CREATE_NEW_PROCESS_GROUP"):
            creationflags |= getattr(subprocess, flag_name, 0)

    child_env = os.environ.copy()
    child_env["PYTHONWARNINGS"] = "ignore"
    if frozen_self_launch:
    ...
    with open(SERVICE_HOST_LOG, "a", encoding="utf-8", errors="ignore") as service_log:
        service_log.write(f"\n{session_marker}\n")
        service_log.write(f"launcher={launcher}\n")
        service_log.write(f"creationflags={creationflags}\n")
        service_log.write(f"frozen_self_launch={frozen_self_launch}\n")
        service_log.flush()
        process = subprocess.Popen(
            launcher,
            cwd=ROOT_DIR,
...
def _managed_backend_stop_service_first(self: ManagedBackendController) -> Tuple[bool, str]:
    service_messages: List[str] = []
    if os.name == "nt":
        try:
            service_info = query_windows_service_info(force_refresh=True)
        except Exception as exc:
            service_info = {"installed": False, "running": False}
            service_messages.append(f"查询 Windows 服务状态失败：{exc}")
        if service_info.get("running"):
            ok, message = stop_windows_service()
            ...
            service_messages.append("Windows 服务已停止")
    ...
    clear_stop_request()
    if service_messages:
        return True, "\n".join(service_messages + ["已发送停止命令，正在结束后台宿主"])
    return True, "已发送停止命令，正在结束后台宿主"


def _managed_backend_restart_service_first(self: ManagedBackendController, config: Dict[str, str] | None = None) -> Tuple[bool, str]:
    if os.name == "nt":
        try:
            auto_start = bool(read_state_file(STATE_PATH).get("launch_manager_on_startup", False))
            install_ok, install_message = install_windows_service(auto_start=auto_start)
            if install_ok:
                ...
                append_text_log(SERVICE_HOST_LOG, f"[manager] Windows service install failed, falling back to session host: {install_message}")
        except Exception as exc:
            append_text_log(SERVICE_HOST_LOG, f"[manager] Windows service restart raised exception, falling back to session host: {exc}")
...
def _managed_backend_poll_status_service_aware(self: ManagedBackendController) -> str:
    probe = _get_backend_runtime_probe()
    state = probe.get("runtime_state") or {}
    backend_pid = int(probe.get("backend_pid") or 0)
    host_pid = int(state.get("service_host_pid") or 0)

    backend_alive = bool(probe.get("backend_alive"))
    host_alive = bool(state.get("service_host_alive")) or (host_pid > 0 and is_process_alive(host_pid))
    ...
    if os.name == "nt":
        try:
            service_info = query_windows_service_info()
            service_running = bool(service_info.get("running"))
            service_pid = int(service_info.get("pid") or 0)
        except Exception:
            service_running = False
            service_pid = 0
    ...
    port_text = str(probe.get("port_text") or "8100")
    port_open = bool(probe.get("port_open"))
    http_ok = bool(probe.get("http_ok"))
    owner_label = ""
    if port_text.isdigit():
        port_num = int(port_text)
        try:
            owner = get_port_owner_info(port_num)
            if owner:
                owner_label = build_port_owner_label(owner)
                owner_pid = int(owner.get("pid") or 0)
                if owner_pid > 0 and owner_pid == backend_pid:
                    backend_alive = True
        except Exception:
            pass

    if backend_alive or (service_running and backend_pid > 0) or http_ok:
        if port_text.isdigit() and (port_open or http_ok):
            return f"运行中 ({host}:{port_text})"
        if backend_pid > 0:
            return f"运行中 (PID {backend_pid})"
        if service_running and service_pid > 0:
            return "运行中 (服务托管)"
        return "运行中"

    if backend_pid > 0:
        return f"启动中 (PID {backend_pid})"

    if service_running and not backend_alive and not port_open and not http_ok:
        if owner_label:
            return "未运行 (后端端口被其他进程占用)"
        return "未运行 (服务宿主未托起后端)"

    if service_running or host_alive:
        if service_running:
            return "启动中 (服务托管)"
        if host_alive and host_pid > 0:
            return "启动中 (宿主运行中)"
        if owner_label:
            return "启动中 (端口已占用)"
        return "启动中"
...
def _frontend_poll_status_service_aware(self: FrontendProcessController) -> str:
    state = read_managed_runtime_state()
    frontend_pid = int(state.get("frontend_pid") or 0)
    host_pid = int(state.get("service_host_pid") or 0)

    frontend_alive = bool(state.get("frontend_alive")) or (frontend_pid > 0 and is_process_alive(frontend_pid))
    host_alive = bool(state.get("service_host_alive")) or (host_pid > 0 and is_process_alive(host_pid))
    ...
    if frontend_port.isdigit():
        ...
        try:
            ...
            if owner:
                owner_label = build_port_owner_label(owner)
                owner_pid = int(owner.get("pid") or 0)
                if owner_pid > 0 and owner_pid == frontend_pid:
                    frontend_alive = True
            port_open = is_port_open(frontend_host, port_num) or is_port_open("127.0.0.1", port_num)
            http_ok = can_connect_http(frontend_host, port_num) or can_connect_http("127.0.0.1", port_num)
        except Exception:
            port_open = False
            http_ok = False

    managed_by_host = service_running or host_alive

    if frontend_alive or (service_running and frontend_pid > 0) or http_ok:
        if frontend_port.isdigit() and (port_open or http_ok):
            mode = "宿主托管" if managed_by_host else "GUI直启"
            return f"运行中 ({mode} {frontend_host}:{frontend_port})"
        if frontend_pid > 0:
            mode = "宿主托管" if managed_by_host else "GUI直启"
            return f"运行中 ({mode} PID {frontend_pid})"
        if service_running and service_pid > 0:
            return "运行中 (宿主托管-服务模式)"
        if host_alive and host_pid > 0:
            return "运行中 (宿主托管-会话宿主)"
        return "运行中 (GUI直启)"

    if service_running or host_alive or frontend_pid > 0:
        if frontend_pid > 0:
            mode = "宿主托管" if managed_by_host else "GUI直启"
            return f"启动中 ({mode} PID {frontend_pid})"
        if service_running:
            return "启动中 (宿主托管-服务模式)"
        if host_alive and host_pid > 0:
            return "启动中 (宿主托管-会话宿主)"
        if owner_label:
            return "启动中 (端口已占用)"
        return "启动中"
...
def _ff_refresh_status_service_aware(self: Any) -> None:
    backend_status = self.backend.poll_status()
    frontend_status = self.frontend.poll_status()
    config = self.get_effective_config()
    backend_port = (config.get("APP_PORT") or "8100").strip() or "8100"
    ...
    if backend_port.isdigit():
        owner = get_port_owner_info(int(backend_port))
        if owner:
            backend_owner_text = _format_port_owner_summary(owner)
        else:
            backend_owner_text = "空闲"
    if frontend_port.isdigit():
        owner = get_port_owner_info(int(frontend_port))
        if owner:
            frontend_owner_text = _format_port_owner_summary(owner)
        else:
            frontend_owner_text = "空闲"
...
def _ff_build_status_tab_compact(self: Any, parent: ttk.Frame) -> None:
    db_config_state, db_config_detail = describe_database_configuration_from_disk()
    self.status_badges = {}
    self.status_nav_items = {}
    self.status_section_frames = {}
    ...
    shell = ttk.Frame(parent, padding=(10, 10, 10, 10))
    shell.grid(row=0, column=0, sticky="nsew")
    shell.columnconfigure(1, weight=1)
    shell.rowconfigure(0, weight=1)

    nav_wrap = ttk.Frame(shell, width=240)
    nav_wrap.grid(row=0, column=0, sticky="nsw", padx=(0, 12))
    nav_wrap.grid_propagate(False)
    ttk.Label(nav_wrap, text="服务状态", font=("Microsoft YaHei UI", 11, "bold")).pack(anchor="w")
    ttk.Label(
        nav_wrap,
        text="按主题查看运行状态、部署环境和可执行操作，避免所有信息堆叠在一个页面。",
        justify="left",
        foreground="#666666",
        wraplength=210,
    ).pack(anchor="w", pady=(4, 10))

    self.status_section_var = tk.StringVar(value="overview")
    status_nav = [
        ("overview", "运行概览", "查看项目入口、运行模式、启动状态与关键路径。"),
        ("services", "前后端服务", "分别查看后端与前端的运行、端口、健康检查与访问地址。"),
        ("database", "数据库状态", "查看数据库配置、连接检查、监控状态与最近检查时间。"),
        ("manager", "宿主管理", "查看宿主进程、自启方式、最近结果、诊断与审计信息。"),
        ("actions", "快捷操作", "集中执行启动、停止、重启、接管、日志和托盘操作。"),
    ]
    self.create_side_nav(nav_wrap, status_nav, self.status_section_var, self.show_status_section, self.status_nav_items)
    ...
    def create_section(title: str, description: str) -> Tuple[ttk.Frame, ttk.Frame]:
        frame = ttk.Frame(content_wrap)
        frame.columnconfigure(0, weight=1)
        ttk.Label(frame, text=title, font=("Microsoft YaHei UI", 12, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Label(frame, text=description, foreground="#666666", justify="left").grid(
    ...
    def add_field_group(container: ttk.Frame, title: str, fields: List[Tuple[str, str]], row: int, column: int = 0) -> ttk.LabelFrame:
        group = ttk.LabelFrame(container, text=title, padding=12)
        group.grid(row=row, column=column, sticky="nsew", padx=6, pady=6)
        group.columnconfigure(1, weight=1)
        for field_row, (label_text, key) in enumerate(fields):
            ttk.Label(group, text=f"{label_text}：").grid(row=field_row, column=0, sticky="nw", padx=(0, 8), pady=4)
            if key in {"backend_status", "frontend_status", "backend_health_status", "frontend_health_status", "db_connection_status", "db_monitor_status"}:
                badge = tk.Label(
                    group,
                ...
                wrap_length = 780 if key in {"host_diagnostics", "audit_status"} else 520 if key in {"startup_detail", "startup_last_result", "startup_last_error", "project_root"} else 360
                ttk.Label(group, textvariable=self.status_vars[key], justify="left", wraplength=wrap_length).grid(
                    row=field_row, column=1, sticky="w", pady=4
                )
        return group

    def add_action_group(container: ttk.Frame, title: str, actions: List[Tuple[str, Any]], row: int) -> None:
        group = ttk.LabelFrame(container, text=title, padding=10)
        group.grid(row=row, column=0, sticky="ew", pady=6)
        for action_col in range(3):
            group.columnconfigure(action_col, weight=1)
        for idx, (button_text, command) in enumerate(actions):
            button = ttk.Button(group, text=button_text, command=command)
    ...
    overview_frame, overview_body = create_section("运行概览", "优先呈现最常看的环境入口与整体运行状态。")
    self.status_section_frames["overview"] = overview_frame
    overview_body.columnconfigure(0, weight=1)
    overview_body.columnconfigure(1, weight=1)
    add_field_group(
        overview_body,
        "部署入口",
        [("项目目录", "project_root"), ("应用入口", "app_url"), ("环境文件", "env_file"), ("密钥文件", "key_file"), ("前端构建", "dist_dir")],
        0,
        0,
    )
    add_field_group(
        overview_body,
        "启动状态",
        [("启动状态", "startup_status"), ("自启方式", "startup_mode"), ("管理模式", "manager_runtime_mode"), ("自启详情", "startup_detail")],
        0,
        1,
    )
    add_field_group(
        overview_body,
        "最近结果",
        [("上次自启结果", "startup_last_result"), ("上次错误摘要", "startup_last_error")],
        1,
        0,
    )
    add_field_group(
        overview_body,
        "宿主摘要",
        [("后台宿主", "host_status"), ("Host诊断", "host_diagnostics"), ("最近审计", "audit_status")],
        1,
        1,
    )

    services_frame, services_body = create_section("前后端服务", "前后端服务拆开展示，便于快速判断是哪一侧出现问题。")
    self.status_section_frames["services"] = services_frame
    services_body.columnconfigure(0, weight=1)
    services_body.columnconfigure(1, weight=1)
    add_field_group(
        services_body,
        "后端服务",
        [("运行状态", "backend_status"), ("端口占用", "backend_port_owner"), ("健康检查", "backend_health_status"), ("访问地址", "backend_url")],
        0,
        0,
    )
    add_field_group(
        services_body,
        "前端服务",
        [("运行状态", "frontend_status"), ("端口占用", "frontend_port_owner"), ("健康检查", "frontend_health_status"), ("访问地址", "frontend_url")],
        0,
        1,
    )

    database_frame, database_body = create_section("数据库状态", "将配置、连接与监控状态独立出来，减少和服务状态互相干扰。")
    self.status_section_frames["database"] = database_frame
    database_body.columnconfigure(0, weight=1)
    add_field_group(
        database_body,
        "数据库检查",
        [("配置状态", "db_config_status"), ("连接状态", "db_connection_status"), ("监控状态", "db_monitor_status"), ("上次检查", "db_last_check_at")],
        0,
        0,
    )

    manager_frame, manager_body = create_section("宿主管理", "聚焦管理器宿主、自启策略与诊断信息，方便排查 EXE 托管场景。")
    self.status_section_frames["manager"] = manager_frame
    manager_body.columnconfigure(0, weight=1)
    manager_body.columnconfigure(1, weight=1)
    add_field_group(
        manager_body,
        "宿主与模式",
        [("后台宿主", "host_status"), ("管理模式", "manager_runtime_mode"), ("自启方式", "startup_mode"), ("自启状态", "startup_status")],
        0,
        0,
    )
    add_field_group(
        manager_body,
        "自启结果",
        [("自启详情", "startup_detail"), ("上次自启结果", "startup_last_result"), ("上次错误摘要", "startup_last_error")],
        0,
        1,
    )
    add_field_group(
        manager_body,
        "诊断与审计",
        [("Host诊断", "host_diagnostics"), ("最近审计", "audit_status")],
        1,
        0,
    )

    actions_frame, actions_body = create_section("快捷操作", "把常用运维动作集中到单独区域，减少状态信息与操作按钮混排造成的拥挤。")
    self.status_section_frames["actions"] = actions_frame
    actions_body.columnconfigure(0, weight=1)
    add_action_group(
        actions_body,
        "全部服务",
        [("启动全部", self.handle_start_all), ("停止全部", self.handle_stop_all), ("重启全部", self.handle_restart_all), ("打开前端", self.open_frontend), ("打开日志目录", self.open_logs_folder), ("最小化到托盘", self.hide_to_tray)],
        0,
    )
    add_action_group(
        actions_body,
        "后端服务",
        [("启动后端", self.handle_start_backend), ("停止后端", self.handle_stop_backend), ("重启后端", self.handle_restart_backend), ("接管后端", self.handle_takeover_backend), ("释放端口", self.handle_force_release_port)],
        1,
    )
    add_action_group(
        actions_body,
        "前端与数据库",
        [("启动前端", self.handle_start_frontend), ("停止前端", self.handle_stop_frontend), ("重启前端", self.handle_restart_frontend), ("检查数据库", self.check_database_connection)],
        2,
    )
    add_action_group(
        actions_body,
        "管理器与自启",
        [("启动后台宿主", self.handle_start_host), ("停止后台宿主", self.handle_stop_host), ("重启后台宿主", self.handle_restart_host), ("打开宿主日志", self.open_host_log), ("启用开机自启", self.enable_startup_now), ("关闭开机自启", self.disable_startup_now), ("重建开机自启", self.rebuild_startup_now), ("查看自启详情", self.show_startup_detail), ("打开管理器", self.show_window)],
        3,
    )
...
ManagedBackendController.stop = _managed_backend_stop_service_first
ManagedBackendController.restart = _managed_backend_restart_service_first
ManagedBackendController.poll_status = _managed_backend_poll_status_service_aware
FrontendProcessController.poll_status = _frontend_poll_status_service_aware
...
_ff_build_status_tab = _ff_build_status_tab_compact
_ff_refresh_status = _ff_refresh_status_service_aware


def _ff_build_status_tab_compact_v2(self: Any, parent: ttk.Frame) -> None:
    db_config_state, db_config_detail = describe_database_configuration_from_disk()
    self.status_badges = {}
    self.status_nav_items = {}
    self.status_section_frames = {}
    ...
    ttk.Label(
        nav_wrap,
        text="按主题查看状态、宿主信息和快捷操作，把首屏重点信息从密集明细里抽离出来。",
        justify="left",
        foreground="#666666",
        wraplength=210,
    ).pack(anchor="w", pady=(4, 10))
    ...
    self.status_section_var = tk.StringVar(value="overview")
    status_nav = [
        ("overview", "运行概览", "优先看摘要卡片、访问入口和最近运行结果。"),
        ("services", "前后端服务", "分别查看前后端运行、端口、健康检查和访问地址。"),
        ("database", "数据库状态", "查看数据库配置、连接状态、监控状态和检查时间。"),
        ("manager", "宿主管理", "查看宿主进程、自启方式、诊断和审计摘要。"),
        ("actions", "快捷操作", "按常用、维护、高风险进行分层，降低误操作。"),
    ]
    self.create_side_nav(nav_wrap, status_nav, self.status_section_var, self.show_status_section, self.status_nav_items)
    ...
    def add_summary_card(container: ttk.Frame, title: str, key: str, sub_key: str, row: int, column: int) -> None:
        bg, fg = self.get_status_badge_palette(key, self.status_vars[key].get())
        card = tk.Frame(container, bg=bg, bd=1, relief="solid", padx=14, pady=12, highlightthickness=0)
        ...
        tk.Label(
            card,
            textvariable=self.status_vars[sub_key],
            bg=bg,
            fg="#4b5563",
            font=("Microsoft YaHei UI", 8),
            anchor="w",
    ...
    def add_action_group(
        ...
        intro = tk.Frame(group, bg=accent, padx=10, pady=8)
        ...
        for action_col in range(3):
            buttons.columnconfigure(action_col, weight=1)
        for idx, (button_text, command) in enumerate(actions):
            button = ttk.Button(buttons, text=button_text, command=command)
            button.grid(row=idx // 3, column=idx % 3, sticky="ew", padx=4, pady=4)
            action_key = {
                "启动全部": "start_all",
                "停止全部": "stop_all",
    ...
    overview_frame, overview_body = create_section("运行概览", "先展示影响决策的核心状态，再往下看部署路径与最近运行结果。")
    self.status_section_frames["overview"] = overview_frame
    overview_body.columnconfigure(0, weight=1)
    overview_body.columnconfigure(1, weight=1)

    summary_cards = ttk.Frame(overview_body)
    summary_cards.grid(row=0, column=0, columnspan=2, sticky="ew", pady=(0, 8))
    for idx in range(4):
        summary_cards.columnconfigure(idx, weight=1)
    add_summary_card(summary_cards, "后端状态", "backend_status", "backend_url", 0, 0)
    add_summary_card(summary_cards, "前端状态", "frontend_status", "frontend_url", 0, 1)
    add_summary_card(summary_cards, "数据库连接", "db_connection_status", "db_last_check_at", 0, 2)
    add_summary_card(summary_cards, "宿主状态", "host_status", "startup_status", 0, 3)
    ...
    services_frame, services_body = create_section("前后端服务", "前端与后端拆开查看，定位问题会更直接。")
    self.status_section_frames["services"] = services_frame
    services_body.columnconfigure(0, weight=1)
    services_body.columnconfigure(1, weight=1)
    add_field_group(
    ...
    database_frame, database_body = create_section("数据库状态", "数据库相关信息独立展示，避免和服务状态互相挤占。")
    self.status_section_frames["database"] = database_frame
    database_body.columnconfigure(0, weight=1)
    add_field_group(
        database_body,
    ...
    manager_frame, manager_body = create_section("宿主管理", "这里聚焦 EXE 托管、自启动和宿主诊断信息。")
    self.status_section_frames["manager"] = manager_frame
    manager_body.columnconfigure(0, weight=1)
    manager_body.columnconfigure(1, weight=1)
    add_field_group(
    ...
    actions_frame, actions_body = create_section("快捷操作", "先放高频动作，再放维护动作，最后把会影响运行状态的操作单独隔离。")
    self.status_section_frames["actions"] = actions_frame
    actions_body.columnconfigure(0, weight=1)
    add_action_group(
        actions_body,
        "常用操作",
        "日常启动、停止、查看入口和日志，适合最频繁使用的运维动作。",
        [("启动全部", self.handle_start_all), ("停止全部", self.handle_stop_all), ("重启全部", self.handle_restart_all), ("打开前端", self.open_frontend), ("打开日志目录", self.open_logs_folder), ("最小化到托盘", self.hide_to_tray)],
        0,
        "#2563eb",
    )
    add_action_group(
        actions_body,
        "维护操作",
        "针对单个组件做维护或查看宿主运行情况，适合排查问题时逐项处理。",
        [("启动后端", self.handle_start_backend), ("停止后端", self.handle_stop_backend), ("重启后端", self.handle_restart_backend), ("启动前端", self.handle_start_frontend), ("停止前端", self.handle_stop_frontend), ("重启前端", self.handle_restart_frontend), ("检查数据库", self.check_database_connection), ("启动后台宿主", self.handle_start_host), ("停止后台宿主", self.handle_stop_host), ("重启后台宿主", self.handle_restart_host), ("打开宿主日志", self.open_host_log), ("查看自启详情", self.show_startup_detail), ("打开管理器", self.show_window)],
        1,
        "#0f766e",
    )
    add_action_group(
        actions_body,
        "高风险操作",
        "这些动作可能接管端口、改变启动方式或影响现有进程，建议确认环境后再执行。",
        [("接管后端", self.handle_takeover_backend), ("释放端口", self.handle_force_release_port), ("启用开机自启", self.enable_startup_now), ("关闭开机自启", self.disable_startup_now), ("重建开机自启", self.rebuild_startup_now)],
        2,
        "#b42318",
    )
...
def _frontend_process_start_production_only(self: FrontendProcessController, config: Dict[str, str]) -> Tuple[bool, str]:
    ...
    if os.name == "nt":
        host_probe = _get_frontend_runtime_probe()
        runtime_state = host_probe.get("runtime_state") or {}
        host_pid = int(host_probe.get("host_pid") or 0)
        host_alive = bool(host_probe.get("host_alive"))
        running_inside_active_host = host_pid > 0 and host_pid == os.getpid()
        service_running = False
        service_pid = 0
        try:
            service_info = query_windows_service_info(force_refresh=True)
        ...
        if (service_running or host_alive) and not running_inside_active_host:
            host_state = _wait_for_runtime_host_ready(expected_service_pid=service_pid if service_running else 0, timeout_seconds=12.0)
            host_pid = int(host_state.get("service_host_pid") or host_pid)
            create_frontend_start_request()
            probe = _wait_for_managed_frontend_ready(timeout_seconds=12.0)
            frontend_pid = int(probe.get("frontend_pid") or 0)
            if probe.get("frontend_alive") or probe.get("port_open") or probe.get("http_ok"):
                probe_state = probe.get("runtime_state") or {}
                owner_pid = frontend_pid or int(probe_state.get("frontend_pid") or 0)
                fragments: List[str] = []
                if service_running and service_pid > 0:
                    fragments.append(f"service PID {service_pid}")
                elif host_pid > 0:
                    fragments.append(f"host PID {host_pid}")
                if owner_pid > 0:
                    fragments.append(f"frontend PID {owner_pid}")
                detail = f"，{'，'.join(fragments)}" if fragments else ""
                self.process = SimpleNamespace(pid=owner_pid) if owner_pid > 0 else None
                self.user_stopped = False
                self.last_start_attempt = time.time()
                self.last_started_at = self.last_start_attempt
                self.last_exit_code = None
                self.auto_restart_suppressed = False
                self.last_exit_at = float(probe_state.get("frontend_last_exit_at", self.last_exit_at) or 0.0)
                self.consecutive_failed_starts = int(probe_state.get("frontend_consecutive_failed_starts", 0) or 0)
                self.last_session_marker = str(probe_state.get("frontend_last_session_marker", "") or "")
                self.last_session_started_label = str(probe_state.get("frontend_last_session_started_label", "") or "")
                self._sync_runtime_state(frontend_pid=owner_pid)
                return True, f"前端静态服务已启动（宿主托管）{detail}，访问地址 {probe.get('frontend_url') or settings['frontend_url']}"

            clear_frontend_start_request()
            runtime_snapshot = json.dumps(probe.get("runtime_state") or host_state or runtime_state, ensure_ascii=False, indent=2)
            return False, "Windows 服务/后台宿主已运行，但前端启动请求未成功完成。\n" + runtime_snapshot
    ...
    launcher = resolve_frontend_host_launcher()
    launcher_target = Path(launcher[0]).resolve() if launcher else None
    if getattr(sys, "frozen", False) and launcher_target == Path(sys.executable).resolve():
    ...
    while time.time() < deadline:
        ...
        if frontend_port.isdigit() and is_port_open(frontend_host, int(frontend_port)):
            self._sync_runtime_state()
            return True, f"前端静态服务已启动（GUI直启） PID={safe_process_pid(self.process)} 地址 {settings['frontend_url']}"
        time.sleep(0.25)

    self._sync_runtime_state()
    return True, f"前端静态服务已启动（GUI直启） PID={safe_process_pid(self.process)}，等待端口稳定：{settings['frontend_url']}"
...
def _handle_stop_frontend_production_override(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    host_probe = _get_frontend_runtime_probe() if os.name == "nt" else {}
    host_alive = bool(host_probe.get("host_alive"))
    service_running = False
    service_pid = 0
    if os.name == "nt":
    ...
    if not self.frontend.is_running() and not host_alive and not service_running:
        info_msg = "前端静态服务未运行"
        self.log_status_var.set(info_msg)
        if show_dialog:
            messagebox.showinfo("服务状态", info_msg)
    ...
    if service_running or host_alive:
        create_frontend_stop_request()
        probe = _wait_for_managed_frontend_stopped(timeout_seconds=12.0)
        if not probe.get("frontend_alive") and not probe.get("port_open") and not probe.get("http_ok"):
            fragments: List[str] = []
            if service_running and service_pid > 0:
                fragments.append(f"service PID {service_pid}")
            elif host_alive:
            ...
            self.frontend.user_stopped = True
            self.frontend.capture_exit()
            ok, message = True, f"前端静态服务已停止（宿主托管）{detail}"
        else:
            clear_frontend_stop_request()
            ok, message = False, "后台宿主已收到前端停止请求，但前端仍未停止"
    else:
        ok, message = self.frontend.stop()
...
def _handle_restart_frontend_production_override(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    ...
    if service_running or host_alive:
        create_frontend_restart_request()
        probe = _wait_for_managed_frontend_ready(timeout_seconds=15.0)
        if probe.get("frontend_alive") or probe.get("port_open") or probe.get("http_ok"):
            frontend_pid = int(probe.get("frontend_pid") or 0)
            probe_state = probe.get("runtime_state") or {}
            self.frontend.process = SimpleNamespace(pid=frontend_pid) if frontend_pid > 0 else None
            self.frontend.user_stopped = False
            self.frontend.last_start_attempt = time.time()
            self.frontend.last_started_at = self.frontend.last_start_attempt
            self.frontend.last_exit_code = None
            self.frontend.auto_restart_suppressed = False
            self.frontend.last_exit_at = float(probe_state.get("frontend_last_exit_at", self.frontend.last_exit_at) or 0.0)
            self.frontend.consecutive_failed_starts = int(probe_state.get("frontend_consecutive_failed_starts", 0) or 0)
            self.frontend.last_session_marker = str(probe_state.get("frontend_last_session_marker", "") or "")
            self.frontend.last_session_started_label = str(probe_state.get("frontend_last_session_started_label", "") or "")
            self.frontend._sync_runtime_state(frontend_pid=frontend_pid)
            fragments: List[str] = []
            if service_running and service_pid > 0:
                fragments.append(f"service PID {service_pid}")
            elif host_alive:
                ...
                if host_pid > 0:
                    fragments.append(f"host PID {host_pid}")
            if frontend_pid > 0:
                fragments.append(f"frontend PID {frontend_pid}")
            detail = f"（{'，'.join(fragments)}）" if fragments else ""
            ok, message = True, f"前端静态服务已重启（宿主托管）{detail}，访问地址 {probe.get('frontend_url') or self.get_frontend_url()}"
        else:
            clear_frontend_restart_request()
            ok, message = False, "后台宿主已收到前端重启请求，但前端未能重新就绪"
    else:
        ok, message = self.frontend.restart(config)
...
WINDOWS_SERVICE_NAME = "FinFlowManagerHost"
WINDOWS_SERVICE_DISPLAY_NAME = "FinFlow Manager Host"
WINDOWS_SERVICE_DESCRIPTION = "FinFlow production runtime host service"


def resolve_windows_service_launcher() -> List[str]:
    if getattr(sys, "frozen", False):
        return [str(resolve_frozen_worker_executable()), "--windows-service"]
    ...
    if python_exe.name.lower() == "pythonw.exe":
        ...
        if candidate.exists():
            python_exe = candidate
    return [str(python_exe), str(TOOLS_DIR / "finflow_manager_host.py"), "--windows-service"]


def _run_sc_command(arguments: List[str], timeout: int = 20) -> subprocess.CompletedProcess:
    return _run_windows_command(["sc", *arguments], timeout=timeout)


def _decode_sc_output(result: subprocess.CompletedProcess) -> str:
    return sanitize_console_text(
        decode_console_output(result.stdout or b"") or decode_console_output(result.stderr or b"")
    ).strip()


def _parse_sc_service_state(text: str) -> str:
    match = re.search(r"STATE\s*:\s*\d+\s+([A-Z_]+)", text, flags=re.IGNORECASE)
    return (match.group(1).upper() if match else "").strip()


def _parse_sc_service_pid(text: str) -> int:
    match = re.search(r"PID\s*:\s*(\d+)", text, flags=re.IGNORECASE)
    return int(match.group(1)) if match else 0


def _parse_sc_start_type(text: str) -> str:
    match = re.search(r"START_TYPE\s*:\s*\d+\s+([A-Z_]+)", text, flags=re.IGNORECASE)
    return (match.group(1).upper() if match else "").strip()


def _parse_sc_binary_path(text: str) -> str:
    match = re.search(r"BINARY_PATH_NAME\s*:\s*(.+)", text, flags=re.IGNORECASE)
    return (match.group(1).strip() if match else "")


WINDOWS_SERVICE_INFO_CACHE_TTL_SECONDS = 5.0
_windows_service_info_cache: Dict[str, Any] = {
    "expires_at": 0.0,
    "info": None,
}


def invalidate_windows_service_info_cache() -> None:
    _windows_service_info_cache["expires_at"] = 0.0
    _windows_service_info_cache["info"] = None


def query_windows_service_info(force_refresh: bool = False, timeout_seconds: int = 3) -> Dict[str, Any]:
    now = time.time()
    cached = _windows_service_info_cache.get("info")
    expires_at = float(_windows_service_info_cache.get("expires_at") or 0.0)
    if not force_refresh and isinstance(cached, dict) and expires_at > now:
        return dict(cached)
    ...
    if os.name != "nt":
        info["detail"] = "Windows service control is only supported on Windows"
        return info

    try:
        query_result = _run_sc_command(["queryex", WINDOWS_SERVICE_NAME], timeout=timeout_seconds)
    except Exception as exc:
        info["detail"] = str(exc)
    else:
        query_text = _decode_sc_output(query_result)
        lowered = query_text.lower()
        if query_result.returncode != 0 and "does not exist" in lowered:
            info["detail"] = query_text or "service not installed"
        elif query_result.returncode != 0 and "specified service does not exist" in lowered:
            info["detail"] = query_text or "service not installed"
        elif query_result.returncode != 0:
            info["detail"] = query_text or f"sc queryex exited with code {query_result.returncode}"
        else:
            info["installed"] = True
            info["state"] = _parse_sc_service_state(query_text)
            info["running"] = info["state"] == "RUNNING"
            info["pid"] = _parse_sc_service_pid(query_text)
            info["detail"] = query_text

            try:
                qc_result = _run_sc_command(["qc", WINDOWS_SERVICE_NAME], timeout=timeout_seconds)
            except Exception:
                qc_result = None

            if qc_result is not None:
                qc_text = _decode_sc_output(qc_result)
                if qc_result.returncode == 0:
                    info["start_type"] = _parse_sc_start_type(qc_text)
                    info["binary_path"] = _parse_sc_binary_path(qc_text)
                    info["detail"] = (query_text + "\n\n" + qc_text).strip()

    _windows_service_info_cache["info"] = dict(info)
    _windows_service_info_cache["expires_at"] = now + WINDOWS_SERVICE_INFO_CACHE_TTL_SECONDS
    return dict(info)


def windows_service_exists() -> bool:
    return bool(query_windows_service_info(force_refresh=True).get("installed"))


def _wait_for_windows_service_state(target_states: set[str], timeout_seconds: float = 25.0) -> Dict[str, Any]:
    deadline = time.time() + timeout_seconds
    last_info = query_windows_service_info(force_refresh=True)
    while time.time() < deadline:
        last_info = query_windows_service_info(force_refresh=True)
        if str(last_info.get("state") or "").upper() in target_states:
            return last_info
        time.sleep(0.5)
    return last_info


def set_windows_service_start_mode(auto_start: bool) -> Tuple[bool, str]:
    if not windows_service_exists():
        return False, "Windows service is not installed"
    start_mode = "auto" if auto_start else "demand"
    try:
        result = _run_sc_command(["config", WINDOWS_SERVICE_NAME, "start=", start_mode], timeout=20)
    except Exception as exc:
        return False, str(exc)
    detail = _decode_sc_output(result)
    if result.returncode == 0:
        invalidate_windows_service_info_cache()
        return True, f"Windows service start mode set to {'automatic' if auto_start else 'manual'}"
    return False, detail or f"sc config exited with code {result.returncode}"


def install_windows_service(auto_start: bool = True) -> Tuple[bool, str]:
    launcher = resolve_windows_service_launcher()
    launcher_path = Path(launcher[0])
    if not launcher_path.exists():
        return False, f"Host service launcher was not found: {launcher_path}"

    if windows_service_exists():
        mode_ok, mode_detail = set_windows_service_start_mode(auto_start)
        return mode_ok, mode_detail

    start_mode = "auto" if auto_start else "demand"
    commandline = subprocess.list2cmdline(launcher)
    try:
        result = _run_sc_command(
            [
                "create",
    ...
    detail = _decode_sc_output(result)
    if result.returncode != 0:
        return False, detail or f"sc create exited with code {result.returncode}"

    try:
        _run_sc_command(["description", WINDOWS_SERVICE_NAME, WINDOWS_SERVICE_DESCRIPTION], timeout=15)
    except Exception:
        pass
    try:
        _run_sc_command(
            ["failure", WINDOWS_SERVICE_NAME, "reset=", "86400", "actions=", "restart/60000/restart/60000/restart/60000"],
            timeout=20,
        )
        _run_sc_command(["failureflag", WINDOWS_SERVICE_NAME, "1"], timeout=15)
    except Exception:
        pass
    invalidate_windows_service_info_cache()
    return True, f"Windows service installed ({'automatic' if auto_start else 'manual'}, LocalSystem account)"


def remove_windows_service() -> Tuple[bool, str]:
    if not windows_service_exists():
        return True, "Windows service is not installed"
    service_info = query_windows_service_info(force_refresh=True)
    if service_info.get("running"):
        stop_windows_service()
    try:
        result = _run_sc_command(["delete", WINDOWS_SERVICE_NAME], timeout=20)
    except Exception as exc:
        return False, str(exc)
    detail = _decode_sc_output(result)
    if result.returncode == 0:
        invalidate_windows_service_info_cache()
        return True, "Windows service deleted"
    return False, detail or f"sc delete exited with code {result.returncode}"


def _get_backend_runtime_probe() -> Dict[str, Any]:
    runtime_state = read_managed_runtime_state()
    backend_pid = int(runtime_state.get("backend_pid") or 0)
    backend_alive = bool(runtime_state.get("backend_alive")) or (backend_pid > 0 and is_process_alive(backend_pid))
    config = load_effective_config_from_disk()
    host = resolve_browser_host(config.get("APP_HOST", "127.0.0.1"))
    port_text = (config.get("APP_PORT") or "8100").strip() or "8100"
    port_open = False
    http_ok = False
    if port_text.isdigit():
        port_num = int(port_text)
        try:
            port_open = is_port_open(host, port_num) or is_port_open("127.0.0.1", port_num)
            http_ok = can_connect_http(host, port_num) or can_connect_http("127.0.0.1", port_num)
            if not backend_alive:
                owner = get_port_owner_info(port_num)
                owner_pid = int((owner or {}).get("pid") or 0)
                if owner_pid > 0 and owner_pid == backend_pid:
                    backend_alive = True
        except Exception:
            port_open = False
...
def _wait_for_managed_backend_ready(timeout_seconds: float = 15.0) -> Dict[str, Any]:
    deadline = time.time() + max(0.5, timeout_seconds)
    last_probe: Dict[str, Any] = _get_backend_runtime_probe()
    while time.time() < deadline:
        last_probe = _get_backend_runtime_probe()
        if last_probe.get("backend_alive") or last_probe.get("port_open") or last_probe.get("http_ok"):
            return last_probe
        time.sleep(0.5)
    return last_probe


def _get_frontend_runtime_probe() -> Dict[str, Any]:
    runtime_state = read_managed_runtime_state()
    frontend_pid = int(runtime_state.get("frontend_pid") or 0)
    frontend_alive = bool(runtime_state.get("frontend_alive")) or (frontend_pid > 0 and is_process_alive(frontend_pid))
    host_pid = int(runtime_state.get("service_host_pid") or 0)
    host_alive = bool(runtime_state.get("service_host_alive")) or (host_pid > 0 and is_process_alive(host_pid))
    config = load_effective_config_from_disk()
    settings = resolve_frontend_service_settings(config)
    frontend_host = settings["frontend_host"]
    frontend_port = settings["frontend_port"]
...
def _wait_for_managed_frontend_ready(timeout_seconds: float = 15.0) -> Dict[str, Any]:
    deadline = time.time() + max(0.5, timeout_seconds)
    last_probe: Dict[str, Any] = _get_frontend_runtime_probe()
    while time.time() < deadline:
        last_probe = _get_frontend_runtime_probe()
        if last_probe.get("frontend_alive") or last_probe.get("port_open") or last_probe.get("http_ok"):
            return last_probe
        time.sleep(0.5)
    return last_probe


def _wait_for_managed_frontend_stopped(timeout_seconds: float = 15.0) -> Dict[str, Any]:
    deadline = time.time() + max(0.5, timeout_seconds)
    last_probe: Dict[str, Any] = _get_frontend_runtime_probe()
    while time.time() < deadline:
        last_probe = _get_frontend_runtime_probe()
        if not last_probe.get("frontend_alive") and not last_probe.get("port_open") and not last_probe.get("http_ok"):
            return last_probe
        time.sleep(0.5)
    return last_probe


def _wait_for_runtime_host_ready(expected_service_pid: int = 0, timeout_seconds: float = 12.0) -> Dict[str, Any]:
    deadline = time.time() + max(0.5, timeout_seconds)
    last_state: Dict[str, Any] = read_managed_runtime_state()
    while time.time() < deadline:
        last_state = read_managed_runtime_state()
        host_pid = int(last_state.get("service_host_pid") or 0)
        host_transition = str(last_state.get("host_last_transition") or "").strip().lower()
        heartbeat = float(last_state.get("host_last_heartbeat_at") or 0.0)
        pid_ready = host_pid > 0 and (expected_service_pid <= 0 or host_pid == expected_service_pid)
        transition_ready = host_transition in {"starting", "running"}
        heartbeat_ready = heartbeat > 0
        if pid_ready and transition_ready and heartbeat_ready:
            return last_state
        time.sleep(0.25)
    return last_state


def start_windows_service() -> Tuple[bool, str]:
    if not windows_service_exists():
        install_ok, install_detail = install_windows_service(auto_start=False)
        if not install_ok:
            return False, install_detail
    invalidate_windows_service_info_cache()
    service_info = query_windows_service_info(force_refresh=True)
    service_already_running = bool(service_info.get("running"))
    detail = ""
    if not service_already_running:
        try:
            result = _run_sc_command(["start", WINDOWS_SERVICE_NAME], timeout=20)
        except Exception as exc:
            return False, str(exc)
        detail = _decode_sc_output(result)
        if result.returncode != 0 and "already running" not in detail.lower():
            return False, detail or f"sc start exited with code {result.returncode}"
    waited = _wait_for_windows_service_state({"RUNNING"}, timeout_seconds=30.0)
    if waited.get("running"):
        probe = _wait_for_managed_backend_ready(timeout_seconds=15.0)
        if not (probe.get("backend_alive") or probe.get("port_open") or probe.get("http_ok")):
            create_backend_start_request()
            probe = _wait_for_managed_backend_ready(timeout_seconds=12.0)
            if not (probe.get("backend_alive") or probe.get("port_open") or probe.get("http_ok")):
                clear_backend_start_request()
        runtime_state = probe.get("runtime_state") or {}
        backend_pid = int(probe.get("backend_pid") or 0)
        frontend_pid = int(runtime_state.get("frontend_pid") or 0)
        if probe.get("backend_alive") or probe.get("port_open") or probe.get("http_ok"):
            fragments = [f"service PID {int(waited.get('pid') or 0)}"]
            if backend_pid > 0:
                fragments.append(f"backend {backend_pid}")
            if frontend_pid > 0:
                fragments.append(f"frontend {frontend_pid}")
            prefix = "Windows service is already running" if service_already_running else "Windows service started"
            return True, prefix + " (" + ", ".join(fragments) + ")"
        runtime_snapshot = build_runtime_state_snapshot_text()
        prefix = "Windows service is running, but backend is not ready" if service_already_running else "Windows service started, but backend is not ready"
        host = str(probe.get("host") or "127.0.0.1")
        port_text = str(probe.get("port_text") or "8100")
        return False, f"{prefix} ({host}:{port_text})\n{runtime_snapshot}"
    runtime_snapshot = build_runtime_state_snapshot_text()
    return False, (detail or "Windows service did not report RUNNING in time") + f"\n{runtime_snapshot}"


def stop_windows_service() -> Tuple[bool, str]:
    invalidate_windows_service_info_cache()
    service_info = query_windows_service_info(force_refresh=True)
    if not service_info.get("installed"):
        return False, "Windows service is not installed"
    if not service_info.get("running"):
        return True, "Windows service is already stopped"
    create_stop_request()
    try:
        result = _run_sc_command(["stop", WINDOWS_SERVICE_NAME], timeout=20)
    except Exception as exc:
        return False, str(exc)
    detail = _decode_sc_output(result)
    if result.returncode != 0 and "service has not been started" not in detail.lower():
        return False, detail or f"sc stop exited with code {result.returncode}"
    waited = _wait_for_windows_service_state({"STOPPED"}, timeout_seconds=30.0)
    if str(waited.get("state") or "").upper() == "STOPPED":
        clear_stop_request()
        return True, "Windows service stopped"
    return False, detail or "Windows service did not stop in time"


def restart_windows_service() -> Tuple[bool, str]:
    stop_ok, stop_detail = stop_windows_service()
    if not stop_ok and "already stopped" not in stop_detail.lower():
...
def create_startup_task() -> Tuple[bool, str]:
    return install_windows_service(auto_start=True)
...
def get_startup_mode_label() -> str:
    ...
    start_type = str(service_info.get("start_type") or "").upper()
    if start_type == "AUTO_START":
        return "Windows Service (Automatic)"
    if start_type == "DEMAND_START":
        return "Windows Service (Manual)"
...
def get_startup_task_detail() -> str:
    service_info = query_windows_service_info()
    if not service_info.get("installed"):
        launcher = subprocess.list2cmdline(resolve_windows_service_launcher())
        return f"Windows service is not installed.\nPlanned command: {launcher}"
    detail_lines = [
        f"Service Name: {WINDOWS_SERVICE_NAME}",
        f"Display Name: {WINDOWS_SERVICE_DISPLAY_NAME}",
        f"State: {service_info.get('state') or '-'}",
        f"Start Type: {service_info.get('start_type') or '-'}",
        f"PID: {int(service_info.get('pid') or 0)}",
        f"Binary: {service_info.get('binary_path') or '-'}",
    ]
    return "\n".join(detail_lines)


def _service_start_type_label(start_type: str) -> str:
    mapping = {
        "AUTO_START": "自动",
        "DEMAND_START": "手动",
        "DISABLED": "禁用",
    }
    return mapping.get(str(start_type or "").upper(), str(start_type or "-") or "-")


def _service_state_label(state: str) -> str:
    mapping = {
        "RUNNING": "运行中",
        "STOPPED": "未运行",
        "START_PENDING": "启动中",
    ...
    return mapping.get(str(state or "").upper(), str(state or "-") or "-")


def _ff_get_startup_status_text_service(self: Any) -> str:
    service_info = query_windows_service_info()
    if not service_info.get("installed"):
        return "未创建 Windows 服务"
    start_type = str(service_info.get("start_type") or "").upper()
    if start_type == "AUTO_START":
        return "已启用（Windows 服务自动启动）"
    return "已创建（Windows 服务手动启动）"


def _ff_get_startup_mode_text_service(self: Any) -> str:
    return get_startup_mode_label()


def _ff_get_startup_detail_text_service(self: Any) -> str:
    return get_startup_task_detail().replace("\n", " | ")


def _ff_get_host_status_text_service(self: Any) -> str:
    service_info = query_windows_service_info()
    runtime_state = read_managed_runtime_state()
    backend_pid = int(runtime_state.get("backend_pid") or 0)
    frontend_pid = int(runtime_state.get("frontend_pid") or 0)
    if service_info.get("running"):
        fragments = [f"服务 PID {int(service_info.get('pid') or 0)}"]
        if backend_pid > 0:
            fragments.append(f"backend {backend_pid}")
        if frontend_pid > 0:
            fragments.append(f"frontend {frontend_pid}")
        return "Windows 服务运行中 (" + ", ".join(fragments) + ")"
    if service_info.get("installed"):
        return f"Windows 服务已安装（{_service_state_label(str(service_info.get('state') or 'STOPPED'))}）"
    return "未创建 Windows 服务"


def _ff_get_host_diagnostics_text_service(self: Any) -> str:
    service_info = query_windows_service_info()
    runtime_state = read_managed_runtime_state()
    state_label = _service_state_label(str(service_info.get("state") or "STOPPED"))
    start_type_label = _service_start_type_label(str(service_info.get("start_type") or "-"))
    service_pid = int(service_info.get("pid") or 0)
    transition = str(runtime_state.get("host_last_transition", "") or "-").strip() or "-"
    started = format_runtime_timestamp(runtime_state.get("host_started_at"))
    heartbeat = format_runtime_timestamp(runtime_state.get("host_last_heartbeat_at"))

    fragments = [state_label, start_type_label]
    if service_pid > 0:
        fragments.append(f"PID {service_pid}")
    if transition not in {"-", ""}:
        fragments.append(f"状态 {transition}")
    if started != "-":
        fragments.append(f"启动 {started}")
    if heartbeat != "-":
        fragments.append(f"心跳 {heartbeat}")
    return " | ".join(fragments)


def _ff_get_manager_runtime_mode_text_service(self: Any) -> str:
    service_info = query_windows_service_info()
    if service_info.get("installed"):
        return f"Windows 服务托管（{_service_start_type_label(str(service_info.get('start_type') or '-'))}）"
    return "Windows 服务管理模式（未安装）"


def _ff_sync_startup_entry_service(self: Any, show_message: bool = False) -> None:
    enabled = bool(self.manager_state.get("launch_manager_on_startup", False))
    try:
        if enabled:
            ok, detail = install_windows_service(auto_start=True)
            if ok:
                self.record_startup_sync_result("Windows 服务自动启动已启用", detail, "")
            else:
                self.record_startup_sync_result("Windows 服务同步失败", "", detail)
                if show_message:
                    messagebox.showerror("Windows 服务", detail)
                return
        else:
            if windows_service_exists():
                ok, detail = set_windows_service_start_mode(False)
                if ok:
                    self.record_startup_sync_result("Windows 服务已切换为手动", detail, "")
                else:
                    self.record_startup_sync_result("Windows 服务同步失败", "", detail)
                    if show_message:
                        messagebox.showerror("Windows 服务", detail)
                ...
                self.record_startup_sync_result("Windows 服务未安装", "", "")
    except Exception as exc:
        self.record_startup_sync_result("Windows 服务同步失败", "", str(exc))
        if show_message:
            messagebox.showerror("Windows 服务", str(exc))
        return

    if show_message:
        summary = self.get_startup_last_result_text()
        messagebox.showinfo("Windows 服务", summary)


def _ff_enable_startup_now_service(self: Any) -> None:
    self.manager_state["launch_manager_on_startup"] = True
    if "launch_manager_on_startup" in self.manager_option_vars:
        self.manager_option_vars["launch_manager_on_startup"].set(True)
    write_state_file(STATE_PATH, self.manager_state)
...
def _ff_disable_startup_now_service(self: Any) -> None:
    self.manager_state["launch_manager_on_startup"] = False
    if "launch_manager_on_startup" in self.manager_option_vars:
        self.manager_option_vars["launch_manager_on_startup"].set(False)
    write_state_file(STATE_PATH, self.manager_state)
...
def _ff_rebuild_startup_now_service(self: Any) -> None:
    self.manager_state["launch_manager_on_startup"] = True
    if "launch_manager_on_startup" in self.manager_option_vars:
        self.manager_option_vars["launch_manager_on_startup"].set(True)
    write_state_file(STATE_PATH, self.manager_state)
...
def _ff_show_startup_detail_service(self: Any) -> None:
    messagebox.showinfo("Windows 服务详情", get_startup_task_detail())


def _ff_handle_start_host_service(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    saved, save_error = _safe_save_config_before_service_action(self)
    auto_start = bool(self.manager_state.get("launch_manager_on_startup", False))
    install_ok, install_message = install_windows_service(auto_start=auto_start)
    if not install_ok:
        append_audit_log("windows_service_install", "failed", install_message)
        if show_dialog:
            messagebox.showwarning("Windows 服务", install_message)
        self.refresh_status()
        return False, install_message
    ok, message = start_windows_service()
    append_audit_log("windows_service_start", "success" if ok else "failed", message)
    if not saved and save_error:
        message += f"\n\n配置保存提醒：{save_error}"
    self.refresh_status()
    if show_dialog:
        (messagebox.showinfo if ok else messagebox.showwarning)("Windows 服务", message)
    return ok, message


def _ff_handle_stop_host_service(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    ok, message = stop_windows_service()
    append_audit_log("windows_service_stop", "success" if ok else "failed", message)
    self.refresh_status()
    if show_dialog:
        (messagebox.showinfo if ok else messagebox.showwarning)("Windows 服务", message)
    return ok, message
...
def _ff_handle_restart_host_service(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    saved, save_error = _safe_save_config_before_service_action(self)
    ok, message = restart_windows_service()
    append_audit_log("windows_service_restart", "success" if ok else "failed", message)
    if not saved and save_error:
        message += f"\n\n配置保存提醒：{save_error}"
    self.refresh_status()
    if show_dialog:
...
def _ff_handle_install_host_service(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    auto_start = bool(self.manager_state.get("launch_manager_on_startup", False))
    ok, message = install_windows_service(auto_start=auto_start)
    append_audit_log("windows_service_install", "success" if ok else "failed", message)
    self.refresh_status()
    if show_dialog:
        (messagebox.showinfo if ok else messagebox.showwarning)("Windows 服务", message)
    return ok, message
...
def _ff_handle_remove_host_service(self: Any, show_dialog: bool = True) -> Tuple[bool, str]:
    ok, message = remove_windows_service()
    append_audit_log("windows_service_remove", "success" if ok else "failed", message)
    self.refresh_status()
    if show_dialog:
        (messagebox.showinfo if ok else messagebox.showwarning)("Windows 服务", message)
    return ok, message
...
def _ff_build_manager_tab_windows_service(self: Any, parent: ttk.Frame) -> None:
    parent.columnconfigure(0, weight=1)
    ...
    option_items = [
        ("auto_restart_backend", "后端异常退出后自动重启"),
        ("auto_restart_frontend", "前端异常退出后自动重启"),
        ("start_backend_on_launch", "服务宿主启动时自动拉起后端"),
        ("start_frontend_on_launch", "服务宿主启动时自动拉起前端"),
        ("hide_to_tray_on_close", "关闭窗口时最小化到托盘"),
        ("launch_manager_on_startup", "Windows 服务设为自动启动"),
        ("enable_health_check", "启用前后端健康检查"),
        ("enable_db_monitor", "启用数据库连接监控"),
    ]
    for idx, (key, label) in enumerate(option_items):
    ...
    service_actions = ttk.Frame(options)
    service_actions.grid(row=(len(option_items) + 1) // 2, column=0, columnspan=2, sticky="w", padx=10, pady=(10, 0))
    ttk.Button(service_actions, text="创建 Windows 服务", command=self.handle_install_host_service).pack(side="left", padx=(0, 8))
    ttk.Button(service_actions, text="启动 Windows 服务", command=self.handle_start_host).pack(side="left", padx=(0, 8))
    ttk.Button(service_actions, text="停止 Windows 服务", command=self.handle_stop_host).pack(side="left", padx=(0, 8))
    ttk.Button(service_actions, text="重启 Windows 服务", command=self.handle_restart_host).pack(side="left", padx=(0, 8))
    ttk.Button(service_actions, text="删除 Windows 服务", command=self.handle_remove_host_service).pack(side="left", padx=(0, 8))

    service_actions_2 = ttk.Frame(options)
    service_actions_2.grid(row=(len(option_items) + 3) // 2, column=0, columnspan=2, sticky="w", padx=10, pady=(8, 0))
    ttk.Button(service_actions_2, text="查看服务详情", command=self.show_startup_detail).pack(side="left", padx=(0, 8))
    ttk.Button(service_actions_2, text="打开服务日志", command=self.open_host_log).pack(side="left", padx=(0, 8))
    ttk.Button(service_actions_2, text="打开审计日志", command=self.open_audit_log).pack(side="left", padx=(0, 8))
    ttk.Label(service_actions_2, textvariable=self.status_vars["host_status"]).pack(side="left", padx=(8, 12))

    diagnostics = ttk.LabelFrame(parent, text="服务诊断与审计", padding=16)
    diagnostics.pack(fill="x", padx=10, pady=(0, 10))
    ttk.Label(diagnostics, text="服务诊断：", width=14).grid(row=0, column=0, sticky="nw", padx=6, pady=4)
    ttk.Label(diagnostics, textvariable=self.status_vars["host_diagnostics"], justify="left", wraplength=860).grid(
        row=0, column=1, sticky="w", padx=6, pady=4
    )
    ttk.Label(diagnostics, text="最近审计：", width=14).grid(row=1, column=0, sticky="nw", padx=6, pady=4)
    ttk.Label(diagnostics, textvariable=self.status_vars["audit_status"], justify="left", wraplength=860).grid(
    ...
    ttk.Label(
        tips,
        text=(
            "服务管理器现在通过原生 Windows 服务统一托管 FinFlow 生产环境运行。\n"
            "创建服务后，系统服务控制管理器会拉起 FinFlowManagerHost，再由宿主统一维护后端和前端进程。\n"
            "“Windows 服务设为自动启动”表示服务器重启后由系统自动拉起，不再依赖计划任务或启动目录脚本。\n"
            "部署、更新、环境配置和日志检查仍然由当前管理器统一处理。"
        ),
        justify="left",
        wraplength=880,
    ).pack(anchor="w")
...
def _ff_build_takeover_check_summary_windows_service(app: Any) -> str:
    config = app.get_effective_config()
    lines: List[str] = ["FinFlow 服务接管检查", ""]

    layout_ok, layout_issues = evaluate_manager_runtime_layout()
    lines.append(f"[{'OK' if layout_ok else 'WARN'}] 运行目录结构: {'正常' if layout_ok else '存在问题'}")
    if layout_issues:
        lines.extend([f"  - {item}" for item in layout_issues[:6]])

    backend_python = get_backend_python_executable()
    lines.append(f"[{'OK' if backend_python.exists() else 'WARN'}] 后端运行时 Python: {backend_python}")
    lines.append(f"[{'OK' if ENV_PATH.exists() else 'WARN'}] 后端环境文件: backend/.env")
    lines.append(f"[{'OK' if KEY_PATH.exists() else 'WARN'}] 加密密钥文件: backend/.encryption.key")
    lines.append(f"[{'OK' if (DIST_DIR / 'index.html').exists() else 'WARN'}] 前端静态构建: frontend/dist")
    lines.append(f"[{'OK' if app.check_git_available() else 'WARN'}] Git 工具")
    lines.append(f"[{'OK' if is_command_available('node') else 'WARN'}] Node.js")

    sqlcmd_value = app.ops_vars.get("sqlcmd_path").get().strip() if app.ops_vars.get("sqlcmd_path") else "sqlcmd"
    lines.append(f"[{'OK' if is_command_available(sqlcmd_value or 'sqlcmd') else 'WARN'}] sqlcmd: {sqlcmd_value or 'sqlcmd'}")

    service_info = query_windows_service_info()
    lines.append(f"[{'OK' if service_info.get('installed') else 'WARN'}] Windows 服务: {app.get_startup_status_text()}")
    lines.append(f"[{'OK' if service_info.get('running') else 'WARN'}] 服务主控: {app.get_host_status_text()}")
    ...
    if port.isdigit():
        ...
        if owner:
            ...
            lines.append(f"[OK] 后端端口 {port}: 空闲")
    else:
        lines.append(f"[WARN] APP_PORT 配置无效: {port}")
    ...
    recommendations: List[str] = []
    if not service_info.get("installed"):
        recommendations.append("先创建 Windows 服务，再由服务管理器统一启停和检查。")
    if service_info.get("installed") and not service_info.get("running"):
        recommendations.append("启动 Windows 服务，让系统服务主控接管前后端运行。")
    if not ENV_PATH.exists():
        recommendations.append("补齐 backend/.env，确保生产环境配置完整。")
    if not KEY_PATH.exists():
        recommendations.append("补齐 backend/.encryption.key。")
    if not backend_python.exists():
        recommendations.append("修复后端运行时 Python 环境。")
    if not (DIST_DIR / "index.html").exists():
        recommendations.append("重新构建前端静态资源 frontend/dist。")
    ...
    if recommendations:
        ...
        lines.append("- 当前服务托管条件已基本就绪，可直接进行启动和验证。")
    return "\n".join(lines)


def _ff_build_status_tab_windows_service_clean(self: Any, parent: ttk.Frame) -> None:
    db_config_state, db_config_detail = describe_database_configuration_from_disk()
    self.status_badges = {}
    self.status_summary_cards = {}
    self.status_nav_items = {}
    self.status_section_frames = {}
    self.service_action_buttons = {}
    ...
    for key, value in status_items.items():
        existing_var = self.status_vars.get(key)
        if existing_var is None:
            self.status_vars[key] = tk.StringVar(value=value)
        else:
            existing_var.set(value)
    ...
    ttk.Label(
        nav_wrap,
        text="按主题查看 Windows 服务、前后端、数据库与快捷操作，让首屏信息保持清晰。",
        justify="left",
        foreground="#666666",
        wraplength=210,
    ).pack(anchor="w", pady=(4, 10))
    ...
    self.status_section_var = tk.StringVar(value="overview")
    status_nav = [
        ("overview", "概览", "查看首屏摘要卡片和核心运行信息。"),
        ("services", "服务组件", "查看后端、前端和访问地址。"),
        ("database", "数据库", "查看数据库配置、连接和监控状态。"),
        ("manager", "服务主控", "查看 Windows 服务状态、模式和诊断信息。"),
        ("actions", "快捷操作", "集中执行常用维护操作。"),
    ]
    self.create_side_nav(nav_wrap, status_nav, self.status_section_var, self.show_status_section, self.status_nav_items)
    ...
    def create_section(title: str, description: str) -> Tuple[ttk.Frame, ttk.Frame]:
        ...
        frame.columnconfigure(0, weight=1)
        ttk.Label(frame, text=title, font=("Microsoft YaHei UI", 12, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Label(frame, text=description, foreground="#666666", justify="left").grid(row=1, column=0, sticky="w", pady=(4, 10))
        body = ttk.Frame(frame)
        body.grid(row=2, column=0, sticky="nsew")
        body.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
    ...
    def add_field_group(container: ttk.Frame, title: str, fields: List[Tuple[str, str]], row: int, column: int = 0) -> None:
        group = ttk.LabelFrame(container, text=title, padding=12)
        group.grid(row=row, column=column, sticky="nsew", padx=6, pady=6)
        group.columnconfigure(1, weight=1)
        for field_row, (label_text, key) in enumerate(fields):
            ttk.Label(group, text=f"{label_text}：").grid(row=field_row, column=0, sticky="nw", padx=(0, 8), pady=4)
            if key in {"backend_status", "frontend_status", "backend_health_status", "frontend_health_status", "db_connection_status", "db_monitor_status"}:
                badge = tk.Label(group, textvariable=self.status_vars[key], anchor="w", justify="left", padx=8, pady=3, relief="groove", bd=1)
                badge.grid(row=field_row, column=1, sticky="ew", pady=4)
                self.status_badges[key] = badge
            else:
                wrap_length = 780 if key in {"host_diagnostics", "audit_status"} else 520 if key in {"startup_detail", "startup_last_result", "startup_last_error", "project_root"} else 360
                ttk.Label(group, textvariable=self.status_vars[key], justify="left", wraplength=wrap_length).grid(row=field_row, column=1, sticky="w", pady=4)

    def add_summary_card(container: ttk.Frame, title: str, key: str, sub_key: str, row: int, column: int) -> None:
        bg, fg = self.get_status_badge_palette(key, self.status_vars[key].get())
        card = tk.Frame(container, bg=bg, bd=1, relief="solid", padx=14, pady=12, highlightthickness=0)
        card.grid(row=row, column=column, sticky="nsew", padx=6, pady=6)
        title_label = tk.Label(card, text=title, bg=bg, fg="#374151", font=("Microsoft YaHei UI", 9, "bold"), anchor="w", justify="left")
        title_label.pack(anchor="w")
        value_label = tk.Label(card, textvariable=self.status_vars[key], bg=bg, fg=fg, font=("Microsoft YaHei UI", 12, "bold"), anchor="w", justify="left", wraplength=220)
        value_label.pack(anchor="w", pady=(8, 6))
        sub_label = tk.Label(card, textvariable=self.status_vars[sub_key], bg=bg, fg="#4b5563", font=("Microsoft YaHei UI", 8), anchor="w", justify="left", wraplength=220)
        sub_label.pack(anchor="w")
        self.status_summary_cards[key] = {
            "card": card,
            "title": title_label,
            "value": value_label,
            "sub": sub_label,
    ...
    def add_action_group(container: ttk.Frame, title: str, description: str, actions: List[Tuple[str, Any]], row: int, accent: str) -> None:
        group = ttk.LabelFrame(container, text=title, padding=12)
        group.grid(row=row, column=0, sticky="ew", pady=6)
        group.columnconfigure(0, weight=1)
        intro = tk.Frame(group, bg=accent, padx=10, pady=8)
        intro.grid(row=0, column=0, sticky="ew", pady=(0, 10))
        tk.Label(intro, text=description, bg=accent, fg="#ffffff", justify="left", wraplength=780, font=("Microsoft YaHei UI", 9)).pack(anchor="w")
        ...
        for idx, (button_text, command) in enumerate(actions):
            button = ttk.Button(buttons, text=button_text, command=command)
            button.grid(row=idx // 3, column=idx % 3, sticky="ew", padx=4, pady=4)
            action_key = action_map.get(button_text)
            if action_key:
                self.service_action_buttons[action_key] = button

    overview_frame, overview_body = create_section("概览", "把最重要的服务状态和入口先放到首屏。")
    self.status_section_frames["overview"] = overview_frame
    overview_body.columnconfigure(0, weight=1)
    overview_body.columnconfigure(1, weight=1)
    ...
    add_summary_card(summary_cards, "前端状态", "frontend_status", "frontend_url", 0, 1)
    add_summary_card(summary_cards, "数据库连接", "db_connection_status", "db_last_check_at", 0, 2)
    add_summary_card(summary_cards, "Windows 服务", "host_status", "startup_status", 0, 3)

    add_field_group(overview_body, "基础信息", [("项目目录", "project_root"), ("应用入口", "app_url"), ("后端环境文件", "env_file"), ("加密密钥文件", "key_file"), ("前端静态构建", "dist_dir")], 1, 0)
    add_field_group(overview_body, "服务托管", [("自动启动状态", "startup_status"), ("托管方式", "startup_mode"), ("管理模式", "manager_runtime_mode"), ("服务详情", "startup_detail")], 1, 1)
    add_field_group(overview_body, "最近结果", [("最近同步结果", "startup_last_result"), ("最近错误摘要", "startup_last_error")], 2, 0)
    add_field_group(overview_body, "诊断与审计", [("服务主控", "host_status"), ("服务诊断", "host_diagnostics"), ("最近审计", "audit_status")], 2, 1)

    services_frame, services_body = create_section("服务组件", "分别查看后端、前端的运行状态和访问地址。")
    self.status_section_frames["services"] = services_frame
    services_body.columnconfigure(0, weight=1)
    services_body.columnconfigure(1, weight=1)
    add_field_group(services_body, "后端服务", [("运行状态", "backend_status"), ("端口占用", "backend_port_owner"), ("健康检查", "backend_health_status"), ("访问地址", "backend_url")], 0, 0)
    add_field_group(services_body, "前端服务", [("运行状态", "frontend_status"), ("端口占用", "frontend_port_owner"), ("健康检查", "frontend_health_status"), ("访问地址", "frontend_url")], 0, 1)

    database_frame, database_body = create_section("数据库", "查看数据库配置、连接状态和监控信息。")
    self.status_section_frames["database"] = database_frame
    database_body.columnconfigure(0, weight=1)
    add_field_group(database_body, "数据库状态", [("配置状态", "db_config_status"), ("连接状态", "db_connection_status"), ("监控状态", "db_monitor_status"), ("上次检查", "db_last_check_at")], 0, 0)

    manager_frame, manager_body = create_section("服务主控", "聚焦 Windows 服务本身的运行方式、状态和诊断。")
    self.status_section_frames["manager"] = manager_frame
    manager_body.columnconfigure(0, weight=1)
    manager_body.columnconfigure(1, weight=1)
    add_field_group(manager_body, "服务信息", [("Windows 服务", "host_status"), ("管理模式", "manager_runtime_mode"), ("启动方式", "startup_mode"), ("自动启动状态", "startup_status")], 0, 0)
    add_field_group(manager_body, "服务记录", [("服务详情", "startup_detail"), ("最近同步结果", "startup_last_result"), ("最近错误摘要", "startup_last_error")], 0, 1)
    add_field_group(manager_body, "服务诊断", [("诊断信息", "host_diagnostics"), ("最近审计", "audit_status")], 1, 0)

    actions_frame, actions_body = create_section("快捷操作", "常用操作按场景分组，优先把生产环境维护动作放在一起。")
    self.status_section_frames["actions"] = actions_frame
    actions_body.columnconfigure(0, weight=1)
    add_action_group(actions_body, "常用操作", "用于整套服务的启动、停止和重启。", [("启动全部", self.handle_start_all), ("停止全部", self.handle_stop_all), ("重启全部", self.handle_restart_all), ("打开前端", self.open_frontend), ("打开日志目录", self.open_logs_folder), ("最小化到托盘", self.hide_to_tray)], 0, "#2563eb")
    add_action_group(actions_body, "服务维护", "用于逐项维护后端、前端和 Windows 服务。", [("启动后端", self.handle_start_backend), ("停止后端", self.handle_stop_backend), ("重启后端", self.handle_restart_backend), ("启动前端", self.handle_start_frontend), ("停止前端", self.handle_stop_frontend), ("重启前端", self.handle_restart_frontend), ("检查数据库", self.check_database_connection), ("启动 Windows 服务", self.handle_start_host), ("停止 Windows 服务", self.handle_stop_host), ("重启 Windows 服务", self.handle_restart_host), ("打开服务日志", self.open_host_log), ("查看服务详情", self.show_startup_detail)], 1, "#0f766e")
    add_action_group(actions_body, "高优先级维护", "适合处理端口冲突、接管和启动方式调整。", [("接管后端", self.handle_takeover_backend), ("释放端口", self.handle_force_release_port), ("启用自动启动", self.enable_startup_now), ("关闭自动启动", self.disable_startup_now), ("重建服务配置", self.rebuild_startup_now)], 2, "#b42318")
...
FinFlowManagerApp.get_startup_status_text = _ff_get_startup_status_text_service
FinFlowManagerApp.get_startup_mode_text = _ff_get_startup_mode_text_service
FinFlowManagerApp.get_startup_detail_text = _ff_get_startup_detail_text_service
FinFlowManagerApp.get_host_status_text = _ff_get_host_status_text_service
FinFlowManagerApp.get_host_diagnostics_text = _ff_get_host_diagnostics_text_service
FinFlowManagerApp.get_manager_runtime_mode_text = _ff_get_manager_runtime_mode_text_service
FinFlowManagerApp.sync_startup_entry = _ff_sync_startup_entry_service
FinFlowManagerApp.enable_startup_now = _ff_enable_startup_now_service
FinFlowManagerApp.disable_startup_now = _ff_disable_startup_now_service
FinFlowManagerApp.rebuild_startup_now = _ff_rebuild_startup_now_service
FinFlowManagerApp.show_startup_detail = _ff_show_startup_detail_service
FinFlowManagerApp.handle_start_host = _ff_handle_start_host_service
FinFlowManagerApp.handle_stop_host = _ff_handle_stop_host_service
FinFlowManagerApp.handle_restart_host = _ff_handle_restart_host_service
FinFlowManagerApp.handle_install_host_service = _ff_handle_install_host_service
FinFlowManagerApp.handle_remove_host_service = _ff_handle_remove_host_service
FinFlowManagerApp.build_manager_tab = _ff_build_manager_tab_windows_service
FinFlowManagerApp.build_status_tab = _ff_build_status_tab_windows_service_clean
build_takeover_check_summary = _ff_build_takeover_check_summary_windows_service


def _ff_build_manager_tab_windows_service_final(self: Any, parent: ttk.Frame) -> None:
    parent.columnconfigure(0, weight=1)

    options = ttk.LabelFrame(parent, text="服务管理选项", padding=16)
    options.pack(fill="x", padx=10, pady=10)

    option_items = [
        ("auto_restart_backend", "后端异常退出后自动重启"),
        ("auto_restart_frontend", "前端异常退出后自动重启"),
        ("start_backend_on_launch", "服务主控启动时自动拉起后端"),
        ("start_frontend_on_launch", "服务主控启动时自动拉起前端"),
        ("launch_manager_on_startup", "Windows 服务设为自动启动"),
        ("enable_health_check", "启用前后端健康检查"),
        ("enable_db_monitor", "启用数据库连接监控"),
        ("hide_to_tray_on_close", "关闭窗口时最小化到托盘"),
    ...
    common_actions = ttk.LabelFrame(parent, text="常用服务操作", padding=16)
    common_actions.pack(fill="x", padx=10, pady=(0, 10))
    ttk.Button(common_actions, text="创建 Windows 服务", command=self.handle_install_host_service).pack(side="left", padx=(0, 8))
    ttk.Button(common_actions, text="启动 Windows 服务", command=self.handle_start_host).pack(side="left", padx=(0, 8))
    ttk.Button(common_actions, text="停止 Windows 服务", command=self.handle_stop_host).pack(side="left", padx=(0, 8))
    ttk.Button(common_actions, text="重启 Windows 服务", command=self.handle_restart_host).pack(side="left", padx=(0, 8))
    ttk.Button(common_actions, text="查看服务详情", command=self.show_startup_detail).pack(side="left", padx=(0, 8))
    ttk.Label(common_actions, textvariable=self.status_vars["host_status"]).pack(side="left", padx=(10, 0))

    support_actions = ttk.LabelFrame(parent, text="辅助操作", padding=16)
    support_actions.pack(fill="x", padx=10, pady=(0, 10))
    ttk.Button(support_actions, text="打开服务日志", command=self.open_host_log).pack(side="left", padx=(0, 8))
    ttk.Button(support_actions, text="打开审计日志", command=self.open_audit_log).pack(side="left", padx=(0, 8))
    ttk.Button(support_actions, text="启用自动启动", command=self.enable_startup_now).pack(side="left", padx=(0, 8))
    ttk.Button(support_actions, text="关闭自动启动", command=self.disable_startup_now).pack(side="left", padx=(0, 8))
    ttk.Button(support_actions, text="重建服务配置", command=self.rebuild_startup_now).pack(side="left", padx=(0, 8))

    danger_actions = ttk.LabelFrame(parent, text="高风险操作", padding=16)
    danger_actions.pack(fill="x", padx=10, pady=(0, 10))
    ttk.Label(
        danger_actions,
        text="删除 Windows 服务会移除系统托管入口，通常仅在服务损坏或需要彻底重建时使用。",
        foreground="#a16207",
        justify="left",
        wraplength=860,
    ).pack(anchor="w", pady=(0, 8))
    ttk.Button(danger_actions, text="删除 Windows 服务", command=self.handle_remove_host_service).pack(anchor="w")

    diagnostics = ttk.LabelFrame(parent, text="服务诊断", padding=16)
    diagnostics.pack(fill="x", padx=10, pady=(0, 10))
    ttk.Label(diagnostics, text="服务诊断：", width=14).grid(row=0, column=0, sticky="nw", padx=6, pady=4)
    ttk.Label(diagnostics, textvariable=self.status_vars["host_diagnostics"], justify="left", wraplength=860).grid(row=0, column=1, sticky="w", padx=6, pady=4)
    ttk.Label(diagnostics, text="最近审计：", width=14).grid(row=1, column=0, sticky="nw", padx=6, pady=4)
...
def _ff_build_takeover_check_summary_windows_service_final(app: Any) -> str:
    config = app.get_effective_config()
    lines: List[str] = ["FinFlow 服务接管检查", ""]

    layout_ok, layout_issues = evaluate_manager_runtime_layout()
    lines.append(f"[{'OK' if layout_ok else 'WARN'}] 目录结构: {'正常' if layout_ok else '存在问题'}")
    if layout_issues:
        lines.extend([f"  - {item}" for item in layout_issues[:6]])

    backend_python = get_backend_python_executable()
    lines.append(f"[{'OK' if backend_python.exists() else 'WARN'}] 后端运行时 Python: {backend_python}")
    lines.append(f"[{'OK' if ENV_PATH.exists() else 'WARN'}] backend/.env")
    lines.append(f"[{'OK' if KEY_PATH.exists() else 'WARN'}] backend/.encryption.key")
    lines.append(f"[{'OK' if (DIST_DIR / 'index.html').exists() else 'WARN'}] frontend/dist")
    ...
    if port.isdigit():
        owner = get_port_owner_info(int(port))
        lines.append(f"[{'WARN' if owner else 'OK'}] 后端端口 {port}: {build_port_owner_label(owner) if owner else '空闲'}")
    else:
        lines.append(f"[WARN] APP_PORT 无效: {port}")
    ...
    recommendations: List[str] = []
    if not service_info.get("installed"):
        recommendations.append("先创建 Windows 服务。")
    if service_info.get("installed") and not service_info.get("running"):
        recommendations.append("启动 Windows 服务，让系统接管前后端。")
    if not ENV_PATH.exists():
        recommendations.append("补齐 backend/.env。")
    if not KEY_PATH.exists():
        recommendations.append("补齐 backend/.encryption.key。")
    if not (DIST_DIR / "index.html").exists():
        recommendations.append("重新构建 frontend/dist。")
    ...
    if recommendations:
        ...
        lines.append("- 当前托管条件已就绪，可以直接启动并验证。")
    return "\n".join(lines)


def _ff_build_status_tab_windows_service_final(self: Any, parent: ttk.Frame) -> None:
    _ff_build_status_tab_windows_service_clean(self, parent)
    if "startup_status" in self.status_vars:
        self.status_vars["startup_status"].set(self.get_startup_status_text())
    if "startup_mode" in self.status_vars:
        self.status_vars["startup_mode"].set(self.get_startup_mode_text())
    if "startup_detail" in self.status_vars:
...
FinFlowManagerApp.build_manager_tab = _ff_build_manager_tab_windows_service_final
FinFlowManagerApp.build_status_tab = _ff_build_status_tab_windows_service_final
build_takeover_check_summary = _ff_build_takeover_check_summary_windows_service_final


def _ff_refresh_status_windows_service_final(self: Any) -> None:
    backend_status = self.backend.poll_status()
    frontend_status = self.frontend.poll_status()
    config = self.get_effective_config()
    backend_port = (config.get("APP_PORT") or "8100").strip() or "8100"
    frontend_settings = resolve_frontend_service_settings(config)
    frontend_port = frontend_settings["frontend_port"]
    backend_owner_text = self.status_vars["backend_port_owner"].get() or "未检测"
    frontend_owner_text = self.status_vars["frontend_port_owner"].get() or "未检测"
    ...
    if active_jobs is None:
        active_jobs = {}
        self._background_refresh_jobs = active_jobs
    if active_jobs.get("status_ports"):
        return

    active_jobs["status_ports"] = True

    backend_running = self.backend.is_running()
    frontend_running = self.frontend.is_running()
    backend_manager_pid = safe_process_pid(self.backend.process) if backend_running else None
    frontend_manager_pid = safe_process_pid(self.frontend.process) if frontend_running else None

    def worker() -> Dict[str, str]:
        resolved_backend_owner = "未占用" if backend_port.isdigit() else "端口无效"
        resolved_frontend_owner = "未占用" if frontend_port.isdigit() else "端口无效"

        if backend_port.isdigit():
            owner = get_port_owner_info(int(backend_port))
            if owner:
                resolved_backend_owner = build_port_owner_label(owner, backend_manager_pid)
        if frontend_port.isdigit():
            owner = get_port_owner_info(int(frontend_port))
            if owner:
                resolved_frontend_owner = build_port_owner_label(owner, frontend_manager_pid)

        return {
            "backend_port_owner": resolved_backend_owner,
            "frontend_port_owner": resolved_frontend_owner,
        }

    def finalize(result: Dict[str, str] | None = None, error: Exception | None = None) -> None:
        active_jobs["status_ports"] = False
        if self.exiting:
            return
        if error is not None:
            return
        assert result is not None
        self.status_vars["backend_port_owner"].set(result["backend_port_owner"])
        self.status_vars["frontend_port_owner"].set(result["frontend_port_owner"])
        self.refresh_status_badges()
    ...
    threading.Thread(target=run_worker, name="finflow-status-ports", daemon=True).start()


def _ff_refresh_status_badges_windows_service_final(self: Any) -> None:
    badges = getattr(self, "status_badges", {}) or {}
    ...
    summary_cards = getattr(self, "status_summary_cards", {}) or {}
    for key, widgets in summary_cards.items():
        if key not in self.status_vars:
            continue
        bg, fg = self.get_status_badge_palette(key, self.status_vars[key].get())
        card = widgets.get("card")
        title = widgets.get("title")
        value = widgets.get("value")
        sub = widgets.get("sub")
        for widget, widget_bg, widget_fg in (
            (card, bg, None),
            (title, bg, "#374151"),
            (value, bg, fg),
            (sub, bg, "#4b5563"),
...
FinFlowManagerApp.refresh_status_badges = _ff_refresh_status_badges_windows_service_final
FinFlowManagerApp.refresh_status = _ff_refresh_status_windows_service_final
